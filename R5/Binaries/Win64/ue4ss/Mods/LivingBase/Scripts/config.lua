...
-- TEMPORARY crash-isolation kill switch (2026-09-16, RedFalcon: "it seems like the belts are a
-- pretty common source. Can we unlink those from detect and save and see if it stabilizes?") --
-- multiple crash dumps this session (all sharing the same/nearby UE4SS.dll exception address,
-- even across an official-vs-updated UE4SS.dll swap and with the Custom tab's own detect gate
-- temporarily bypassed) have landed inside the belts/straps BuildedCompositeMeshes walk
-- specifically (findBeltStrapComponent, then its buildBeltStrapMap replacement). Set true to skip
-- the belts/straps read entirely from BOTH Read Current (main.lua step 8b) and Save Customizations
-- (Spawner.BuildCustomStateLines).
--
-- Re-enabled (2026-09-16, RedFalcon: "can we try to hook the belt back in now?") -- the real
-- trigger turned out to be the Barbie-only auto-detect/auto-save firing immediately after a dense
-- underwear-strip mutation burst (now disabled at its own two call sites, spawner.lua), not
-- belts/straps reading itself; that read already got its own real fix too (buildBeltStrapMap's
-- single-pass walk, replacing 4 separate re-walks of the same array). Flip back to true if belts
-- reading turns out to still be a problem on its own.
Config.TEMP_SKIP_BELTSTRAPS_READ = false
...
-- CUSTOM_RESTORE_STAGGER_MS (2026-09-24, RedFalcon: "i also think we can reduce the time to custom
-- restores") -- Spawner.RestoreCustomState (the Custom-tab cosmetics pass, AFTER persist.txt's own
-- restore + post-process finish) used to just reuse RESTORE_STAGGER_MS (350ms), copying the movers'
-- own pacing purely by analogy when its ordering race got fixed (see that function's own header
-- comment) -- never independently tuned for this specific, lighter workload (no AI wake/possession,
-- just composite/clothing property writes on an ALREADY-BUILT actor). Split into its own knob so it
-- can be tuned without touching the movers' proven-safe cadence. 200ms is a first, moderate cut --
-- between the cheap statics rate (150) and the AI-pawn mover rate (350), not a wild guess. If a
-- large base with many customized Barbies/statues still hitches or crashes on load, raise this back
-- toward 350 first before suspecting anything else.
Config.CUSTOM_RESTORE_STAGGER_MS = 200
-- Widened 400->900 (2026-09-16, RedFalcon: a crash-to-desktop during world load, same UE4SS.dll
-- exception address as every Custom-tab crash diagnosed the same session, died mid-way through
-- Testbed.RestoreHook's own ability-cleanup work for one restored actor -- i.e. this same "dense
-- burst of native calls" instability, just in pre-existing restore code rather than anything new.
-- The per-actor pacing this value already provides is the one lever available here without a risky
-- rewrite of RESTORE_RULES' own per-rule apply functions; widening it gives the engine/UE4SS more
-- recovery time between each actor's own burst.
Config.RESTORE_POSTPROCESS_SPACING_MS = 900
-- Same mitigation, applied to the Custom tab's own "Read Current" (2026-09-16) -- that function
-- used to run all 8 of its steps (100+ native reflection calls) inside ONE ExecuteInGameThread
-- callback on a single frame, the densest native-call burst anywhere in the Custom tab and the
-- most repeatedly-crashed code path this session (4 separate crash dumps, all the same UE4SS.dll
-- exception address, from different steps each time). pollCustomColorReadRequest (main.lua) now
-- chains its 8 steps one per frame, paced by this value, instead of one synchronous burst.
Config.CUSTOM_READ_STEP_SPACING_MS = 50
...
-- Config.DrownedSkinSwapRules(sex) / Config.GhoulSkinSwapRules(sex) / Config.IsraelSkinSwapRules(sex)
-- -- 3 new skin tones (2026-09-14) for the Custom tab's new realtime "Skin Tone" swatch selector
-- (RedFalcon: "I am also adding a couple we don't currently have"), kept SEPARATE from
-- Config.CUSTOM_SKIN_TONES/Spawner.TestApplySkinFamily above (the OLD tree-based "Custom > Skin
-- Tones" feature) so this never risks that feature's own spawn_menu.ini stale-index gotcha (see
-- project_spawn_menu_ini_stale_indices memory) -- see Config.CUSTOM_TAB_SKIN_TONES/
-- Spawner.ApplyCustomTabSkinTone below for the new feature these actually feed.
--
-- Drowned: RedFalcon's own paths are Male-named ONLY (MI_Drowned_Male_<Size>) but "Apply to both
-- male and female" -- there is no separate Female Drowned asset, so a Female target's current
-- material (still matched by ITS OWN "_Female_<size>" suffix) swaps to that same Male-named asset
-- regardless, same "one real asset, both sexes route to it" shape as Config.CorruptedSkinSwapRules'
-- own Female branch.
function Config.DrownedSkinSwapRules(sex)
  sex = sex or "Female"
  local dir = HMN .. "Drowned/Materials/"
  local rules = {}
  for _, size in ipairs({ "Large", "Medium", "Small" }) do
    table.insert(rules, {
      match = "MI_%a+_" .. sex .. "_" .. size,
      to = objPath(dir, "MI_Drowned_Male_" .. size),
    })
  end
  return rules
end

-- Ghoul: a single asset with NO Small/Medium/Large variant at all (RedFalcon: "Apply to both male
-- and female"), the same real Israel-boss materials folder Config.IsraelSkinSwapRules below also
-- uses (a DIFFERENT filename inside it -- MI_Boss_IsraelHands vs MI_NPC_IsraelHands, both real,
-- distinct assets RedFalcon confirmed). Still generates a match rule per size so it swaps
-- regardless of whichever build the target's CURRENT material happens to be, all pointing at the
-- one Ghoul asset -- once applied, Spawner.TestSetSkinSize/Physique has nothing further to act on
-- (the applied name no longer ends in a size word), which is the correct, expected "just the one"
-- behavior, not a bug.
function Config.GhoulSkinSwapRules(sex)
  sex = sex or "Female"
  local dir = "/Game/Character/Skeletal_Meshes/Bosses/IsraelHands/Materials/"
  local rules = {}
  for _, size in ipairs({ "Large", "Medium", "Small" }) do
    table.insert(rules, {
      match = "MI_%a+_" .. sex .. "_" .. size,
      to = objPath(dir, "MI_Boss_IsraelHands"),
    })
  end
  return rules
end

-- Israel: Male gets the real Israel boss material (same one-size-fits-all shape as Ghoul above);
-- Female has no equivalent asset at all, so RedFalcon's own call was "Apply Albion to Female
-- instead" -- delegates straight to the existing Config.SkinFamilySwapRules for that case (a REAL
-- substitution, not a fake label -- Spawner.TestReadSkinTone will correctly report "Albion", not
-- "Israel", for a Female target this was applied to).
function Config.IsraelSkinSwapRules(sex)
  sex = sex or "Female"
  if sex == "Female" then
    return Config.SkinFamilySwapRules("Albion", "Female")
  end
  local dir = "/Game/Character/Skeletal_Meshes/Bosses/IsraelHands/Materials/"
  local rules = {}
  for _, size in ipairs({ "Large", "Medium", "Small" }) do
    table.insert(rules, {
      match = "MI_%a+_Male_" .. size,
      to = objPath(dir, "MI_NPC_IsraelHands"),
    })
  end
  return rules
end

-- Config.CUSTOM_TAB_SKIN_TONES -- the new Custom tab "Skin Tone" swatch selector's own name list
-- (2026-09-14, RedFalcon: "put in place of where Physique was a Skin Tone Swatch Selector"),
-- ALPHABETICAL-BY-NOTHING -- this exact order is RedFalcon's own requested swatch order, do not
-- re-sort. Reuses the same 7 Config.SKIN_FAMILIES + Config.CorruptedSkinSwapRules the old tree
-- already established for those 8, plus the 3 new ones above. See Spawner.ApplyCustomTabSkinTone.
Config.CUSTOM_TAB_SKIN_TONES = {
  "Adventurer", "African", "Albion", "Fable", "Native", "Orient", "Scum",
  "Israel", "Corrupted", "Drowned", "Ghoul",
}
...
Config.CUSTOM_CLOTHES = {
  ...
  -- "Neck" here is a descriptive label from the source spreadsheet/asset folder name, NOT one of
  -- the 18 real BodyPart enum values (BODY_PART_ENUM_BY_NAME has no "Neck" entry at all, same class
  -- of mismatch as "TorsoCloth"). Confirmed 2026-09-14 via `lbdumpbodypart` reading Feather 01's own
  -- DA_..._Neck_01_CompositeMeshData.CompositeMeshesData[1].MeshBodyPart directly (no live actor
  -- needed) -- it's REAL BodyPart 10, i.e. Sling. Feather 02 is presumably the same slot (same
  -- family/folder), not independently re-confirmed. Use Sling (lbtestpiece Sling <mesh>, or the
  -- Clothes dropdown's Sling row) to actually apply/swap this piece, not anything Neck-specific.
...
Config.CUSTOM_COMPOSITE_PIECES = {
  ...
  -- "Neck" is a descriptive label only -- confirmed 2026-09-14 via `lbdumpbodypart` (reads
  -- CompositeMeshesData[1].MeshBodyPart directly off the DataAsset, no live actor needed) that "01"
  -- is actually REAL BodyPart 10 (Sling). See the sibling note on this same family's own entry in
  -- Config.CUSTOM_BODY_MESHES above for the fuller explanation.
...
Config.STANDING_STATUES = {
  ...
  -- SailorInCage's idle/statue placement (2026-09-20, RedFalcon: "Put his basic in People >
  -- Standing and call him 'Scared Sailor'"). A QuestStatic class like the "quest folk" block above,
  -- but deliberately given a plain generic label here rather than folded into that block's own
  -- People.Named tree -- RedFalcon: "the sailor randomizes [his look/archetype at BeginPlay] so
  -- treat it like other randomized npcs," matching the plain faction-generic entries elsewhere in
  -- this same table, not the fixed-identity named quest-givers above. His REAL walking-AI version
  -- ("Sailor") lives in Config.MOBILE_QUEST_NPCS instead, its own new People.Walkers branch.
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/QuestStatic/BP_NPC_QuestStatic_SailorInCage.BP_NPC_QuestStatic_SailorInCage_C" },  -- Scared Sailor

  -- NewItems.xlsx batch (2026-09-25): generic "Citizen Static" posed NPCs, distinct assets from the
  -- faction-specific ones above despite similar poses. Appended at the end (never inserted mid-list)
  -- to keep every existing index stable for spawn_menu.ini.
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_DesperateSailor.BP_NPC_Citizen_Static_DesperateSailor_C" },  -- Citizen Sailor Leaning
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_Trader.BP_NPC_Citizen_Static_Trader_C" },  -- Citizen Merchant
}

-- Ghost Pirate's idle/statue placement (2026-09-20, RedFalcon: "Let's make a new root category
-- called Monsterous and add both idle and mobile versions. We'll be adding more to this category
-- later") -- deliberately its OWN roster/tree, not folded into STANDING_STATUES/People, so future
-- monster-family additions get their own home instead of crowding the People tree. Same
-- {faction, path} shape and spawnPosed/statueByName mechanism as STANDING_STATUES itself.
Config.MONSTEROUS_STANDING = {
  -- 2026-09-24, RedFalcon: combine Idle+Mobile under one "Monsterous > Ghost Pirate" subcategory
  -- instead of two separate ones (Standing/Walkers) -- menuPath override, same mechanism
  -- Config.MOBILE_QUEST_NPCS' own Ghost Pirate (Mobile) row already uses (see that table's row,
  -- updated to match: {"Monsterous", "Ghost Pirate"} there too).
  { faction = "Monsterous", path = "/Game/Gameplay/Character/AI/NPC/QuestStatic/BP_NPC_QuestStatic_Ghost_01_WithFXLogic.BP_NPC_QuestStatic_Ghost_01_WithFXLogic_C", label = "Ghost Pirate (Idle)", menuPath = { "Monsterous", "Ghost Pirate" } },
}

-- MONSTEROUS_MOBS (2026-09-25, NewItems.xlsx batch): real HOSTILE creature spawns, not posed idle
-- statues like MONSTEROUS_STANDING above -- these keep their own native AI/behavior (roam, attack)
-- when spawned, no faction-copy/pacify step like LIVESTOCK's spawnCreature() applies. `sub` picks
-- the People-tree-style subcategory (Monsterous > Basic/Boss/Corrupted). See
-- Testbed.SpawnMonsterousMobByName (testbed.lua) for the plain, unpacified spawn path.
-- Idle/Mobile split (2026-09-25, RedFalcon: "in monsterous, the senkamati and ghost pirate both
-- have idle and mobile versions, can we do that with the rest of the entries" -- same pattern as
-- Config.NEW_PEOPLE's Named idles/mobiles: the "(Mobile)" row is the plain pacified spawn (own
-- native AI, walking), the "(Idle)" row spawns the same class then freezes it (idlePose = true,
-- via Testbed's spawnFrozenIdleNewPerson) -- no specific pose requested for any of these, just
-- frozen wherever the AI happened to be. `_Idle` suffix on `name` keeps the two independently
-- spawnable/lookupable despite sharing one `path`.
Config.MONSTEROUS_MOBS = {
  { name = "Drowned - Armored (Mobile)", sub = "Basic", path = "/Game/Gameplay/Character/AI/Mob/Drowned/BP_Mob_Drowned_Armored.BP_Mob_Drowned_Armored_C" },
  { name = "Drowned - Crab (Mobile)", sub = "Basic", path = "/Game/Gameplay/Character/AI/Mob/Crab_Drowned/BP_Mob_Crab_Drowned.BP_Mob_Crab_Drowned_C" },
  { name = "Drowned - Naked (Mobile)", sub = "Basic", path = "/Game/Gameplay/Character/AI/Mob/Drowned/BP_Mob_Drowned_Naked.BP_Mob_Drowned_Naked_C" },
  { name = "Drowned - Spitter (Mobile)", sub = "Basic", path = "/Game/Gameplay/Character/AI/Mob/Drowned/DrownedSpitter/BP_Mob_Drowned_Spitter.BP_Mob_Drowned_Spitter_C" },
  { name = "Zombie (Mobile)", sub = "Basic", path = "/Game/Gameplay/Character/AI/Mob/Zombie/BP_Mob_Zombie.BP_Mob_Zombie_C" },
  { name = "Israel Hands (Mobile)", sub = "Boss", path = "/Game/Gameplay/Character/AI/Bosses/IsraelHands/BP_Boss_IsraelHands.BP_Boss_IsraelHands_C" },
  { name = "Swamp Toad (Mobile)", sub = "Boss", path = "/Game/Gameplay/Character/AI/Bosses/SwampToad/BP_Boss_SwampToad.BP_Boss_SwampToad_C" },
  { name = "Giant Shaker (Mobile)", sub = "Corrupted", path = "/Game/Gameplay/Character/AI/Mob/SenkamatiCorrupted/Giant_Shaker/BP_Mob_SenkamatiCorrupted_Giant_Shaker.BP_Mob_SenkamatiCorrupted_Giant_Shaker_C" },
  -- (Idle) siblings appended at the very end -- keeps the 8 (Mobile) rows above at their ORIGINAL
  -- array positions (indices 1-8 in spawn_menu.ini stay valid), no reshuffle needed.
  { name = "Drowned - Armored (Idle)", sub = "Basic", idlePose = true, path = "/Game/Gameplay/Character/AI/Mob/Drowned/BP_Mob_Drowned_Armored.BP_Mob_Drowned_Armored_C" },
  { name = "Drowned - Crab (Idle)", sub = "Basic", idlePose = true, path = "/Game/Gameplay/Character/AI/Mob/Crab_Drowned/BP_Mob_Crab_Drowned.BP_Mob_Crab_Drowned_C" },
  { name = "Drowned - Naked (Idle)", sub = "Basic", idlePose = true, path = "/Game/Gameplay/Character/AI/Mob/Drowned/BP_Mob_Drowned_Naked.BP_Mob_Drowned_Naked_C" },
  { name = "Drowned - Spitter (Idle)", sub = "Basic", idlePose = true, path = "/Game/Gameplay/Character/AI/Mob/Drowned/DrownedSpitter/BP_Mob_Drowned_Spitter.BP_Mob_Drowned_Spitter_C" },
  { name = "Zombie (Idle)", sub = "Basic", idlePose = true, path = "/Game/Gameplay/Character/AI/Mob/Zombie/BP_Mob_Zombie.BP_Mob_Zombie_C" },
  { name = "Israel Hands (Idle)", sub = "Boss", idlePose = true, path = "/Game/Gameplay/Character/AI/Bosses/IsraelHands/BP_Boss_IsraelHands.BP_Boss_IsraelHands_C" },
  { name = "Swamp Toad (Idle)", sub = "Boss", idlePose = true, path = "/Game/Gameplay/Character/AI/Bosses/SwampToad/BP_Boss_SwampToad.BP_Boss_SwampToad_C" },
  { name = "Giant Shaker (Idle)", sub = "Corrupted", idlePose = true, path = "/Game/Gameplay/Character/AI/Mob/SenkamatiCorrupted/Giant_Shaker/BP_Mob_SenkamatiCorrupted_Giant_Shaker.BP_Mob_SenkamatiCorrupted_Giant_Shaker_C" },
}
...
-- first pass because the sheet left their PAKFile column blank (an unrelated data gap in the
-- sheet, not a real "not in the game" signal -- every one of these has a real, populated class
-- path) -- RedFalcon: "i feel like a lot of people are missing from the list... almost all the
-- new merchants and named folks." Real Crew/Merchant/Recruit/Named character Blueprints, each with
-- their own native AI -- spawned the SAME pacified way as Config.MONSTEROUS_MOBS (RedFalcon's own
-- call after that roster shipped hostile by mistake), not the composite-look reskin machinery
-- Config.FACTION_VISITOR_LOOKS uses (wrong shape here -- these are real distinct classes, not one
-- shared base pawn reskinned). `sub` places each under People > Crew/Walkers/Leaning/Named/
-- Standing, exactly the subfolder the sheet itself specified. `label` is REQUIRED (used verbatim
-- as the runtime spawn label), same reasoning as Config.MOBILE_QUEST_NPCS' own comment on why it
-- doesn't rely on Spawner.FriendlyLabels here.
Config.NEW_PEOPLE = {
  { name = "BP_Mob_Blackbeard_Regular_Grenadier", sub = "Crew", label = "Crew - Grenadier Blackbeard", path = "/Game/Gameplay/Character/AI/Mob/Blackbeard/Regular_Grenadier/BP_Mob_Blackbeard_Regular_Grenadier.BP_Mob_Blackbeard_Regular_Grenadier_C" },
  { name = "BP_Mob_Blackbeard_Regular_Grenadier_ForQuest_EddieLowe", sub = "Crew", label = "Crew - Grenadier Eddie Lowe", path = "/Game/Gameplay/Character/AI/Mob/Blackbeard/Regular_Grenadier/ChildForQuests/BP_Mob_Blackbeard_Regular_Grenadier_ForQuest_EddieLowe.BP_Mob_Blackbeard_Regular_Grenadier_ForQuest_EddieLowe_C" },
  { name = "BP_Mob_BlackBeard_Regular_Musketeer", sub = "Crew", label = "Crew - Musketeer Blackbeard", path = "/Game/Gameplay/Character/AI/Mob/Blackbeard/Regular_Musketeer/BP_Mob_BlackBeard_Regular_Musketeer.BP_Mob_BlackBeard_Regular_Musketeer_C" },
  { name = "BP_Mob_BlackBeard_Regular_Musketeer_ForQuest_JimGodart", sub = "Crew", label = "Crew - Musketeer Godart", path = "/Game/Gameplay/Character/AI/Mob/Blackbeard/Regular_Musketeer/ChildForQuests/BP_Mob_BlackBeard_Regular_Musketeer_ForQuest_JimGodart.BP_Mob_BlackBeard_Regular_Musketeer_ForQuest_JimGodart_C" },
  { name = "BP_Mob_Crew_Officer_Base", sub = "Crew", label = "Crew - Officer Base", path = "/Game/Gameplay/Character/AI/Crew/Officer/BP_Mob_Crew_Officer_Base.BP_Mob_Crew_Officer_Base_C" },
  { name = "BP_Mob_Crew_Officer_Blackbeard", sub = "Crew", label = "Crew - Officer Blackbeard", path = "/Game/Gameplay/Character/AI/Crew/Officer/Faction/Blackbeard/BP_Mob_Crew_Officer_Blackbeard.BP_Mob_Crew_Officer_Blackbeard_C" },
  { name = "BP_Mob_Crew_Officer_Player", sub = "Crew", label = "Crew - Officer Player", path = "/Game/Gameplay/Character/AI/Crew/Officer/Faction/Player/BP_Mob_Crew_Officer_Player.BP_Mob_Crew_Officer_Player_C" },
  { name = "BP_Mob_Crew_Regular_Player", sub = "Crew", label = "Crew - Regular Player", path = "/Game/Gameplay/Character/AI/Crew/Regular/Faction/Player/BP_Mob_Crew_Regular_Player.BP_Mob_Crew_Regular_Player_C" },
  { name = "BP_Mob_BlackBeard_Regular_Sailor", sub = "Crew", label = "Crew - Sailor Blackbeard", path = "/Game/Gameplay/Character/AI/Mob/Blackbeard/Regular_Sailor/BP_Mob_BlackBeard_Regular_Sailor.BP_Mob_BlackBeard_Regular_Sailor_C" },
  { name = "BP_Mob_BlackBeard_Regular_Sergeant_BlackMarks", sub = "Crew", label = "Crew - Sergeant Black Marks", path = "/Game/Gameplay/Character/AI/Mob/Blackbeard/Regular_Sergeant/ChildForQuest/BP_Mob_BlackBeard_Regular_Sergeant_BlackMarks.BP_Mob_BlackBeard_Regular_Sergeant_BlackMarks_C" },
  { name = "BP_Mob_BlackBeard_Regular_Sergeant", sub = "Crew", label = "Crew - Sergeant Blackbeard", path = "/Game/Gameplay/Character/AI/Mob/Blackbeard/Regular_Sergeant/BP_Mob_BlackBeard_Regular_Sergeant.BP_Mob_BlackBeard_Regular_Sergeant_C" },
  { name = "BP_Mob_BlackBeard_Regular_Sergeant_ForQuest_LostCargo_01", sub = "Crew", label = "Crew - Sergeant Lost Cargo 1", path = "/Game/Gameplay/Character/AI/Mob/Blackbeard/Regular_Sergeant/ChildForQuest/BP_Mob_BlackBeard_Regular_Sergeant_ForQuest_LostCargo_01.BP_Mob_BlackBeard_Regular_Sergeant_ForQuest_LostCargo_01_C" },
  { name = "BP_Mob_BlackBeard_Regular_Sergeant_ForQuest_LostCargo_02", sub = "Crew", label = "Crew - Sergeant Lost Cargo 2", path = "/Game/Gameplay/Character/AI/Mob/Blackbeard/Regular_Sergeant/ChildForQuest/BP_Mob_BlackBeard_Regular_Sergeant_ForQuest_LostCargo_02.BP_Mob_BlackBeard_Regular_Sergeant_ForQuest_LostCargo_02_C" },
  { name = "BP_NPC_Employee", sub = "Walkers", label = "Employee", path = "/Game/Gameplay/Character/AI/NPC/Employee/BP_NPC_Employee.BP_NPC_Employee_C" },
  { name = "BP_NPC_Merchant_Buyer", sub = "Leaning", label = "Merchant - Buyer", path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Buyer/BP_NPC_Merchant_Buyer.BP_NPC_Merchant_Buyer_C" },
  { name = "BP_NPC_GalenSkelton_OnboardingTalker", sub = "Named", label = "Galen Skeltion (Game Start)", path = "/Game/Gameplay/Character/AI/NPC/GalenSkelton/BP_NPC_GalenSkelton_OnboardingTalker.BP_NPC_GalenSkelton_OnboardingTalker_C" },
  { name = "BP_Sc_BossArena_Highlands_Israel_RitualActor_01", sub = "Named", label = "Israel Hands (Ritual)", path = "/Game/Gameplay/Scenario/POI/BossArena/Israel/ArenaParts/BP_Sc_BossArena_Highlands_Israel_RitualActor_01.BP_Sc_BossArena_Highlands_Israel_RitualActor_01_C" },
  { name = "BP_AnimatedActor_Quests_BotC_JacquesArno", sub = "Named", label = "Jaques Arnaud", path = "/Game/Gameplay/Character/AI/NPC/FactionActors/QuestAnimatedActor/BP_AnimatedActor_Quests_BotC_JacquesArno.BP_AnimatedActor_Quests_BotC_JacquesArno_C" },
  { name = "BP_NPC_Ksant_InShackles", sub = "Named", label = "John (Shackled)", path = "/Game/Gameplay/Character/AI/NPC/Ksant/BP_NPC_Ksant_InShackles.BP_NPC_Ksant_InShackles_C" },
  { name = "BP_Mob_Crew_Officer_Marlowe", sub = "Named", label = "Marlowe (Mobile)", path = "/Game/Gameplay/Character/AI/Crew/Officer/QuestChilds/BP_Mob_Crew_Officer_Marlowe.BP_Mob_Crew_Officer_Marlowe_C" },
  { name = "BP_R5Mannequin", sub = "Named", label = "Player Character", path = "/Game/Gameplay/Character/BP_R5Mannequin.BP_R5Mannequin_C" },
  { name = "BP_Mob_Crew_Officer_SteedBonet", sub = "Named", label = "Steed Bonet (Mobile)", path = "/Game/Gameplay/Character/AI/Crew/Officer/QuestChilds/BP_Mob_Crew_Officer_SteedBonet.BP_Mob_Crew_Officer_SteedBonet_C" },
  { name = "BP_Boss_Boatswain", sub = "Named", label = "Thomas Richards (Mobile)", path = "/Game/Gameplay/Character/AI/Bosses/Boatswain/BP_Boss_Boatswain.BP_Boss_Boatswain_C" },
  { name = "BP_NPC_RecruitAlchemist", sub = "Standing", label = "Alchemist", path = "/Game/Gameplay/Character/AI/NPC/RecruitTortuga/BP_NPC_RecruitAlchemist.BP_NPC_RecruitAlchemist_C" },
  { name = "BP_NPC_RecruitCanoneer", sub = "Standing", label = "Canoneer", path = "/Game/Gameplay/Character/AI/NPC/RecruitTortuga/BP_NPC_RecruitCanoneer.BP_NPC_RecruitCanoneer_C" },
  { name = "BP_AnimatedActor_Buyer_01", sub = "Standing", label = "Merchant - Animal Products", path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Buyer/BP_AnimatedActor_Buyer_01.BP_AnimatedActor_Buyer_01_C" },
  { name = "BP_NPC_Merchant_Base", sub = "Standing", label = "Merchant - Base", path = "/Game/Gameplay/Character/AI/NPC/Merchant/BP_NPC_Merchant_Base.BP_NPC_Merchant_Base_C" },
  { name = "BP_NPC_Merchant_DeputyGovernor", sub = "Standing", label = "Merchant - Dep Gov", path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_DeputyGovernor/BP_NPC_Merchant_DeputyGovernor.BP_NPC_Merchant_DeputyGovernor_C" },
  { name = "BP_NPC_Merchant_Employees", sub = "Standing", label = "Merchant - Employee", path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Employees/BP_NPC_Merchant_Employees.BP_NPC_Merchant_Employees_C" },
  { name = "BP_AnimatedActor_Buyer_02", sub = "Standing", label = "Merchant - Food (F)", path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Buyer/BP_AnimatedActor_Buyer_02.BP_AnimatedActor_Buyer_02_C" },
  -- mobileOverride (2026-09-25, RedFalcon: "can we wire her like we did the quest npcs" -- a live
  -- probe of BP_NPC_Merchant_Generalist_C showed native AIControllerClass=BP_NPC_AIController_
  -- QuestStatic_C + a live-possessed controller + a real PathFollowingComponent + MaxWalkSpeed=600,
  -- but she stands in a fixed pose both in the wild and spawned -- the AI plumbing is there for
  -- something other than locomotion. The QuestStatic AIController/AnimBP FORCE override
  -- (Config.MOBILE_QUEST_NPC_AI_CONTROLLER/ANIM_CLASS, same mechanism placeMobileQuestNPC uses for
  -- Marita/Letty/etc) CONFIRMED WORKING live 2026-09-25 -- she walks. Kept as its own separate
  -- "(Mobile)" row under Walkers (below), same idle/mobile split as Config.MONSTEROUS_STANDING /
  -- MOBILE_QUEST_NPCS -- this row stays the plain default Standing spawn.
  { name = "BP_NPC_Merchant_Generalist", sub = "Standing", label = "Merchant - General (F)", path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Generalist/BP_NPC_Merchant_Generalist.BP_NPC_Merchant_Generalist_C" },
  { name = "BP_NPC_Merchant_Herbalist", sub = "Standing", label = "Merchant - Herbalist", path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Herbalist/BP_NPC_Merchant_Herbalist.BP_NPC_Merchant_Herbalist_C" },
  { name = "BP_AnimatedActor_Buyer_03", sub = "Standing", label = "Merchant - Resources", path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Buyer/BP_AnimatedActor_Buyer_03.BP_AnimatedActor_Buyer_03_C" },
  { name = "BP_NPC_Merchant_Ship", sub = "Standing", label = "Merchant - Ship", path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Ship/BP_NPC_Merchant_Ship.BP_NPC_Merchant_Ship_C" },
  { name = "BP_NPC_RecruitWoodcutter", sub = "Standing", label = "Woodcutter", path = "/Game/Gameplay/Character/AI/NPC/RecruitTortuga/BP_NPC_RecruitWoodcutter.BP_NPC_RecruitWoodcutter_C" },
  -- (Mobile) variant of Merchant - General (F) above -- same class, forced onto the QuestStatic
  -- AIController/AnimBP override (CONFIRMED WORKING 2026-09-25). Separate row/name (not a flag
  -- toggle on the Standing one) so both are independently spawnable, same pattern as
  -- Config.MOBILE_QUEST_NPCS' own "(Mobile)" siblings.
  { name = "BP_NPC_Merchant_Generalist_Mobile", sub = "Walkers", label = "Merchant - General (F) (Mobile)", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Generalist/BP_NPC_Merchant_Generalist.BP_NPC_Merchant_Generalist_C" },

  -- "New Walkers.xlsx" batch (2026-09-25): 16 more entries RedFalcon confirmed walk via lbtestaianim
  -- (the 6 "Citizen Static" rows are the SAME classes as the People > Activities/Chair Sitting/
  -- Leaning/Squatting statue entries added earlier this session -- those turned out to walk too).
  -- Kept in the SAME subfolder as their idle counterpart (not moved to Walkers) per this sheet's own
  -- Category column -- a deliberate change from the single Merchant Generalist precedent above.
  -- `_Mobile`-suffixed names avoid colliding with an existing Standing/Leaning entry for the same
  -- underlying class (e.g. BP_NPC_RecruitAlchemist already exists as the plain "Alchemist" row).
  { name = "BP_NPC_Citizen_Static_Looker_Chest", sub = "Walkers", label = "Citizen Moonwalker", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_Looker_Chest.BP_NPC_Citizen_Static_Looker_Chest_C" },
  { name = "BP_NPC_Citizen_Static_FireWarm", sub = "Walkers", label = "Citizen 1", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_FireWarm.BP_NPC_Citizen_Static_FireWarm_C" },
  { name = "BP_NPC_Citizen_Static_DrunkCannoneer", sub = "Walkers", label = "Canoneer 1", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_DrunkCannoneer.BP_NPC_Citizen_Static_DrunkCannoneer_C" },
  { name = "BP_NPC_Citizen_Static_SitterOnStool", sub = "Walkers", label = "Citizen 2", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_SitterOnStool.BP_NPC_Citizen_Static_SitterOnStool_C" },
  { name = "BP_NPC_Citizen_Static_DesperateSailor", sub = "Walkers", label = "Citizen Sailor", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_DesperateSailor.BP_NPC_Citizen_Static_DesperateSailor_C" },
  { name = "BP_NPC_Merchant_Buyer_Mobile", sub = "Walkers", label = "Merchant - Buyer (Mobile)", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Buyer/BP_NPC_Merchant_Buyer.BP_NPC_Merchant_Buyer_C" },
  { name = "BP_NPC_Citizen_Static_TiredSoldier", sub = "Walkers", label = "Citizen 3", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_TiredSoldier.BP_NPC_Citizen_Static_TiredSoldier_C" },
  { name = "BP_NPC_RecruitAlchemist_Mobile", sub = "Walkers", label = "Alchemist (Mobile)", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/RecruitTortuga/BP_NPC_RecruitAlchemist.BP_NPC_RecruitAlchemist_C" },
  { name = "BP_NPC_RecruitCanoneer_Mobile", sub = "Walkers", label = "Canoneer 2", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/RecruitTortuga/BP_NPC_RecruitCanoneer.BP_NPC_RecruitCanoneer_C" },
  { name = "BP_NPC_Citizen_Static_Trader", sub = "Walkers", label = "Citizen Merchant (Mobile)", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_Trader.BP_NPC_Citizen_Static_Trader_C" },
  { name = "BP_NPC_Merchant_Base_Mobile", sub = "Walkers", label = "Merchant - Base (Mobile)", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Merchant/BP_NPC_Merchant_Base.BP_NPC_Merchant_Base_C" },
  { name = "BP_NPC_Merchant_DeputyGovernor_Mobile", sub = "Walkers", label = "Merchant - Dep Gov (Mobile)", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_DeputyGovernor/BP_NPC_Merchant_DeputyGovernor.BP_NPC_Merchant_DeputyGovernor_C" },
  { name = "BP_NPC_Merchant_Employees_Mobile", sub = "Walkers", label = "Merchant - Employee (Mobile)", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Employees/BP_NPC_Merchant_Employees.BP_NPC_Merchant_Employees_C" },
  { name = "BP_NPC_Merchant_Herbalist_Mobile", sub = "Walkers", label = "Merchant - Herbalist (Mobile)", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Herbalist/BP_NPC_Merchant_Herbalist.BP_NPC_Merchant_Herbalist_C" },
  { name = "BP_NPC_Merchant_Ship_Mobile", sub = "Walkers", label = "Merchant - Ship (Mobile)", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Ship/BP_NPC_Merchant_Ship.BP_NPC_Merchant_Ship_C" },
  { name = "BP_NPC_RecruitWoodcutter_Mobile", sub = "Walkers", label = "Woodcutter (Mobile)", mobileOverride = true, path = "/Game/Gameplay/Character/AI/NPC/RecruitTortuga/BP_NPC_RecruitWoodcutter.BP_NPC_RecruitWoodcutter_C" },

  -- Named-walker IDLE counterparts (2026-09-25, RedFalcon: "for the named walkers I'd like to add
  -- some idles for them"). `idlePose` (true, or a Config.CUSTOM_POSES-style animation asset path)
  -- freezes AI right after spawn (Spawner.SetAILogic(false), same mechanism as lbfreeze/
  -- lbspawnnoai) and, when a path is given, locks that specific pose via Spawner.ApplyFrozenPose --
  -- `true` alone (Thomas Richards) just freezes whatever stance the AI was in, no pose forced.
  { name = "BP_Mob_Crew_Officer_Marlowe_Idle", sub = "Named", label = "Marlowe (Idle)", idlePose = "/Game/Character/Animations/Human/Regular/Hero/Saber/A_Regular_Male_Hero_Saber_Armed_Idle", path = "/Game/Gameplay/Character/AI/Crew/Officer/QuestChilds/BP_Mob_Crew_Officer_Marlowe.BP_Mob_Crew_Officer_Marlowe_C" },
  { name = "BP_Mob_Crew_Officer_SteedBonet_Idle", sub = "Named", label = "Steed Bonet (Idle)", idlePose = "/Game/Character/Animations/Human/Regular/Shared/Vendors/A_Regular_Male_Vendor01", path = "/Game/Gameplay/Character/AI/Crew/Officer/QuestChilds/BP_Mob_Crew_Officer_SteedBonet.BP_Mob_Crew_Officer_SteedBonet_C" },
  { name = "BP_Boss_Boatswain_Idle", sub = "Named", label = "Thomas Richards (Idle)", idlePose = true, path = "/Game/Gameplay/Character/AI/Bosses/Boatswain/BP_Boss_Boatswain.BP_Boss_Boatswain_C" },
}

-- "Mobile" named-NPC spawns (2026-09-20) -- real walking AI + real walk animation on the NPC's own
-- ACTUAL class (no reskin/donor-body approximation), via the same aiControllerClassPath +
-- animClassPath mechanism `lbtestaianim` proved live -- see WINDROSE_MODDING_NOTES.md 19at/19au.
-- CONFIRMED WORKING on this whole family (all Pawn-derived BP_NPC_QuestStatic_* classes); do NOT
-- add a BP_AnimatedActor_* class here -- 19au proved that whole family structurally can't work
-- (not Pawn-derived, no Controller/movement machinery at all).
--
-- `label` is REQUIRED and used verbatim as the runtime spawn label (toast/target-name/persist) --
-- deliberately NOT derived from Spawner.FriendlyLabels[shortClassName] the way statue rosters do,
-- since several of these rows share a class with an EXISTING STANDING_STATUES entry (e.g. Marita's
-- idle vs. mobile spawn are the same class) -- that map is keyed purely by class name with no
-- per-roster dimension, so two rosters sharing one class would silently clobber each other's label
-- depending on Lua's unspecified `pairs()` iteration order. An explicit label per row sidesteps
-- that entirely.
--
-- `menuPath` (optional): overrides the default People.Named tree placement for an entry that needs
-- its own home -- Sailor (People.Walkers, RedFalcon's own explicit ask, distinct from his "Scared
-- Sailor" idle counterpart in plain STANDING_STATUES/People.Standing) and Ghost Pirate
-- (Monsterous.Walkers, sibling to his own MONSTEROUS_STANDING idle entry).
Config.MOBILE_QUEST_NPCS = {
  { faction = "Brethren of the Coast", path = "/Game/Gameplay/Character/AI/NPC/QuestStatic/BP_NPC_QuestStatic_BotC_FrancoisArno.BP_NPC_QuestStatic_BotC_FrancoisArno_C", label = "Francois Arnaud (Mobile)" },
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/QuestStatic/BP_NPC_QuestStatic_Letty.BP_NPC_QuestStatic_Letty_C", label = "Letty (Mobile)" },
  { faction = "Brethren of the Coast", path = "/Game/Gameplay/Character/AI/NPC/QuestStatic/BP_NPC_QuestStatic_BotC_BenjaminHornigold.BP_NPC_QuestStatic_BotC_BenjaminHornigold_C", label = "Benjamin Hornigold (Mobile)" },
  { faction = "Buccaneers", path = "/Game/Gameplay/Character/AI/NPC/QuestStatic/BP_NPC_QuestStatic_Buccaneers_HenriBoucher.BP_NPC_QuestStatic_Buccaneers_HenriBoucher_C", label = "Henri Boucher (Mobile)" },
  { faction = "Smugglers", path = "/Game/Gameplay/Character/AI/NPC/QuestStatic/BP_NPC_QuestStatic_Smugglers_MaritaSuares.BP_NPC_QuestStatic_Smugglers_MaritaSuares_C", label = "Marita Suares (Mobile)" },
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/QuestStatic/BP_NPC_QuestStatic_TortugaCitizen_LongBen.BP_NPC_QuestStatic_TortugaCitizen_LongBen_C", label = "Long Ben (Mobile)" },
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/QuestStatic/BP_NPC_QuestStatic_TortugaCitizen_CharlieSharp.BP_NPC_QuestStatic_TortugaCitizen_CharlieSharp_C", label = "Charlie Sharp (Mobile)" },
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/QuestStatic/BP_NPC_QuestStatic_SailorInCage.BP_NPC_QuestStatic_SailorInCage_C", label = "Sailor", menuPath = { "People", "Walkers" } },
  { faction = "Monsterous", path = "/Game/Gameplay/Character/AI/NPC/QuestStatic/BP_NPC_QuestStatic_Ghost_01_WithFXLogic.BP_NPC_QuestStatic_Ghost_01_WithFXLogic_C", label = "Ghost Pirate (Mobile)", menuPath = { "Monsterous", "Ghost Pirate" } },
}

-- The two class paths every Mobile Quest NPC spawn shares -- confirmed live end to end (Marita,
-- WINDROSE_MODDING_NOTES.md 19at): a generic ambient-wander brain (no work-zone/blackboard data
-- required, unlike Handyman) and the shared "Share_HumanAI"-style walking AnimBP every real human
-- NPC uses. Both are ALREADY loaded classes by the time any of this fires (real NPCs use them
-- constantly), so resolveClass never needs a fresh-CDO-load path for either.
Config.MOBILE_QUEST_NPC_AI_CONTROLLER =
...
Config.SEATED_STATUES = {
  ...
  -- NewItems.xlsx batch (2026-09-25): generic "Citizen Static" squat pose, appended at the end.
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_TiredSoldier.BP_NPC_Citizen_Static_TiredSoldier_C" },  -- Citizen Squat
}
...
Config.CHAIR_STATUES = {
  ...
  -- NewItems.xlsx batch (2026-09-25): generic "Citizen Static" chair poses, appended at the end.
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_DrunkCannoneer.BP_NPC_Citizen_Static_DrunkCannoneer_C" },  -- Citizen Canoneer Sitting
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_SitterOnStool.BP_NPC_Citizen_Static_SitterOnStool_C" },  -- Citizen Sitting
}
...
Config.INTERACTIVE_STATUES = {
  ...
  -- NewItems.xlsx batch (2026-09-25): generic "Citizen Static" activity poses, appended at the end.
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_Looker_Chest.BP_NPC_Citizen_Static_Looker_Chest_C" },  -- Citizen Chest
  { faction = "People of Tortuga", path = "/Game/Gameplay/Character/AI/NPC/Citizen/StaticAnimated/BP_NPC_Citizen_Static_FireWarm.BP_NPC_Citizen_Static_FireWarm_C" },  -- Citizen Fire
}
...
Config.CUSTOM_POSES = {
  ...
  -- 2026-09-21, RedFalcon's "additional poses" tab (Other\SocketItems.xlsx) -- Path column had a
  -- stray trailing " ik_weapon_rSocket,ik_weapon_lSocket" test-command-argument fragment on the 8
  -- Consumables/Potion rows (copy/paste leftover, not part of the real asset path) -- stripped
  -- here, real path only. Friendly name column encoded the FULL menu path with " > " separators
  -- (e.g. "Standing > Food and Meds > Drink > Drink Holding Axe") -- the trailing segment is
  -- `name`, everything between `topCategory` and it is `subCategory`; the 8 Potion rows needed
  -- TWO subcategory levels ("Food and Meds" > "Drink"), the first time this table has needed more
  -- than one, so subCategory can now be a plain string (existing rows, unchanged) OR a list of
  -- strings for deeper nesting -- see spawnmenu_manifest.lua's custom_poses_path_and_label for the
  -- (now list-aware) code that turns this into the actual spawn_menu.ini category path.
  { path = "/Game/Character/Animations/Human/Regular/Hero/Fishing_Rod/A_Regular_Male_Hero_Fishing_Armed_Idle", topCategory = "Standing", subCategory = "Fishing", name = "Armed" },
  { path = "/Game/Character/Animations/Human/Regular/Hero/Fishing_Rod/AM_Regular_Male_Hero_Fishing_Fight", topCategory = "Standing", subCategory = "Fishing", name = "Reel In" },
  { path = "/Game/Character/Animations/Human/Regular/Hero/Fishing_Rod/AM_Regular_Male_Hero_Fishing_IdleFishing", topCategory = "Standing", subCategory = "Fishing", name = "Idle" },
  { path = "/Game/Character/Animations/Human/Regular/Hero/Consumables/Potion/AM_Regular_Male_Hero_Axe_Armed_Potion", topCategory = "Standing", subCategory = { "Food and Meds", "Drink" }, name = "Drink Holding Axe" },
  { path = "/Game/Character/Animations/Human/Regular/Hero/Consumables/Potion/AM_Regular_Male_Hero_Fists_Armed_Potion", topCategory = "Standing", subCategory = { "Food and Meds", "Drink" }, name = "Drink With Fists" },
  { path = "/Game/Character/Animations/Human/Regular/Hero/Consumables/Potion/AM_Regular_Male_Hero_Halberd_Armed_Potion", topCategory = "Standing", subCategory = { "Food and Meds", "Drink" }, name = "Drink Holding Halberd" },
  { path = "/Game/Character/Animations/Human/Regular/Hero/Consumables/Potion/AM_Regular_Male_Hero_Machete_Armed_Potion", topCategory = "Standing", subCategory = { "Food and Meds", "Drink" }, name = "Drink Holding Machete" },
  { path = "/Game/Character/Animations/Human/Regular/Hero/Consumables/Potion/AM_Regular_Male_Hero_Pickaxe_Armed_Potion", topCategory = "Standing", subCategory = { "Food and Meds", "Drink" }, name = "Drink Holding Pickaxe" },
  { path = "/Game/Character/Animations/Human/Regular/Hero/Consumables/Potion/AM_Regular_Male_Hero_Recall_Potion", topCategory = "Standing", subCategory = { "Food and Meds", "Drink" }, name = "Drink Recall Potion" },
  { path = "/Game/Character/Animations/Human/Regular/Hero/Consumables/Potion/AM_Regular_Male_Hero_Saber_Armed_Potion", topCategory = "Standing", subCategory = { "Food and Meds", "Drink" }, name = "Drink Holding Saber" },
  { path = "/Game/Character/Animations/Human/Regular/Hero/Consumables/Potion/AM_Regular_Male_Hero_Shovel_Armed_Potion", topCategory = "Standing", subCategory = { "Food and Meds", "Drink" }, name = "Drink Holding Shovel" },
  { path = "/Game/Character/Animations/Human/Regular/Hero/Consumables/Potion/AM_Regular_Male_Hero_Unarmed_Potion", topCategory = "Standing", subCategory = { "Food and Meds", "Drink" }, name = "Drink Unarmed" },
}
...
------------------------------------------------------------------
Config.BELTSTRAPS_PIECES = {
  { type="Belt", friendlyName="Belt 1", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Belt_01_Male.SK_Belt_01_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Belt_01_Female.SK_Belt_01_Female" },
  { type="Belt", friendlyName="Belt 2", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Belt_02_Male.SK_Belt_02_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Belt_02_Female.SK_Belt_02_Female" },
  { type="Belt", friendlyName="Belt 3", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Belt_03_Male.SK_Belt_03_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Belt_03_Female.SK_Belt_03_Female" },
  { type="Belt", friendlyName="Belt 4", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Belt_04_Male.SK_Belt_04_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Belt_04_Female.SK_Belt_04_Female" },
  { type="Frog", friendlyName="Frog 1", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Frog_01_Male.SK_Frog_01_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Frog_01_Female.SK_Frog_01_Female" },
  { type="Frog", friendlyName="Frog 3", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Frog_03_Male.SK_Frog_03_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Frog_03_Female.SK_Frog_03_Female" },
  { type="Frog", friendlyName="Frog 4", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Frog_04_Male.SK_Frog_04_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Frog_04_Female.SK_Frog_04_Female" },
  { type="Sling", friendlyName="Sling 1", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Sling_01_Male.SK_Sling_01_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Sling_01_Female.SK_Sling_01_Female" },
  { type="Sling", friendlyName="Sling 3", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Sling_03_Male.SK_Sling_03_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Sling_03_Female.SK_Sling_03_Female" },
  { type="Sling", friendlyName="Sling 4", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Sling_04_Male.SK_Sling_04_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Sling_04_Female.SK_Sling_04_Female" },
  { type="Sling", friendlyName="Shaman Necklace", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_01_Neck.SK_ArmorCreature_Senkamati_Witch_Feather_01_Neck" },
  { type="Strap", friendlyName="Strap 1", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Strap_01_Male.SK_Strap_01_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Strap_01_Female.SK_Strap_01_Female" },
  { type="Strap", friendlyName="Strap 3", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Strap_03_Male.SK_Strap_03_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Strap_03_Female.SK_Strap_03_Female" },
  { type="Strap", friendlyName="Strap 4", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Strap_04_Male.SK_Strap_04_Male", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Meshes/SK_Strap_04_Female.SK_Strap_04_Female" },
  { type="Set", friendlyName="Set 1", maleMesh=nil, femaleMesh=nil },
  { type="Set", friendlyName="Set 3", maleMesh=nil, femaleMesh=nil },
  { type="Set", friendlyName="Set 4", maleMesh=nil, femaleMesh=nil },
}

Config.SOCKETITEMS_SOCKETS = {
  { socket="Axe1h_backsocket", location="Use Specific Weapon, Do not use with other Back Sockets", socType="weapon", beltpiece={ "sling" }, locationTag="Back", friendlyName="Back" },
  { socket="Axe2h_backsocket", location="Use Specific Weapon, Do not use with other Back Sockets", socType="weapon", beltpiece={ "sling" }, locationTag="Back", friendlyName="Back" },
  { socket="Crossbow2h_backsocket", location="Use Specific Weapon, Do not use with other Back Sockets", socType="weapon", beltpiece={ "sling" }, locationTag="Back", friendlyName="Back" },
  { socket="GSword_backsocket", location="Use Specific Weapon, Do not use with other Back Sockets", socType="weapon", beltpiece={ "sling" }, locationTag="Back", friendlyName="Back" },
  { socket="Halberd_backsocket", location="Use Specific Weapon, Do not use with other Back Sockets", socType="weapon", beltpiece={ "sling" }, locationTag="Back", friendlyName="Back" },
  { socket="Musket_backsocket", location="Use Specific Weapon, Do not use with other Back Sockets", socType="weapon", beltpiece={ "sling" }, locationTag="Back", friendlyName="Back Weapon" },
  { socket="soc_Strap01F", location="Bottom Right Front on Strap at Belt Level", socType="soc", beltpiece={ "belt" }, locationTag="Front", friendlyName="Belt 1" },
  { socket="soc_beltB02_l", location="Belt, Front Left", socType="soc", beltpiece={ "belt" }, locationTag="Front", friendlyName="Belt 2" },
  { socket="soc_beltSlingF", location="Bottom Left Front on Sling at Belt Level", socType="soc", beltpiece={ "belt" }, locationTag="Front", friendlyName="Belt 3" },
  { socket="soc_beltSlingB", location="Bottom Left Back on Sling at Belt Level", socType="soc", beltpiece={ "belt" }, locationTag="Back", friendlyName="Belt 4" },
  { socket="soc_belt02B_l", location="Belt, Back Left", socType="soc", beltpiece={ "belt" }, locationTag="Back", friendlyName="Belt 5" },
  { socket="soc_beltB", location="Belt, Back Middle", socType="soc", beltpiece={ "belt" }, locationTag="Back", friendlyName="Belt 6" },
  { socket="soc_beltStrapB", location="Bottom Right Back on Sling at Belt Level", socType="soc", beltpiece={ "belt" }, locationTag="Back", friendlyName="Belt 7" },
  { socket="soc_Lantern", location="Belt, Front Right by Belt Buckle", socType="soc", beltpiece={ "belt" }, locationTag="Front", friendlyName="Lantern Slot" },
  { socket="beltSlot_01_lSocket", location="Left Belt Hip Holster Socket", socType="weapon", beltpiece={ "belt" }, locationTag="Hip", friendlyName="Left Pistol" },
  { socket="beltSlot_01_rSocket", location="Right Belt Hip Holster Socket", socType="weapon", beltpiece={ "belt" }, locationTag="Hip", friendlyName="Right Pistol" },
  { socket="rapierSlot_lSocket", location="Left Hip in Sheath, do not use with other lSockets", socType="weapon", beltpiece={ "belt" }, locationTag="Sheath", friendlyName="Sheath" },
  { socket="swordSlot_lSocket", location="Left Hip in Sheath, do not use with other lSockets", socType="weapon", beltpiece={ "belt" }, locationTag="Sheath", friendlyName="Sheath" },
  { socket="soc_Sling04F", location="Top Right Front on Sling By Shoulder", socType="soc", beltpiece={ "sling" }, locationTag="Front", friendlyName="Sling 1" },
  { socket="soc_Sling03F", location="Middle Right Front on Sling Above Cross", socType="soc", beltpiece={ "sling" }, locationTag="Front", friendlyName="Sling 2" },
  { socket="soc_Sling02F", location="Middle Left Front on Sling Below Cross", socType="soc", beltpiece={ "sling" }, locationTag="Front", friendlyName="Sling 3" },
  { socket="soc_Sling01F", location="Bottom Left Front on Sling", socType="soc", beltpiece={ "sling" }, locationTag="Front", friendlyName="Sling 4" },
  { socket="soc_Sling04B", location="Top Right Back on Sling By Shoulder", socType="soc", beltpiece={ "sling" }, locationTag="Back", friendlyName="Sling 5" },
  { socket="soc_Sling03B", location="Middle Right Back on Sling Above Cross", socType="soc", beltpiece={ "sling" }, locationTag="Back", friendlyName="Sling 6" },
  { socket="soc_Sling02B", location="Middle Left Back on Sling On Cross", socType="soc", beltpiece={ "sling" }, locationTag="Back", friendlyName="Sling 7" },
  { socket="soc_Sling01B", location="Bottom Left Back on Sling", socType="soc", beltpiece={ "sling" }, locationTag="Back", friendlyName="Sling 8" },
  { socket="soc_Strap04F", location="Top Left Front on Strap By Shoulder", socType="soc", beltpiece={ "strap" }, locationTag="Front", friendlyName="Strap 1" },
  { socket="soc_Strap03F", location="Middle Left Front on Strap Above Cross", socType="soc", beltpiece={ "strap" }, locationTag="Front", friendlyName="Strap 2" },
  { socket="soc_Strap02F", location="Middle Right Front on Strap Below Cross", socType="soc", beltpiece={ "strap" }, locationTag="Front", friendlyName="Strap 3" },
  { socket="soc_Strap01F", location="Bottom Right Front on Strap at Belt Level", socType="soc", beltpiece={ "strap" }, locationTag="Front", friendlyName="Strap 4" },
  { socket="soc_Strap04B", location="Top Left Back on Strap By Shoulder", socType="soc", beltpiece={ "strap" }, locationTag="Back", friendlyName="Strap 5" },
  { socket="soc_Strap03B", location="Middle Left Back on Strap Above Cross", socType="soc", beltpiece={ "strap" }, locationTag="Back", friendlyName="Strap 6" },
  { socket="soc_Strap02B", location="Middle Right Back on Strap On Cross", socType="soc", beltpiece={ "strap" }, locationTag="Back", friendlyName="Strap 7" },
  { socket="soc_Strap01B", location="Bottom Right Back on Strap", socType="soc", beltpiece={ "strap" }, locationTag="Back", friendlyName="Strap 8" },
  { socket="soc_Strap_r", location="Bottom Right on Strap at Hip", socType="soc", beltpiece={ "strap" }, locationTag="Side", friendlyName="Strap 9" },
}
...
Config.SOCKETITEMS_ITEMS = {
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_BeltBag_02_FR2", shortName="SM_Belt_Misc_BeltBag_02_FR2", friendlyName="Bag - Belt", sockets={ "soc_Strap01B", "soc_Strap01F", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B", "soc_Strap03F", "soc_Strap04B", "soc_Strap04F", "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Sling02B", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Sling04B", "soc_Sling04F" }, limit=4, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_Wallet_01", shortName="SM_Belt_Misc_Wallet_01", friendlyName="Bag - Fancy Coinpurse", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap01F" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_BagDrGalen_01", shortName="SM_Belt_Misc_BagDrGalen_01", friendlyName="Bag - Galen's", sockets={ "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap_r", "soc_Strap01F" }, limit=2, tags={}, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_BagKsante_02", shortName="SM_Belt_Misc_BagKsante_02", friendlyName="Bag - John's", sockets={ "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap_r", "soc_Strap01F" }, limit=2, tags={}, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_BagKsante_01", shortName="SM_Belt_Misc_BagKsante_01", friendlyName="Bag - John's  With Scroll", sockets={ "soc_beltB", "soc_Strap_r" }, limit=1, tags={}, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_BeltBag_02_Large_Strap", shortName="SM_Belt_Misc_BeltBag_02_Large_Strap", friendlyName="Bag - Large Belt", sockets={ "soc_beltB", "soc_Strap_r" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_BeltBag_02_BonesBomb_Strap", shortName="SM_Belt_Misc_BeltBag_02_BonesBomb_Strap", friendlyName="Bag - Large Explosives", sockets={ "soc_beltB", "soc_Strap_r" }, limit=1, tags={ "Hunting", "Soldiering" }, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_BeltBag_02_Strap", shortName="SM_Belt_Misc_BeltBag_02_Strap", friendlyName="Bag - Medium Bag 2", sockets={ "soc_belt02B_l", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Strap01F", "soc_Strap02F", "soc_Strap04B", "soc_Strap01B" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_BeltBag_02_Simple", shortName="SM_Belt_Misc_BeltBag_02_Simple", friendlyName="Bag - Medium Belt 1", sockets={ "soc_beltB", "soc_Strap_r" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_BeltBag_01_Simple", shortName="SM_Belt_Misc_BeltBag_01_Simple", friendlyName="Bag - Simple Belt ", sockets={ "soc_beltB", "soc_Strap_r" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_BeltBag_01_SimpleSmall", shortName="SM_Belt_Misc_BeltBag_01_SimpleSmall", friendlyName="Bag - Small Belt", sockets={ "soc_Sling01B", "soc_Sling01F", "soc_Sling02B", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Sling04B", "soc_Sling04F", "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap01B", "soc_Strap01F", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B", "soc_Strap03F", "soc_Strap04B", "soc_Strap04F" }, limit=4, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_BeltBag_02_Powder_FR2", shortName="SM_Belt_Misc_BeltBag_02_Powder_FR2", friendlyName="Bag - With Powder", sockets={ "soc_belt02B_l", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Strap01F", "soc_Strap02F", "soc_Strap04B", "soc_Strap01B" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bags/SM_Belt_Misc_Bag_01", shortName="SM_Belt_Misc_Bag_01", friendlyName="Bag - Worn", sockets={ "soc_beltB", "soc_Strap_r" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bones/SM_Belt_Misc_BoneSmall_01", shortName="SM_Belt_Misc_BoneSmall_01", friendlyName="Bone 1", sockets={ "soc_Sling01B", "soc_Sling01F", "soc_Sling02B", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Sling04B", "soc_Sling04F", "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap01B", "soc_Strap01F", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B", "soc_Strap03F", "soc_Strap04B", "soc_Strap04F" }, limit=1, tags={ "Creepy" }, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bones/SM_Belt_Misc_BoneSmall_02", shortName="SM_Belt_Misc_BoneSmall_02", friendlyName="Bone 2", sockets={ "soc_Sling01B", "soc_Sling01F", "soc_Sling02B", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Sling04B", "soc_Sling04F", "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap01B", "soc_Strap01F", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B", "soc_Strap03F", "soc_Strap04B", "soc_Strap04F" }, limit=2, tags={ "Creepy" }, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bones/SM_Belt_Misc_BonesBelt_01_FR", shortName="SM_Belt_Misc_BonesBelt_01_FR", friendlyName="Bones", sockets={ "soc_Sling01B", "soc_Sling01F", "soc_Sling02B", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Sling04B", "soc_Sling04F", "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap01B", "soc_Strap01F", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B", "soc_Strap03F", "soc_Strap04B", "soc_Strap04F" }, limit=2, tags={ "Creepy" }, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bones/SM_Belt_Misc_RopedBones_01_S_FR", shortName="SM_Belt_Misc_RopedBones_01_S_FR", friendlyName="Bones - Hanging", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB" }, limit=2, tags={ "Creepy" }, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bones/SM_Belt_Misc_BoneNecklace_01", shortName="SM_Belt_Misc_BoneNecklace_01", friendlyName="Bones - Necklace", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Sling03F", "soc_Sling04F", "soc_Strap04F", "soc_Strap03F" }, limit=1, tags={ "Creepy" }, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Book_01", shortName="SM_Belt_Misc_Book_01", friendlyName="Book", sockets={ "soc_beltStrapB", "soc_beltSlingB", "soc_beltB02_l" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Bottle_01", shortName="SM_Belt_Misc_Bottle_01", friendlyName="Canteen", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Feathers_01", shortName="SM_Belt_Misc_Feathers_01", friendlyName="Feather", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB" }, limit=1, tags={ "Hunting" }, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Feathers_02", shortName="SM_Belt_Misc_Feathers_02", friendlyName="Feathers", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB" }, limit=1, tags={ "Hunting" }, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Fishhook_01", shortName="SM_Belt_Misc_Fishhook_01", friendlyName="Fish Hook", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Strap02F", "soc_Strap03F" }, limit=2, tags={ "Fishing" }, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_FishingRod/Fishing_Copper/SK_Gear_Fishing_Bobber_Copper", shortName="SK_Gear_Fishing_Bobber_Copper", friendlyName="Fishing Bobber", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap01F" }, limit=2, tags={ "Fishing" }, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Gloves_01_FL", shortName="SM_Belt_Misc_Gloves_01_FL", friendlyName="Gloves", sockets={ "soc_beltStrapB", "soc_beltB02_l", "soc_Sling01B", "soc_Sling01F", "soc_Sling02B", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Sling04F", "soc_Strap01B", "soc_Strap01F", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B", "soc_Strap03F", "soc_Strap04F" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Grenade_01", shortName="SM_Belt_Misc_Grenade_01", friendlyName="Grenade", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Sling02B", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Sling04B", "soc_Sling04F", "soc_Strap01B", "soc_Strap01F", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B", "soc_Strap03F", "soc_Strap04B", "soc_Strap04F" }, limit=6, tags={ "Soldiering" }, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Fan", shortName="SM_Belt_Misc_Fan", friendlyName="Hand Fan", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB" }, limit=1, tags={}, rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Knifes/SM_Belt_Misc_Knife_04", shortName="SM_Belt_Misc_Knife_04", friendlyName="Knife - Fancy", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Strap01B", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Knifes/SM_Belt_Misc_Knife_01_Simple", shortName="SM_Belt_Misc_Knife_01_Simple", friendlyName="Knife - No Sheath", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Strap01B", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Knifes/SM_Belt_Misc_Knife_03", shortName="SM_Belt_Misc_Knife_03", friendlyName="Knife - Primitive Iron", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Strap01B", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Knifes/SM_Belt_Misc_Knife_02", shortName="SM_Belt_Misc_Knife_02", friendlyName="Knife - Primitive Stone", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Strap01B", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Knifes/SM_Belt_Misc_Knife_01", shortName="SM_Belt_Misc_Knife_01", friendlyName="Knife - Sheath", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Sling04F", "soc_Strap01B", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Smokepipe_01", shortName="SM_Belt_Misc_Smokepipe_01", friendlyName="Pipe", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB" }, limit=1, tags={}, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Pouch_02", shortName="SM_Belt_Misc_Pouch_02", friendlyName="Pouch - Dark", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap01F" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Pouch_01", shortName="SM_Belt_Misc_Pouch_01", friendlyName="Pouch - Light", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap01F" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Powder_01", shortName="SM_Belt_Misc_Powder_01", friendlyName="Powder Horn", sockets={ "soc_Sling01B", "soc_Sling02B", "soc_Sling02F", "soc_Sling03B", "soc_Sling04B", "soc_Strap01B", "soc_Strap01F", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B", "soc_Strap03F", "soc_Strap04B", "soc_beltB", "soc_beltSlingF", "soc_beltStrapB", "soc_Lantern" }, limit=1, tags={}, rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bones/SM_Belt_Misc_SkullDodo_01", shortName="SM_Belt_Misc_SkullDodo_01", friendlyName="Skull - Dodo", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingF", "soc_beltStrapB" }, limit=1, tags={ "Hunting" }, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bones/SM_Belt_Misc_SkullMale_01", shortName="SM_Belt_Misc_SkullMale_01", friendlyName="Skull - Human", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Strap04B", "soc_Strap01B" }, limit=1, tags={ "Creepy" }, rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bones/SM_Belt_Misc_SkullWolf_01", shortName="SM_Belt_Misc_SkullWolf_01", friendlyName="Skull - Wolf 1", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingF", "soc_beltStrapB" }, limit=1, tags={ "Hunting" }, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bones/SM_Belt_Misc_SkullWolf_01_L", shortName="SM_Belt_Misc_SkullWolf_01_L", friendlyName="Skull - Wolf 2", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingF", "soc_beltStrapB" }, limit=1, tags={ "Hunting" }, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Skulls/SM_Belt_Misc_SkullMark", shortName="SM_Belt_Misc_SkullMark", friendlyName="Skull Badge", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Sling02B", "soc_Sling02F", "soc_Sling03B", "soc_Sling03F", "soc_Sling04B", "soc_Sling04F", "soc_Strap01B", "soc_Strap01F", "soc_Strap02B", "soc_Strap02F", "soc_Strap03B", "soc_Strap03F", "soc_Strap04B", "soc_Strap04F" }, limit=2, tags={}, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Skulls/SM_Belt_Misc_SkullMetal_Roped_01", shortName="SM_Belt_Misc_SkullMetal_Roped_01", friendlyName="Skulls - Metal", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB" }, limit=2, tags={ "Creepy" }, rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bandoleers/SM_Belt_Misc_Bandoleer_01_Rope", shortName="SM_Belt_Misc_Bandoleer_01_Rope", friendlyName="Vials - Hanging x1", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap01F", "soc_Strap01F", "soc_Sling01F" }, limit=3, tags={}, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bandoleers/SM_Belt_Misc_Bandoleer_01_FL", shortName="SM_Belt_Misc_Bandoleer_01_FL", friendlyName="Vials - Hanging x2", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Sling03F", "soc_Sling04F", "soc_Strap04F", "soc_Strap03F" }, limit=3, tags={}, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bandoleers/SM_Belt_Misc_Bandoleer_01_FR", shortName="SM_Belt_Misc_Bandoleer_01_FR", friendlyName="Vials - Hanging x3", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Sling01B", "soc_Sling01F", "soc_Sling02F", "soc_Sling03F", "soc_Sling04F" }, limit=3, tags={}, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bandoleers/SM_Belt_Misc_Bandoleer_01_Tripple01", shortName="SM_Belt_Misc_Bandoleer_01_Tripple01", friendlyName="Vials - Strapped 1", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap01F", "soc_Strap01F", "soc_Sling01F" }, limit=3, tags={}, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/Bandoleers/SM_Belt_Misc_Bandoleer_01_Tripple03", shortName="SM_Belt_Misc_Bandoleer_01_Tripple03", friendlyName="Vials - Strapped 2", sockets={ "soc_belt02B_l", "soc_beltB", "soc_beltB02_l", "soc_beltSlingB", "soc_beltSlingF", "soc_beltStrapB", "soc_Strap01F", "soc_Strap01F", "soc_Sling01F" }, limit=3, tags={}, rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_HolderCommon_01", shortName="SM_Belt_Misc_HolderCommon_01", friendlyName="Holding Strap", sockets={}, limit=1, tags={}, rarity="uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Belt/Miscs/SM_Belt_Misc_Holster_01", shortName="SM_Belt_Misc_Holster_01", friendlyName="Holster", sockets={ "beltSlot_01_rSocket" }, limit=0, tags={}, rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT01/Drop/SM_Drop_PistolT01_02", shortName="SM_Drop_PistolT01_02", friendlyName="Pistol - Basic 3", sockets={ "soc_Strap01F", "soc_Sling03F", "soc_beltStrapB", "soc_Sling01F" }, limit=0, tags={ "Pistol" }, rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT01/Drop/SM_Drop_PistolT01_03", shortName="SM_Drop_PistolT01_03", friendlyName="Pistol - Basic 4", sockets={ "soc_Strap01F", "soc_Sling03F", "soc_beltStrapB", "soc_Sling01F" }, limit=0, tags={ "Pistol" }, rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/Drop/SM_Drop_PistolT02_01", shortName="SM_Drop_PistolT02_01", friendlyName="Pistol - Basic 5", sockets={ "soc_Strap01F", "soc_Sling03F", "soc_beltStrapB", "soc_Sling01F" }, limit=0, tags={ "Pistol" }, rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/Drop/SM_Drop_PistolT02_03", shortName="SM_Drop_PistolT02_03", friendlyName="Pistol - Basic 6", sockets={ "soc_Strap01F", "soc_Sling03F", "soc_beltStrapB", "soc_Sling01F" }, limit=0, tags={ "Pistol" }, rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/Drop/SM_Drop_PistolT02_04", shortName="SM_Drop_PistolT02_04", friendlyName="Pistol - Basic 7", sockets={ "soc_Strap01F", "soc_Sling03F", "soc_beltStrapB", "soc_Sling01F" }, limit=0, tags={ "Pistol" }, rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_Blank", shortName="SM_Drop_WeaponRange_Pistol_Blank", friendlyName="Pistol - Belt", sockets={ "soc_Strap01F", "soc_Sling03F", "soc_beltStrapB", "soc_Sling01F" }, limit=0, tags={ "Pistol" }, rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_DrakesDoom", shortName="SM_Drop_WeaponRange_Pistol_DrakesDoom", friendlyName="Pistol - Drake's Doom - Belt", sockets={ "soc_Strap01F", "soc_Sling03F", "soc_beltStrapB", "soc_Sling01F" }, limit=0, tags={ "Pistol" }, rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_Faithful", shortName="SM_Drop_WeaponRange_Pistol_Faithful", friendlyName="Pistol - Faithful - Belt", sockets={ "soc_Strap01F", "soc_Sling03F", "soc_beltStrapB", "soc_Sling01F" }, limit=0, tags={ "Pistol" }, rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_Rusty", shortName="SM_Drop_WeaponRange_Pistol_Rusty", friendlyName="Pistol - Rusty - Belt", sockets={ "soc_Strap01F", "soc_Sling03F", "soc_beltStrapB", "soc_Sling01F" }, limit=0, tags={ "Pistol" }, rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_Devastating", shortName="SM_Drop_WeaponRange_Pistol_Devastating", friendlyName="Pistol - Devastating - Belt", sockets={ "soc_Lantern" }, limit=0, tags={ "Pistol" }, rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_Baneful", shortName="SM_Drop_WeaponRange_Pistol_Baneful", friendlyName="Pistol - Baneful - Belt", sockets={ "soc_Lantern" }, limit=0, tags={ "Pistol" }, rarity="-" },
}

Config.SOCKETITEMS_WEAPONS = {
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT01/Drop/SM_Drop_MusketT01_01", shortName="SM_Drop_MusketT01_01", friendlyName="Musket - Basic 1", sockets={ "Crossbow2h_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT01/Drop/SM_Drop_MusketT01_03", shortName="SM_Drop_MusketT01_03", friendlyName="Musket - Basic 3", sockets={ "Crossbow2h_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT01/SK_MusketT01_02", shortName="SK_MusketT01_02", friendlyName="Musket - Basic 2", sockets={ "Musket_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/Drop/SM_Drop_MusketArtifact_01", shortName="SM_Drop_MusketArtifact_01", friendlyName="Musket - Artifact", sockets={ "Crossbow2h_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/Drop/SM_Drop_MusketT02_01", shortName="SM_Drop_MusketT02_01", friendlyName="Musket - Basic 4", sockets={ "Crossbow2h_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/Drop/SM_Drop_MusketT02_03", shortName="SM_Drop_MusketT02_03", friendlyName="Musket - Basic 5", sockets={ "Crossbow2h_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/SK_PistolT02_01", shortName="SK_PistolT02_01", friendlyName="Pistol - Basic 1", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={ "Soldiering" }, location="Pistol", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/SK_PistolT02_03", shortName="SK_PistolT02_03", friendlyName="Pistol - Basic 2", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={ "Soldiering" }, location="Pistol", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT01/Drop/SM_Drop_PistolT01_02", shortName="SM_Drop_PistolT01_02", friendlyName="Pistol - Basic 3", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={}, location="Pistol", rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT01/Drop/SM_Drop_PistolT01_03", shortName="SM_Drop_PistolT01_03", friendlyName="Pistol - Basic 4", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={}, location="Pistol", rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/Drop/SM_Drop_PistolT02_01", shortName="SM_Drop_PistolT02_01", friendlyName="Pistol - Basic 5", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={}, location="Pistol", rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/Drop/SM_Drop_PistolT02_03", shortName="SM_Drop_PistolT02_03", friendlyName="Pistol - Basic 6", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={}, location="Pistol", rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/Drop/SM_Drop_PistolT02_04", shortName="SM_Drop_PistolT02_04", friendlyName="Pistol - Basic 7", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={}, location="Pistol", rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_Blank", shortName="SM_Drop_WeaponRange_Pistol_Blank", friendlyName="Pistol - Belt", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={}, location="Pistol", rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_DrakesDoom", shortName="SM_Drop_WeaponRange_Pistol_DrakesDoom", friendlyName="Pistol - Drake's Doom - Belt", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={}, location="Pistol", rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_Faithful", shortName="SM_Drop_WeaponRange_Pistol_Faithful", friendlyName="Pistol - Faithful - Belt", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={}, location="Pistol", rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_Rusty", shortName="SM_Drop_WeaponRange_Pistol_Rusty", friendlyName="Pistol - Rusty - Belt", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={}, location="Pistol", rarity="-" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_Artifact/Drop/SM_Drop_ClubArtifact_01", shortName="SM_Drop_ClubArtifact_01", friendlyName="Club - Artifact", sockets={ "swordSlot_lSocket" }, tags={ "Hunting" }, location="Sheath", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_Boss/Thorn/Saber/SK_WeaponBoss_Thorn_Saber", shortName="SK_WeaponBoss_Thorn_Saber", friendlyName="Saber - Thorn", sockets={ "Axe2h_backsocket" }, tags={ "Creepy", "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_Corrupted/Drop/SM_Drop_Spear_Corrupted", shortName="SM_Drop_Spear_Corrupted", friendlyName="Spear - Corrupted", sockets={ "Halberd_backsocket" }, tags={ "Creepy", "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_Corrupted/SK_Axe_Corrupted", shortName="SK_Axe_Corrupted", friendlyName="Axe - Corrupted", sockets={ "Axe2h_backsocket" }, tags={ "Creepy" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_Corrupted/SK_Pickaxe_Corrupted", shortName="SK_Pickaxe_Corrupted", friendlyName="Pickaxe - Corrupted", sockets={ "Axe2h_backsocket" }, tags={ "Creepy", "Tool" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_Dendromorph/SK_Weapon_Dendromorph_Macuahuitl", shortName="SK_Weapon_Dendromorph_Macuahuitl", friendlyName="Macuahuitl - Dendromorph", sockets={ "swordSlot_lSocket" }, tags={ "Creepy", "Soldiering" }, location="Sheath", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_Dendromorph/SK_Weapon_Dendromorph_Spear_Scale075", shortName="SK_Weapon_Dendromorph_Spear_Scale075", friendlyName="Spear - Dendromorph", sockets={ "Musket_backsocket" }, tags={ "Creepy", "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_FishingRod/Fishing_Regular/SK_Gear_Fishing_Rod_Regular", shortName="SK_Gear_Fishing_Rod_Regular", friendlyName="Fishing Rod", sockets={ "Halberd_backsocket" }, tags={ "Fishing" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_Steel/Drop/SM_Drop_Gaff_Steel_01", shortName="SM_Drop_Gaff_Steel_01", friendlyName="Gaff - Steel", sockets={ "Halberd_backsocket" }, tags={ "Fishing" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_Steel/Drop/SM_Drop_Spear_Steel_01", shortName="SM_Drop_Spear_Steel_01", friendlyName="Spear - Steel", sockets={ "Halberd_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_Stone/SK_Weapon_Stone_Axe", shortName="SK_Weapon_Stone_Axe", friendlyName="Axe - Stone", sockets={ "Axe2h_backsocket" }, tags={ "Tool" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/Weapon_Stone/SK_Weapon_Stone_Pickaxe", shortName="SK_Weapon_Stone_Pickaxe", friendlyName="Pickaxe - Stone", sockets={ "Axe2h_backsocket" }, tags={ "Tool" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Club/SK_WeaponMelee_Club_Blank", shortName="SK_WeaponMelee_Club_Blank", friendlyName="Club", sockets={ "swordSlot_lSocket" }, tags={ "Hunting" }, location="Sheath", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Club/SK_WeaponMelee_Club_Corrupted", shortName="SK_WeaponMelee_Club_Corrupted", friendlyName="Club - Corrupted", sockets={ "swordSlot_lSocket" }, tags={}, location="Sheath", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Club/SK_WeaponMelee_Club_Forceful", shortName="SK_WeaponMelee_Club_Forceful", friendlyName="Club - Forceful", sockets={ "swordSlot_lSocket" }, tags={ "Hunting" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Club/SK_WeaponMelee_Club_Stunning", shortName="SK_WeaponMelee_Club_Stunning", friendlyName="Club - Stunning", sockets={ "swordSlot_lSocket" }, tags={ "Hunting" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Club/SK_WeaponMelee_Club_Tremor", shortName="SK_WeaponMelee_Club_Tremor", friendlyName="Club - Tremor", sockets={ "swordSlot_lSocket" }, tags={}, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/Drop/SM_Drop_WeaponMelee_GreatSword_Voved", shortName="SM_Drop_WeaponMelee_GreatSword_Voved", friendlyName="GreatSword - Voved", sockets={ "Crossbow2h_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Blank", shortName="SK_WeaponMelee_Greatsword_Blank", friendlyName="Greatsword", sockets={ "GSword_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Parrying", shortName="SK_WeaponMelee_Greatsword_Parrying", friendlyName="Greatsword - Parrying", sockets={ "GSword_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Reliable", shortName="SK_WeaponMelee_Greatsword_Reliable", friendlyName="Greatsword - Reliable", sockets={ "GSword_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Savage", shortName="SK_WeaponMelee_Greatsword_Savage", friendlyName="Greatsword - Savage", sockets={ "GSword_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Slicer", shortName="SK_WeaponMelee_Greatsword_Slicer", friendlyName="Greatsword - Slicer", sockets={ "GSword_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Souldrinker", shortName="SK_WeaponMelee_Greatsword_Souldrinker", friendlyName="Greatsword - Souldrinker", sockets={ "GSword_backsocket" }, tags={ "Creepy", "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Wicked", shortName="SK_WeaponMelee_Greatsword_Wicked", friendlyName="Greatsword - Wicked", sockets={ "GSword_backsocket" }, tags={ "Creepy", "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Halberd/SK_WeaponMelee_Halberd_Blank", shortName="SK_WeaponMelee_Halberd_Blank", friendlyName="Halberd", sockets={ "Halberd_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Halberd/SK_WeaponMelee_Halberd_Corrupted", shortName="SK_WeaponMelee_Halberd_Corrupted", friendlyName="Halberd - Corrupted", sockets={ "Halberd_backsocket" }, tags={ "Creepy", "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Halberd/SK_WeaponMelee_Halberd_Executioner", shortName="SK_WeaponMelee_Halberd_Executioner", friendlyName="Halberd - Executioner", sockets={ "Halberd_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Halberd/SK_WeaponMelee_Halberd_Reliable", shortName="SK_WeaponMelee_Halberd_Reliable", friendlyName="Halberd - Reliable", sockets={ "Halberd_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Blank", shortName="SK_WeaponMelee_Rapier_Blank", friendlyName="Rapier", sockets={ "rapierSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Bleeding", shortName="SK_WeaponMelee_Rapier_Bleeding", friendlyName="Rapier - Bleeding", sockets={ "rapierSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Bleeding_Advanced", shortName="SK_WeaponMelee_Rapier_Bleeding_Advanced", friendlyName="Rapier - Advanced - Bleeding", sockets={ "rapierSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Eviscerate", shortName="SK_WeaponMelee_Rapier_Eviscerate", friendlyName="Rapier - Eviscerate", sockets={ "rapierSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Loyal", shortName="SK_WeaponMelee_Rapier_Loyal", friendlyName="Rapier - Loyal", sockets={ "rapierSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Relentless", shortName="SK_WeaponMelee_Rapier_Relentless", friendlyName="Rapier - Relentless", sockets={ "rapierSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Reliable", shortName="SK_WeaponMelee_Rapier_Reliable", friendlyName="Rapier - Reliable", sockets={ "rapierSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Swift", shortName="SK_WeaponMelee_Rapier_Swift", friendlyName="Rapier - Swift", sockets={ "rapierSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Baneful", shortName="SK_WeaponMelee_Saber_Baneful", friendlyName="Saber - Baneful", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Blank", shortName="SK_WeaponMelee_Saber_Blank", friendlyName="Saber", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Boarding", shortName="SK_WeaponMelee_Saber_Boarding", friendlyName="Saber - Boarding", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Broken", shortName="SK_WeaponMelee_Saber_Broken", friendlyName="Saber - Broken", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Corrupted", shortName="SK_WeaponMelee_Saber_Corrupted", friendlyName="Saber - Corrupted", sockets={ "swordSlot_lSocket" }, tags={ "Creepy", "Soldiering" }, location="Sheath", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Feral", shortName="SK_WeaponMelee_Saber_Feral", friendlyName="Saber - Feral", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Flaweless", shortName="SK_WeaponMelee_Saber_Flaweless", friendlyName="Saber - Flaweless", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Forged", shortName="SK_WeaponMelee_Saber_Forged", friendlyName="Saber - Forged", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Graceful", shortName="SK_WeaponMelee_Saber_Graceful", friendlyName="Saber - Graceful", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Parrying", shortName="SK_WeaponMelee_Saber_Parrying", friendlyName="Saber - Parrying", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Relentless", shortName="SK_WeaponMelee_Saber_Relentless", friendlyName="Saber - Relentless", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Reliable", shortName="SK_WeaponMelee_Saber_Reliable", friendlyName="Saber - Reliable", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Resolute", shortName="SK_WeaponMelee_Saber_Resolute", friendlyName="Saber - Resolute", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Restless", shortName="SK_WeaponMelee_Saber_Restless", friendlyName="Saber - Restless", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Savage", shortName="SK_WeaponMelee_Saber_Savage", friendlyName="Saber - Savage", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Severe", shortName="SK_WeaponMelee_Saber_Severe", friendlyName="Saber - Severe", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Vengeful", shortName="SK_WeaponMelee_Saber_Vengeful", friendlyName="Saber - Vengeful", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Blunderbuss/Drop/SM_Drop__WeaponRange_Blunderbuss_Fateful", shortName="SM_Drop__WeaponRange_Blunderbuss_Fateful", friendlyName="Blunderbuss - Fateful", sockets={ "Crossbow2h_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Blunderbuss/SK_WeaponRange_Blunderbuss_Blank", shortName="SK_WeaponRange_Blunderbuss_Blank", friendlyName="Blunderbuss", sockets={ "Musket_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Blunderbuss/SK_WeaponRange_Blunderbuss_DragonBreath", shortName="SK_WeaponRange_Blunderbuss_DragonBreath", friendlyName="Blunderbuss - DragonBreath", sockets={ "Musket_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Blunderbuss/SK_WeaponRange_Blunderbuss_Reliable", shortName="SK_WeaponRange_Blunderbuss_Reliable", friendlyName="Blunderbuss - Reliable", sockets={ "Musket_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Musket/Drop/SM_Drop_WeaponRange_Musket_Baneful", shortName="SM_Drop_WeaponRange_Musket_Baneful", friendlyName="Musket - Baneful", sockets={ "Crossbow2h_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Musket/Drop/SM_Drop_WeaponRange_Musket_Relentless", shortName="SM_Drop_WeaponRange_Musket_Relentless", friendlyName="Musket - Relentless", sockets={ "Crossbow2h_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Musket/Drop/SM_Drop_WeaponRange_Musket_Wicked", shortName="SM_Drop_WeaponRange_Musket_Wicked", friendlyName="Musket - Wicked", sockets={ "Crossbow2h_backsocket" }, tags={ "Creepy", "Hunting", "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Musket/SK_WeaponRange_Musket_Blank", shortName="SK_WeaponRange_Musket_Blank", friendlyName="Musket", sockets={ "Musket_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Musket/SK_WeaponRange_Musket_Infantry", shortName="SK_WeaponRange_Musket_Infantry", friendlyName="Musket - Infantry", sockets={ "Musket_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Musket/SK_WeaponRange_Musket_Reliable", shortName="SK_WeaponRange_Musket_Reliable", friendlyName="Musket - Reliable", sockets={ "Musket_backsocket" }, tags={ "Hunting", "Soldiering" }, location="Back", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Musket/SK_WeaponRange_Musket_Sniper", shortName="SK_WeaponRange_Musket_Sniper", friendlyName="Musket - Sniper", sockets={ "Musket_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/SK_WeaponRange_Pistol_Blank", shortName="SK_WeaponRange_Pistol_Blank", friendlyName="Pistol", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={ "Hunting" }, location="Pistol", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/SK_WeaponRange_Pistol_Corrupted", shortName="SK_WeaponRange_Pistol_Corrupted", friendlyName="Pistol - Corrupted", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={ "Creepy", "Hunting", "Soldiering" }, location="Pistol", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/SK_WeaponRange_Pistol_DrakesDoom", shortName="SK_WeaponRange_Pistol_DrakesDoom", friendlyName="Pistol - Drake's Doom", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={ "Hunting", "Soldiering" }, location="Pistol", rarity="Rare" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/SK_WeaponRange_Pistol_Reliable", shortName="SK_WeaponRange_Pistol_Reliable", friendlyName="Pistol - Reliable", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={ "Hunting", "Soldiering" }, location="Pistol", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/SK_WeaponRange_Pistol_Rusty", shortName="SK_WeaponRange_Pistol_Rusty", friendlyName="Pistol - Rusty", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={ "Hunting", "Soldiering" }, location="Pistol", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT01/SK_ClubT01_01", shortName="SK_ClubT01_01", friendlyName="Club - Basic", sockets={ "Axe2h_backsocket" }, tags={ "Hunting" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT01/SK_HalberdT01_01", shortName="SK_HalberdT01_01", friendlyName="Halberd - Basic", sockets={ "Halberd_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT01/SK_KnifeT01_01", shortName="SK_KnifeT01_01", friendlyName="Knife - Basic", sockets={ "swordSlot_lSocket" }, tags={ "Hunting", "Fishing" }, location="Sheath", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT02/Drop/SM_Drop_GreatAxeT02_01", shortName="SM_Drop_GreatAxeT02_01", friendlyName="Great Axe - Basic", sockets={ "Crossbow2h_backsocket" }, tags={ "Soldiering" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT02/SK_AxeT02_01", shortName="SK_AxeT02_01", friendlyName="Axe - Basic 1", sockets={ "Axe2h_backsocket" }, tags={}, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT02/SK_PickaxeT02_01", shortName="SK_PickaxeT02_01", friendlyName="Pickaxe - Basic 1", sockets={ "Axe2h_backsocket" }, tags={ "Tool" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT02/SK_SaberT02_01", shortName="SK_SaberT02_01", friendlyName="Saber - Basic 1", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT02/SK_ShovelT02_01", shortName="SK_ShovelT02_01", friendlyName="Shovel 1", sockets={ "Musket_backsocket" }, tags={ "Tool" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT03/Drop/SM_Drop_ShovelT03_01", shortName="SM_Drop_ShovelT03_01", friendlyName="Shovel 2", sockets={ "Crossbow2h_backsocket" }, tags={ "Tool" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT03/SK_AxeT03_01", shortName="SK_AxeT03_01", friendlyName="Axe - Basic 2", sockets={ "Axe2h_backsocket" }, tags={}, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT03/SK_PickaxeT03_01", shortName="SK_PickaxeT03_01", friendlyName="Pickaxe - Basic 2", sockets={ "Axe2h_backsocket" }, tags={ "Tool" }, location="Back", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponT03/SK_SaberT03_01", shortName="SK_SaberT03_01", friendlyName="Saber - Basic 2", sockets={ "swordSlot_lSocket" }, tags={ "Soldiering" }, location="Sheath", rarity="Common" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_Devastating", shortName="SM_Drop_WeaponRange_Pistol_Devastating", friendlyName="Pistol - Devastating", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={ "Hunting", "Soldiering" }, location="Pistol", rarity="Uncommon" },
  { asset="/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/Drop/SM_Drop_WeaponRange_Pistol_Baneful", shortName="SM_Drop_WeaponRange_Pistol_Baneful", friendlyName="Pistol - Baneful", sockets={ "beltSlot_01_lSocket", "beltSlot_01_rSocket" }, tags={ "Hunting", "Soldiering" }, location="Pistol", rarity="Rare" },
}

-- Config.SOCKETITEMS_TOOLS (2026-09-21) -- RedFalcon's "Tools" tab in Other\SocketItems.xlsx: the
-- flat item list backing the Custom tab's Left/Right Hand item dropdowns (Weapons/Tools/Bottles/
-- Other, one dropdown per `type` value here -- these are exactly the 4 real Type values the sheet
-- itself uses). Deliberately a SEPARATE table from SOCKETITEMS_ITEMS/SOCKETITEMS_WEAPONS above
-- (no shared friendly names between this and Items; some overlap with Weapons is fine -- see
-- Spawner.ApplySocketItemManual's own header for why that never causes ambiguity: it only ever
-- checks Items then this table, never Weapons, for a manual hand pick). Same `asset` field name/
-- convention as those tables (some rows already carry the dotted .AssetName suffix, some don't --
-- resolveMeshAndActorForFill handles either, same as everywhere else in this file).
Config.SOCKETITEMS_TOOLS = {
  { friendlyName = "Bulbous Bottle", type = "Bottles", asset = "/Game/Environment/Gameplay/Resources/Potions/SM_PotionT02_01" },
  { friendlyName = "Clay Bottle", type = "Bottles", asset = "/Game/TMP/ResourcesTEMP/Craft/SM_Craft_T01_ClayBottle_01" },
  { friendlyName = "Faceted Bottle", type = "Bottles", asset = "/Game/Environment/Gameplay/Resources/Potions/SM_PotionT03_01" },
  { friendlyName = "Fire Damage Potion", type = "Bottles", asset = "/Game/Character/Skeletal_Meshes/Environment/Workbenches/Meshes/SK_OilT01_FireDmg_01" },
  { friendlyName = "Flat Bottomed Bottle", type = "Bottles", asset = "/Game/Environment/Gameplay/Resources/Potions/SM_PotionT02_02" },
  { friendlyName = "Greater Healing Potion", type = "Bottles", asset = "/Game/Environment/Gameplay/Resources/Potions/SM_PotionT03_Heal_01" },
  { friendlyName = "Healing Potion", type = "Bottles", asset = "/Game/Environment/Gameplay/Resources/Potions/SM_PotionT02_Heal_01" },
  { friendlyName = "Large Faceted Bottle", type = "Bottles", asset = "/Game/Environment/Gameplay/Resources/Potions/SM_PotionT03_02" },
  { friendlyName = "Lesser Healing Potion", type = "Bottles", asset = "/Game/Environment/Gameplay/Resources/Potions/SM_PotionT01_Heal_01" },
  { friendlyName = "Rum Bottle", type = "Bottles", asset = "/Game/Environment/Gameplay/Resources/Craft/SM_CraftT03_RumBottle_01" },
  { friendlyName = "Vial Bottle", type = "Bottles", asset = "/Game/Environment/Gameplay/Resources/Potions/SM_PotionT01_01" },
  { friendlyName = "Coconut", type = "Other", asset = "/Game/Environment/Gameplay/Resources/SK_Coconut_01" },
  { friendlyName = "Gem Stone", type = "Other", asset = "/Game/Character/Skeletal_Meshes/Environment/Workbenches/Meshes/SK_LootT02_AmuletPieceBig_01" },
  { friendlyName = "Shackles", type = "Other", asset = "/Game/TMP/RuinsTEMP/SM_Shackles_01" },
  { friendlyName = "Cleaver", type = "Tools", asset = "/Game/Character/Skeletal_Meshes/Environment/Workbenches/Meshes/SK_CraftStation_Tools_Cleaver_01" },
  { friendlyName = "Fishing Rod", type = "Tools", asset = "/Game/Character/Skeletal_Meshes/Weapons/Weapon_FishingRod/Fishing_Regular/SK_Gear_Fishing_Rod_Regular.SK_Gear_Fishing_Rod_Regular" },
  { friendlyName = "Hammer - Steel", type = "Tools", asset = "/Game/Character/Skeletal_Meshes/Environment/Workbenches/Meshes/SK_CraftStation_Tools_Hammer_01" },
  { friendlyName = "Hammer - Wooden", type = "Tools", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponT01/SK_HammerT01_01" },
  { friendlyName = "Hand Saw", type = "Tools", asset = "/Game/Character/Skeletal_Meshes/Environment/Workbenches/Meshes/SK_CraftStation_Tools_Saw_01" },
  { friendlyName = "Kitchen Knife", type = "Tools", asset = "/Game/Character/Skeletal_Meshes/Environment/Workbenches/Meshes/SK_CraftStation_Tools_Knife_01" },
  { friendlyName = "Pickaxe - Copper", type = "Tools", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponT02/SK_PickaxeT02_01" },
  { friendlyName = "Pickaxe - Corrupted", type = "Tools", asset = "/Game/Character/Skeletal_Meshes/Weapons/Weapon_Corrupted/SK_Pickaxe_Corrupted.SK_Pickaxe_Corrupted" },
  { friendlyName = "Pickaxe - Iron", type = "Tools", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponT03/SK_PickaxeT03_01" },
  { friendlyName = "Pickaxe - Stone", type = "Tools", asset = "/Game/Character/Skeletal_Meshes/Weapons/Weapon_Stone/SK_Weapon_Stone_Pickaxe" },
  { friendlyName = "Shovel", type = "Tools", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponT02/SK_ShovelT02_01" },
  { friendlyName = "Axe - Copper", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponT02/SK_AxeT02_01" },
  { friendlyName = "Axe - Corrupted", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/Weapon_Corrupted/SK_Axe_Corrupted.SK_Axe_Corrupted" },
  { friendlyName = "Axe - Iron", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponT03/SK_AxeT03_01" },
  { friendlyName = "Axe - Stone", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/Weapon_Stone/SK_Weapon_Stone_Axe" },
  { friendlyName = "Blunderbuss", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Blunderbuss/SK_WeaponRange_Blunderbuss_Blank" },
  { friendlyName = "Blunderbuss - Dragon's Breath", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Blunderbuss/SK_WeaponRange_Blunderbuss_DragonBreath" },
  { friendlyName = "Blunderbuss - Reliable", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Blunderbuss/SK_WeaponRange_Blunderbuss_Reliable" },
  { friendlyName = "Club", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Club/SK_WeaponMelee_Club_Blank" },
  { friendlyName = "Club - Boatwain", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponHeavy/SK_WeaponBoss_Boatswain_Club" },
  { friendlyName = "Club - Corrupted", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Club/SK_WeaponMelee_Club_Corrupted" },
  { friendlyName = "Club - Forceful", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Club/SK_WeaponMelee_Club_Forceful" },
  { friendlyName = "Club - Stunning", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Club/SK_WeaponMelee_Club_Stunning" },
  { friendlyName = "Club - Tremor", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Club/SK_WeaponMelee_Club_Tremor" },
  { friendlyName = "Greatsword", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Blank" },
  { friendlyName = "Greatsword - Parrying", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Parrying" },
  { friendlyName = "Greatsword - Reliable", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Reliable" },
  { friendlyName = "Greatsword - Savage", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Savage" },
  { friendlyName = "Greatsword - Slicer", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Slicer" },
  { friendlyName = "Greatsword - Soul Drinker", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Souldrinker" },
  { friendlyName = "Greatsword - Wicked", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_GreatSword/SK_WeaponMelee_Greatsword_Wicked" },
  { friendlyName = "Halberd", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Halberd/SK_WeaponMelee_Halberd_Blank" },
  { friendlyName = "Halberd - Chipped", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponT01/SK_HalberdT01_01" },
  { friendlyName = "Halberd - Corrupted", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Halberd/SK_WeaponMelee_Halberd_Corrupted" },
  { friendlyName = "Halberd - Executioner", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Halberd/SK_WeaponMelee_Halberd_Executioner" },
  { friendlyName = "Halberd - Reliable", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Halberd/SK_WeaponMelee_Halberd_Reliable" },
  { friendlyName = "Large Knife", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponT01/SK_KnifeT01_01" },
  { friendlyName = "Macuahuitl", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/Weapon_Dendromorph/SK_Weapon_Dendromorph_Macuahuitl.SK_Weapon_Dendromorph_Macuahuitl" },
  { friendlyName = "Musket", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Musket/SK_WeaponRange_Musket_Blank" },
  { friendlyName = "Musket - Infantry", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Musket/SK_WeaponRange_Musket_Infantry" },
  { friendlyName = "Musket - Reliable", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Musket/SK_WeaponRange_Musket_Reliable" },
  { friendlyName = "Musket - Sniper", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Musket/SK_WeaponRange_Musket_Sniper" },
  { friendlyName = "Pistol", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/SK_WeaponRange_Pistol_Blank" },
  { friendlyName = "Pistol - Corrupted", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/SK_WeaponRange_Pistol_Corrupted" },
  { friendlyName = "Pistol - Drake's Doom", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/SK_WeaponRange_Pistol_DrakesDoom" },
  { friendlyName = "Pistol - Reliable", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/SK_WeaponRange_Pistol_Reliable" },
  { friendlyName = "Pistol - Rusty", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponRange_Pistol/SK_WeaponRange_Pistol_Rusty" },
  { friendlyName = "Pistol - Shabby", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/SK_PistolT02_03" },
  { friendlyName = "Pistol - Worn", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/FirearmsT02/SK_PistolT02_01" },
  { friendlyName = "Rapier", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Blank" },
  { friendlyName = "Rapier - Bleeding", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Bleeding" },
  { friendlyName = "Rapier - Bleeding Advanced", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Bleeding_Advanced" },
  { friendlyName = "Rapier - Eviserate", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Eviscerate" },
  { friendlyName = "Rapier - Loyal", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Loyal" },
  { friendlyName = "Rapier - Relentless", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Relentless" },
  { friendlyName = "Rapier - Reliable", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Reliable" },
  { friendlyName = "Rapier - Swift", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Rapier/SK_WeaponMelee_Rapier_Swift" },
  { friendlyName = "Saber - Baneful", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Baneful" },
  { friendlyName = "Saber - Boarding", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Boarding" },
  { friendlyName = "Saber - Broken", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Broken" },
  { friendlyName = "Saber - Corrupted", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Corrupted" },
  { friendlyName = "Saber - Feral", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Feral" },
  { friendlyName = "Saber - Filigree", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Blank" },
  { friendlyName = "Saber - Fish", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/Weapon_Strange/SK_SaberFish_01" },
  { friendlyName = "Saber - Flawless", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Flaweless" },
  { friendlyName = "Saber - Forged", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Forged" },
  { friendlyName = "Saber - Graceful", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Graceful" },
  { friendlyName = "Saber - No Filigree", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponT03/SK_SaberT03_01" },
  { friendlyName = "Saber - Parrying", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Parrying" },
  { friendlyName = "Saber - Relentless", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Relentless" },
  { friendlyName = "Saber - Reliable", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Reliable" },
  { friendlyName = "Saber - Resolute", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Resolute" },
  { friendlyName = "Saber - Restless", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Restless" },
  { friendlyName = "Saber - Rusty", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponT02/SK_SaberT02_01" },
  { friendlyName = "Saber - Savage", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Savage" },
  { friendlyName = "Saber - Severe", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Severe" },
  { friendlyName = "Saber - Thorn", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/Weapon_Boss/Thorn/Saber/SK_WeaponBoss_Thorn_Saber" },
  { friendlyName = "Saber - Vengeful", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/WeaponMelee_Saber/SK_WeaponMelee_Saber_Vengeful" },
  { friendlyName = "Spear", type = "Weapons", asset = "/Game/Character/Skeletal_Meshes/Weapons/Weapon_Dendromorph/SK_Weapon_Dendromorph_Spear_Scale075.SK_Weapon_Dendromorph_Spear_Scale075" },
}
------------------------------------------------------------------
...
-- MASK added (2026-09-16, RedFalcon: "add it back in... they are all one color so treat it the same
-- as waist or cape") -- bodyPart 5 is CPD_BODYPART_COLOR_INFO's own "Scarf" ordinal
-- (colorSlots=1, "Only Blackbeard Pirate has this piece at all"); no specific slot documented there
-- (unlike Waist's own confirmed Color3-only note), so this follows Cape's own convention: pick one
-- representative slot (Color1) and rely on writeColorRequest's existing "fan out to all 3 slots"
-- rule for a slotCount==1 row to land correctly regardless of which one this piece actually reads.
Config.CUSTOM_TAB_CLOTH_CATEGORIES = {
  ...
  { key = "MASK",  label = "Mask",  bodyPart = 5 },
}

-- Config.HAIR_CATEGORY_ITEMS (2026-09-12) -- generated from Other/HairCategories.xlsx
-- (RedFalcon's own spreadsheet, sheet 'Hair-Beard-Mustache-Whiskers'). One row per real
-- hair/facial-hair option, grouped by bodyPart (Hairs/Beard/Mustache/Whiskers/Sets) and
-- already sorted alphabetically by friendlyName within each group (the exact order the
-- Custom tab's dropdowns list them in -- do not re-sort at runtime, this order IS the
-- source of truth). 'Sets' rows carry no mesh paths at all -- selecting a Set means look
-- up the SAME friendlyName in Beard/Mustache/Whiskers and apply all three (RedFalcon's
-- own design). hatFriendly is a placeholder column in the spreadsheet, not yet populated
-- for any row -- carried through here so a future data pass can add hat-compatibility
-- info without a schema change; nil today for every entry, not a bug.
-- 'Eyebrows' rows (2026-09-14 addition, hand-ported from Config.CUSTOM_FACIAL, NOT from the
-- spreadsheet) are sex-paired like Hairs -- no facial-hair male-only gate applies to them, see
-- Spawner.ApplyHairCategoryMesh/ApplyHairCategoryColor's own bodyPartKey checks.
Config.HAIR_CATEGORY_ITEMS = {
  { bodyPart="Hairs", name="Afro_01", friendlyName="Afro 1", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Afro/SK_Hair_Afro_01_Default_Male.SK_Hair_Afro_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Afro/SK_Hair_Afro_01_Default_Female.SK_Hair_Afro_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Afro_02", friendlyName="Afro 2", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Afro/SK_Hair_Afro_02_Default_Male.SK_Hair_Afro_02_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Afro/SK_Hair_Afro_02_Default_Female.SK_Hair_Afro_02_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Afro_03", friendlyName="Afro 3", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Afro/SK_Hair_Afro_03_Default_Male.SK_Hair_Afro_03_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Afro/SK_Hair_Afro_03_Default_Female.SK_Hair_Afro_03_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Afro_04", friendlyName="Afro 4", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Afro/SK_Hair_Afro_04_Default_Male.SK_Hair_Afro_04_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Afro/SK_Hair_Afro_04_Default_Female.SK_Hair_Afro_04_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Afro_05", friendlyName="Afro 5", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Afro/SK_Hair_Afro_05_Default_Male.SK_Hair_Afro_05_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Afro/SK_Hair_Afro_05_Default_Female.SK_Hair_Afro_05_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Braid", friendlyName="Braid", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Braid/SK_Hair_Braid_01_Default_Male.SK_Hair_Braid_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Braid/SK_Hair_Braid_01_Default_Female.SK_Hair_Braid_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Bristle", friendlyName="Bristle", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Bristle/SK_Hair_Bristle_01_Default_Male.SK_Hair_Bristle_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Bristle/SK_Hair_Bristle_01_Default_Female.SK_Hair_Bristle_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Bun_01", friendlyName="Bun 1", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Bun/SK_Hair_Bun_01_Default_Male.SK_Hair_Bun_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Bun/SK_Hair_Bun_01_Default_Female.SK_Hair_Bun_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Bun_02", friendlyName="Bun 2", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Bun/SK_Hair_Bun_02_Default_Male.SK_Hair_Bun_02_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Bun/SK_Hair_Bun_02_Default_Female.SK_Hair_Bun_02_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="LayeredBob", friendlyName="Layered Bob", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/LayeredBob/SK_Hair_LayeredBob_01_Default_Male.SK_Hair_LayeredBob_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/LayeredBob/SK_Hair_LayeredBob_01_Default_Female.SK_Hair_LayeredBob_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="LayeredBobDecor", friendlyName="Layered Bob Decorated", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/LayeredBob/SK_Hair_LayeredBobDecor_01_Default_Male.SK_Hair_LayeredBobDecor_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/LayeredBob/SK_Hair_LayeredBobDecor_01_Default_Female.SK_Hair_LayeredBobDecor_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Mohawk", friendlyName="Mohawk", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Mohawk/SK_Hair_Mohawk_01_Default_Male.SK_Hair_Mohawk_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Mohawk/SK_Hair_Mohawk_01_Default_Female.SK_Hair_Mohawk_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="PartialDredlocks", friendlyName="Partial Dredlocks", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/PartialDreadlocks/SK_Hair_PartialDreadlocks_01_Default_Male.SK_Hair_PartialDreadlocks_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/PartialDreadlocks/SK_Hair_PartialDreadlocks_01_Default_Female.SK_Hair_PartialDreadlocks_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Pixie_01", friendlyName="Pixie 1", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Pixie/SK_Hair_Pixie_01_Default_Male.SK_Hair_Pixie_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Pixie/SK_Hair_Pixie_01_Default_Female.SK_Hair_Pixie_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Pixie_02", friendlyName="Pixie 2", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Pixie/SK_Hair_Pixie_02_Default_Male.SK_Hair_Pixie_02_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Pixie/SK_Hair_Pixie_02_Default_Female.SK_Hair_Pixie_02_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Pixie_03", friendlyName="Pixie 3", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Pixie/SK_Hair_Pixie_03_Default_Male.SK_Hair_Pixie_03_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Pixie/SK_Hair_Pixie_03_Default_Female.SK_Hair_Pixie_03_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Ponytail", friendlyName="Ponytail", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Ponytail/SK_Hair_Ponytail_01_Default_Male.SK_Hair_Ponytail_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Ponytail/SK_Hair_Ponytail_01_Default_Female.SK_Hair_Ponytail_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Shag", friendlyName="Shag 1", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Shag/SK_Hair_Shag_Default_Male.SK_Hair_Shag_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Shag/SK_Hair_Shag_Default_Female.SK_Hair_Shag_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Shag_02", friendlyName="Shag 2", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Shag/SK_Hair_Shag_02_Default_Male.SK_Hair_Shag_02_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Shag/SK_Hair_Shag_02_Default_Female.SK_Hair_Shag_02_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Shag_03", friendlyName="Shag 3", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Shag/SK_Hair_Shag_03_Default_Male.SK_Hair_Shag_03_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Shag/SK_Hair_Shag_03_Default_Female.SK_Hair_Shag_03_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Shag_04", friendlyName="Shag 4", availability="BOTH", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Shag/SK_Hair_Shag_04_Default_Male.SK_Hair_Shag_04_Default_Male", femalePath=nil, unisexPath=nil },
  { bodyPart="Hairs", name="Shag_05", friendlyName="Shag 5", availability="BOTH", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Shag/SK_Hair_Shag_05_Default_Male.SK_Hair_Shag_05_Default_Male", femalePath=nil, unisexPath=nil },
  { bodyPart="Hairs", name="Shag_06", friendlyName="Shag 6", availability="BOTH", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Shag/SK_Hair_Shag_06_Default.SK_Hair_Shag_06_Default", femalePath=nil, unisexPath=nil },
  { bodyPart="Hairs", name="ShortBob", friendlyName="Short Bob", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/ShortBob/SK_Hair_ShortBob_Default_Male.SK_Hair_ShortBob_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/ShortBob/SK_Hair_ShortBob_Default_Female.SK_Hair_ShortBob_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Slickback", friendlyName="Slicked Back", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Slickback/SK_Hair_Slickback_Default_Male.SK_Hair_Slickback_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Slickback/SK_Hair_Slickback_Default_Female.SK_Hair_Slickback_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Undercut", friendlyName="Undercut", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Undercut/SK_Undercut_01_Default_Male.SK_Undercut_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Undercut/SK_Undercut_01_Default_Female.SK_Undercut_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="DA_CompositeMeshData_NPC_Hairs_GalenSkelton", friendlyName="Unique - Galen", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/NPC/GalenSkelton/SK_Undercut_01_Default_GalenSkelton.SK_Undercut_01_Default_GalenSkelton", femalePath=nil, unisexPath=nil },
  { bodyPart="Hairs", name="DA_CompositeMeshData_NPC_Hairs_Ksante", friendlyName="Unique - John", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/NPC/Ksante/SK_Hair_Wick_02_Default_Ksante.SK_Hair_Wick_02_Default_Ksante", femalePath=nil, unisexPath=nil },
  { bodyPart="Hairs", name="Wavy_01", friendlyName="Wavy 1", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Wavy/SK_Hair_Wavy_01_Default_Male.SK_Hair_Wavy_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Wavy/SK_Hair_Wavy_01_Default_Female.SK_Hair_Wavy_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Wavy_02", friendlyName="Wavy 2", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Wavy/SK_Hair_Wavy_02_Default_Male.SK_Hair_Wavy_02_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Wavy/SK_Hair_Wavy_02_Default_Female.SK_Hair_Wavy_02_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Wavy_03", friendlyName="Wavy 3", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Wavy/SK_Hair_Wavy_03_Default_Male.SK_Hair_Wavy_03_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Wavy/SK_Hair_Wavy_03_Default_Female.SK_Hair_Wavy_03_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Wick_01", friendlyName="Wavy 4", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Wick/SK_Hair_Wick_01_Default_Male.SK_Hair_Wick_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Wick/SK_Hair_Wick_01_Default_Female.SK_Hair_Wick_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Wick_02", friendlyName="Wavy 5", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Wick/SK_Hair_Wick_02_Default_Male.SK_Hair_Wick_02_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Wick/SK_Hair_Wick_02_Default_Female.SK_Hair_Wick_02_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Wig_01", friendlyName="Wig 1", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Wig/SK_Hair_Wig_01_Default_Male.SK_Hair_Wig_01_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Wig/SK_Hair_Wig_01_Default_Female.SK_Hair_Wig_01_Default_Female", unisexPath=nil },
  { bodyPart="Hairs", name="Wig_02", friendlyName="Wig 2", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Hair/Male/Wig/SK_Hair_Wig_02_Default_Male.SK_Hair_Wig_02_Default_Male", femalePath="/Game/Character/Skeletal_Meshes/Hair/Female/Wig/SK_Hair_Wig_02_Default_Female.SK_Hair_Wig_02_Default_Female", unisexPath=nil },
  -- Eyebrows (2026-09-14 addition, RedFalcon: "use the eyebrows list in our current tree but
  -- instead of just '01' use 'Style 1' etc") -- same rows as Config.CUSTOM_FACIAL's own
  -- family="Eyebrows"/"BlackSmith" entries (see that table's own header for the sex-pairing
  -- notes: Female has 5 numbered variants, Male has 4, no Male 05 exists), just renamed to this
  -- table's friendlyName convention and re-sorted alphabetically ("Blacksmith" < "Style 1").
  { bodyPart="Eyebrows", name="BlackSmith", friendlyName="Blacksmith", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/BlackSmith/SK_Eyebrow_BlackSmith.SK_Eyebrow_BlackSmith", femalePath=nil, unisexPath=nil },
  { bodyPart="Eyebrows", name="01", friendlyName="Style 1", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Eyebrows/Meshes/SK_Eyebrows_Male_01.SK_Eyebrows_Male_01", femalePath="/Game/Character/Skeletal_Meshes/Facial/Female/Eyebrows/Meshes/SK_Eyebrows_Female_01.SK_Eyebrows_Female_01", unisexPath=nil },
  { bodyPart="Eyebrows", name="02", friendlyName="Style 2", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Eyebrows/Meshes/SK_Eyebrows_Male_02.SK_Eyebrows_Male_02", femalePath="/Game/Character/Skeletal_Meshes/Facial/Female/Eyebrows/Meshes/SK_Eyebrows_Female_02.SK_Eyebrows_Female_02", unisexPath=nil },
  { bodyPart="Eyebrows", name="03", friendlyName="Style 3", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Eyebrows/Meshes/SK_Eyebrows_Male_03.SK_Eyebrows_Male_03", femalePath="/Game/Character/Skeletal_Meshes/Facial/Female/Eyebrows/Meshes/SK_Eyebrows_Female_03.SK_Eyebrows_Female_03", unisexPath=nil },
  { bodyPart="Eyebrows", name="04", friendlyName="Style 4", availability="Both (distinct Male + Female art)", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Eyebrows/Meshes/SK_Eyebrows_Male_04.SK_Eyebrows_Male_04", femalePath="/Game/Character/Skeletal_Meshes/Facial/Female/Eyebrows/Meshes/SK_Eyebrows_Female_04.SK_Eyebrows_Female_04", unisexPath=nil },
  { bodyPart="Eyebrows", name="05", friendlyName="Style 5", availability="FEMALE ONLY", hatFriendly=nil, malePath=nil, femalePath="/Game/Character/Skeletal_Meshes/Facial/Female/Eyebrows/Meshes/SK_Eyebrows_Female_05.SK_Eyebrows_Female_05", unisexPath=nil },
  { bodyPart="Beard", name="Bristle_01", friendlyName="Bristle", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Bristle/SK_Beard_Bristle_01.SK_Beard_Bristle_01", femalePath=nil, unisexPath=nil },
  { bodyPart="Beard", name="HalfPonytail", friendlyName="Half Ponytail", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/HalfPonytail/SK_Beard_HalfPonytail_Adventurer_Male.SK_Beard_HalfPonytail_Adventurer_Male" },
  { bodyPart="Beard", name="Hungover", friendlyName="Hungover", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Hungover/SK_Beard_Hungover.SK_Beard_Hungover" },
  { bodyPart="Beard", name="Jag_01", friendlyName="Jag 1", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Jag/SK_Beard_Jag_01.SK_Beard_Jag_01", femalePath=nil, unisexPath=nil },
  { bodyPart="Beard", name="Jag_02", friendlyName="Jag 2", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Jag/SK_Beard_Jag_02.SK_Beard_Jag_02", femalePath=nil, unisexPath=nil },
  { bodyPart="Beard", name="Nordic", friendlyName="Nordic", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Nordic/SK_Beard_Nordic_Adventurer_Male.SK_Beard_Nordic_Adventurer_Male" },
  { bodyPart="Beard", name="RoyalMarine", friendlyName="Royal Marine", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/RoyalMarine/SK_Beard_RoyalMarine_Adventurer_Male.SK_Beard_RoyalMarine_Adventurer_Male" },
  { bodyPart="Beard", name="Shag_02", friendlyName="Shag 2", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Beard_Shag_02.SK_Beard_Shag_02", femalePath=nil, unisexPath=nil },
  { bodyPart="Beard", name="Shag_03", friendlyName="Shag 3", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Beard_Shag_03.SK_Beard_Shag_03", femalePath=nil, unisexPath=nil },
  { bodyPart="Beard", name="Shag_04", friendlyName="Shag 4", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Beard_Shag_04.SK_Beard_Shag_04", femalePath=nil, unisexPath=nil },
  { bodyPart="Beard", name="Shag_05", friendlyName="Shag 5", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Beard_Shag_05.SK_Beard_Shag_05", femalePath=nil, unisexPath=nil },
  { bodyPart="Beard", name="Sparse_01", friendlyName="Sparse 1", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Sparse/SK_Beard_Sparse_01.SK_Beard_Sparse_01", femalePath=nil, unisexPath=nil },
  { bodyPart="Beard", name="Sparse_02", friendlyName="Sparse 2", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Sparse/SK_Beard_Sparse_02.SK_Beard_Sparse_02", femalePath=nil, unisexPath=nil },
  { bodyPart="Beard", name="Sparse_03", friendlyName="Sparse 3", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Sparse/SK_Beard_Sparse_03.SK_Beard_Sparse_03", femalePath=nil, unisexPath=nil },
  { bodyPart="Beard", name="DA_CompositeMeshData_NPC_Beard_GalenSkelton", friendlyName="Unique - Galen", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/NPC/GalenSkelton/SK_Beard_Hungover_GalenSkelton.SK_Beard_Hungover_GalenSkelton" },
  { bodyPart="Beard", name="DA_CompositeMeshData_NPC_Beard_Jag_Ksante", friendlyName="Unique - John", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/NPC/Ksante/SK_Beard_Jag_01_Ksante.SK_Beard_Jag_01_Ksante" },
  { bodyPart="Mustache", name="Mustaches_Bristle_01", friendlyName="Bristle", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Bristle/SK_Beard_Bristle_01.SK_Beard_Bristle_01", femalePath=nil, unisexPath=nil },
  { bodyPart="Mustache", name="HalfPonytail", friendlyName="Half Ponytail", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/HalfPonytail/SK_Mustache_HalfPonytail_Adventurer_Male.SK_Mustache_HalfPonytail_Adventurer_Male" },
  { bodyPart="Mustache", name="Mustaches_Hungover", friendlyName="Hungover", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Hungover/SK_Mustaches_Hungover.SK_Mustaches_Hungover" },
  { bodyPart="Mustache", name="Jag_01", friendlyName="Jag 1", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Jag/SK_Mustache_Jag_01.SK_Mustache_Jag_01", femalePath=nil, unisexPath=nil },
  { bodyPart="Mustache", name="Jag_02", friendlyName="Jag 2", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Jag/SK_Mustache_Jag_02.SK_Mustache_Jag_02", femalePath=nil, unisexPath=nil },
  { bodyPart="Mustache", name="Nordic", friendlyName="Nordic", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Nordic/SK_Mustache_Nordic_Adventurer_Male.SK_Mustache_Nordic_Adventurer_Male" },
  { bodyPart="Mustache", name="RoyalMarine", friendlyName="Royal Marine", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/RoyalMarine/SK_Mustaches_RoyalMarine_Adventurer_Male.SK_Mustaches_RoyalMarine_Adventurer_Male" },
  { bodyPart="Mustache", name="Shag_02", friendlyName="Shag 2", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Mustache_Shag_02.SK_Mustache_Shag_02", femalePath=nil, unisexPath=nil },
  { bodyPart="Mustache", name="Shag_03", friendlyName="Shag 3", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Mustache_Shag_03.SK_Mustache_Shag_03", femalePath=nil, unisexPath=nil },
  { bodyPart="Mustache", name="Shag_04", friendlyName="Shag 4", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Mustache_Shag_04.SK_Mustache_Shag_04", femalePath=nil, unisexPath=nil },
  { bodyPart="Mustache", name="Shag_05", friendlyName="Shag 5", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Mustache_Shag_05.SK_Mustache_Shag_05", femalePath=nil, unisexPath=nil },
  { bodyPart="Mustache", name="Sparse_01", friendlyName="Sparse 1", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Sparse/SK_Mustache_Sparse_01.SK_Mustache_Sparse_01", femalePath=nil, unisexPath=nil },
  { bodyPart="Mustache", name="Sparse_02", friendlyName="Sparse 2", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Sparse/SK_Mustache_Sparse_02.SK_Mustache_Sparse_02", femalePath=nil, unisexPath=nil },
  { bodyPart="Mustache", name="Sparse_03", friendlyName="Sparse 3", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Sparse/SK_Mustache_Sparse_03.SK_Mustache_Sparse_03", femalePath=nil, unisexPath=nil },
  { bodyPart="Mustache", name="DA_CompositeMeshData_NPC_Mustaches_GalenSkelton", friendlyName="Unique - Galen", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/NPC/GalenSkelton/SK_Mustaches_Hungover_GalenSkelton.SK_Mustaches_Hungover_GalenSkelton" },
  { bodyPart="Mustache", name="DA_CompositeMeshData_NPC_Mustaches_Jag_Ksante", friendlyName="Unique - John", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/NPC/Ksante/SK_Mustache_Jag_01_Ksante.SK_Mustache_Jag_01_Ksante" },
  { bodyPart="Whiskers", name="Bristle_01", friendlyName="Bristle", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Bristle/SK_Beard_Bristle_01.SK_Beard_Bristle_01", femalePath=nil, unisexPath=nil },
  { bodyPart="Whiskers", name="HalfPonytail", friendlyName="Half Ponytail", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/HalfPonytail/SK_Whiskers_HalfPonytail_Adventurer_Male.SK_Whiskers_HalfPonytail_Adventurer_Male" },
  { bodyPart="Whiskers", name="Hungover", friendlyName="Hungover", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Hungover/SK_Whiskers_Hungover.SK_Whiskers_Hungover" },
  { bodyPart="Whiskers", name="Jag_01", friendlyName="Jag 1", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Jag/SK_Whiskers_Jag_01.SK_Whiskers_Jag_01", femalePath=nil, unisexPath=nil },
  { bodyPart="Whiskers", name="Jag_02", friendlyName="Jag 2", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Jag/SK_Whiskers_Jag_02.SK_Whiskers_Jag_02", femalePath=nil, unisexPath=nil },
  { bodyPart="Whiskers", name="Nordic", friendlyName="Nordic", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Nordic/SK_Whiskers_Nordic_Adventurer_Male.SK_Whiskers_Nordic_Adventurer_Male" },
  { bodyPart="Whiskers", name="RoyalMarine", friendlyName="Royal Marine", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/RoyalMarine/SK_Whiskers_RoyalMarine_Adventurer_Male.SK_Whiskers_RoyalMarine_Adventurer_Male" },
  { bodyPart="Whiskers", name="Shag_02", friendlyName="Shag 2", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Whiskers_Shag_02.SK_Whiskers_Shag_02", femalePath=nil, unisexPath=nil },
  { bodyPart="Whiskers", name="Shag_03", friendlyName="Shag 3", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Whiskers_Shag_03.SK_Whiskers_Shag_03", femalePath=nil, unisexPath=nil },
  { bodyPart="Whiskers", name="Shag_04", friendlyName="Shag 4", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Whiskers_Shag_04.SK_Whiskers_Shag_04", femalePath=nil, unisexPath=nil },
  { bodyPart="Whiskers", name="Shag_05", friendlyName="Shag 5", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Shag/SK_Whiskers_Shag_05.SK_Whiskers_Shag_05", femalePath=nil, unisexPath=nil },
  { bodyPart="Whiskers", name="Sparse_01", friendlyName="Sparse 1", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Sparse/SK_Whiskers_Sparse_01.SK_Whiskers_Sparse_01", femalePath=nil, unisexPath=nil },
  { bodyPart="Whiskers", name="Sparse_02", friendlyName="Sparse 2", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Sparse/SK_Whiskers_Sparse_02.SK_Whiskers_Sparse_02", femalePath=nil, unisexPath=nil },
  { bodyPart="Whiskers", name="Sparse_03", friendlyName="Sparse 3", availability="MALE ONLY", hatFriendly=nil, malePath="/Game/Character/Skeletal_Meshes/Facial/Male/Beard/Sparse/SK_Whiskers_Sparse_03.SK_Whiskers_Sparse_03", femalePath=nil, unisexPath=nil },
  { bodyPart="Whiskers", name="DA_CompositeMeshData_NPC_Whiskers_GalenSkelton", friendlyName="Unique - Galen", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/NPC/GalenSkelton/SK_Whiskers_Hungover_GalenSkelton.SK_Whiskers_Hungover_GalenSkelton" },
  { bodyPart="Whiskers", name="DA_CompositeMeshData_NPC_Whiskers_Jag_Ksante", friendlyName="Unique - John", availability="Both (single unisex mesh)", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath="/Game/Character/Skeletal_Meshes/Facial/NPC/Ksante/SK_Whiskers_Jag_01_Ksante.SK_Whiskers_Jag_01_Ksante" },
  { bodyPart="Sets", name=nil, friendlyName="Bristle", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Half Ponytail", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Hungover", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Jag 1", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Jag 2", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Nordic", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Royal Marine", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Shag 2", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Shag 3", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Shag 4", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Shag 5", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Sparse 1", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Sparse 2", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Unique - Galen", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
  { bodyPart="Sets", name=nil, friendlyName="Unique - John", availability="MALE", hatFriendly=nil, malePath=nil, femalePath=nil, unisexPath=nil },
}

-- Config.CLOTHES_ITEMS -- generated by gen_clothes_lua.py from Other\Hair_And_Clothes_Export.xlsx's
-- "Clothes Adjusted" sheet (RedFalcon's own validated per-body-type pass). One row per real,
-- pickable clothing item, keyed by bodyPart (Torso/Legs/Waist/Hands/Feets/Headgear/Cape -- the SAME
-- spelling as spawner.lua's own BODY_PART_ENUM_BY_NAME), sorted alphabetically by friendlyName
-- WITHIN each bodyPart -- this order IS the dropdown order, do not re-sort at runtime (same rule
-- Config.HAIR_CATEGORY_ITEMS' own header already established). `availability` is the raw sheet
-- string (e.g. "MALE ONLY", "Both (single unisex mesh) - Only with Unlock") -- a string containing
-- "Only with Unlock" gates this item behind Config.CLOTHES_UNLOCK_ALL (lbunlockclothes), same flag
-- as the existing women's-fit-rule bypass. Mesh resolution follows the EXACT same fallback chain
-- Spawner.ApplyHairCategoryMesh already uses for non-Hairs rows: `unisexMesh or (isFemale and
-- femaleMesh or maleMesh) or maleMesh or femaleMesh` -- this alone implements RedFalcon's "where
-- you see [a sex-flagged item] with a mesh only for one sex type, use it for both" rule, since a
-- row validated as BOTH-but-only-one-mesh-populated just falls through to that one mesh either way.
-- `sourceAsset` (2026-09-14, RedFalcon: "assigning headgear also needs to behave correctly with
-- the hair") is the item's own backing DA_Armor_..._CompositeMeshData path (the sheet's own
-- "Source asset" column) -- for Headgear specifically, Spawner.ApplyClothesItem reads its real
-- SlotsToSuspend property (the SAME proven mechanism lbdumpsuspend/DumpHeadgearSuspend already
-- use) to decide exactly how the hair should react, rather than guessing a category from the
-- mesh filename.
Config.CLOTHES_ITEMS = {
  Cape = {
    { name="Hero_Conquistador_Torso_01", friendlyName="Conquistador Cape 1", setName="Conquistador 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_03_Male_Cape.SK_Armor_Conquistador_03_Male_Cape", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_03_Female_Cape.SK_Armor_Conquistador_03_Female_Cape", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Conquistador/CompositeMeshData/Torso/DA_Armor_Regular_Hero_Conquistador_Torso_01_CompositeMeshData" },
    { name="Flibustier_Cape_02", friendlyName="Flibustier Cape 2", setName="Flibustier 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Male_Cape.SK_Armor_Flibustier_01_Male_Cape", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Female_Cape.SK_Armor_Flibustier_01_Female_Cape", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Cape/DA_Armor_Regular_Flibustier_Cape_02_CompositeMeshData" },
    { name="Jeweler_Cape_02", friendlyName="Jeweler Cape 2", setName="Jeweler 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Male_Cape_02.SK_Armor_Jeweler_01_Male_Cape_02", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Female_Cape_02.SK_Armor_Jeweler_01_Female_Cape_02", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Cape/DA_Armor_Regular_Jeweler_Cape_02_CompositeMeshData" },
    { name="Jeweler_Cape_03", friendlyName="Jeweler Cape 3", setName="Jeweler 3", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Male_Cape_03.SK_Armor_Jeweler_01_Male_Cape_03", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Female_Cape_03.SK_Armor_Jeweler_01_Female_Cape_03", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Cape/DA_Armor_Regular_Jeweler_Cape_03_CompositeMeshData" },
    { name="Jeweler_Cape_04", friendlyName="Jeweler Cape 4", setName="Jeweler 4", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Male_Cape.SK_Armor_Jeweler_02_Male_Cape", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Female_Cape.SK_Armor_Jeweler_02_Female_Cape", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Cape/DA_Armor_Regular_Jeweler_Cape_04_CompositeMeshData" },
    { name="Hero_Pikeman_Torso_01", friendlyName="Pikeman Torso 1", setName="Pikeman 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_01_Male_Cape.SK_Armor_Conquistador_01_Male_Cape", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_01_Female_Cape.SK_Armor_Conquistador_01_Female_Cape", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Pikeman/CompositeMeshData/Torso/DA_Armor_Regular_Hero_Pikeman_Torso_01_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Cape_01", friendlyName="Senkamati Shaman Feather Cape 1", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_01_TorsoCloth.SK_ArmorCreature_Senkamati_Witch_Feather_01_TorsoCloth", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Cape/DA_Armor_Regular_Senkamati_Shaman_Feather_Cape_01_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Cape_02", friendlyName="Senkamati Shaman Feather Cape 2", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_02_TorsoCloth.SK_ArmorCreature_Senkamati_Witch_Feather_02_TorsoCloth", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Cape/DA_Armor_Regular_Senkamati_Shaman_Feather_Cape_02_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Neck_02", friendlyName="Senkamati Shaman Feather Neck 2", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_02_Neck.SK_ArmorCreature_Senkamati_Witch_Feather_02_Neck", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Neck/DA_Armor_Regular_Senkamati_Shaman_Feather_Neck_02_CompositeMeshData" },
    { name="NPC_GalenSkelton_Cape_01", friendlyName="Unique - Galen Skelton Cape", setName=nil, availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/DrGalen/Meshes/SK_Armor_DrGalen_Cape.SK_Armor_DrGalen_Cape", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/NPC_GalenSkelton/CompositeMeshData/Torso/DA_Armor_Regular_NPC_GalenSkelton_Cape_01_CompositeMeshData" },
    { name="NPC_Ksante_Cape_01", friendlyName="Unique - John Cape", setName=nil, availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Ksante/Meshes/SK_Armor_Ksante_Cape_01.SK_Armor_Ksante_Cape_01", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/NPC_Ksante/CompositeMeshData/Torso/DA_Armor_Regular_NPC_Ksante_Cape_01_CompositeMeshData" },
  },
  Feets = {
    { name="Hero_Adventurer_Feet_01", friendlyName="Adventurer Feet 1", setName="Adventurer 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_Adventurer_Male_Feet_Long.SK_Armor_Flibustier_Adventurer_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_Adventurer_Female_Feet_Long.SK_Armor_Flibustier_Adventurer_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Adventurer/CompositeMeshData/Feet/DA_Armor_Regular_Hero_Adventurer_Feet_01_CompositeMeshData" },
    { name="Hero_Bandit_Feet_01", friendlyName="Bandit Feet 1", setName="Bandit 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Male_Feet_Long.SK_Armor_Bandit_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Female_Feet_Long.SK_Armor_Bandit_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Bandit/CompositeMeshData/Feet/DA_Armor_Regular_Hero_Bandit_Feet_01_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Feet_01", friendlyName="Blackbeard Grenadier Feet 1", setName="Blackbeard Grenadier 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_01_Feet_Long.SK_Armor_Blackbeard_Grenadier_01_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Grenadier_Feet_01_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Feet_02", friendlyName="Blackbeard Grenadier Feet 2", setName="Blackbeard Grenadier 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_02_Feet_Long.SK_Armor_Blackbeard_Grenadier_02_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Grenadier_Feet_02_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Feet_03", friendlyName="Blackbeard Grenadier Feet 3", setName="Blackbeard Grenadier 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_03_Feet_Long.SK_Armor_Blackbeard_Grenadier_03_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Grenadier_Feet_03_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Feet_01", friendlyName="Blackbeard Musketeer Feet 1", setName=nil, availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_01_Feet.SK_Armor_Blackbeard_Musketeer_01_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Musketeer_Feet_01_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Feet_01_Long", friendlyName="Blackbeard Musketeer Feet 1 - Long", setName="Blackbeard Musketeer 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer01_Feet_Long.SK_Armor_Musketeer01_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Musketeer_Feet_01_Long_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Feet_02", friendlyName="Blackbeard Musketeer Feet 2", setName=nil, availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_02_Feet.SK_Armor_Blackbeard_Musketeer_02_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Musketeer_Feet_02_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Feet_02_Long", friendlyName="Blackbeard Musketeer Feet 2 - Long", setName="Blackbeard Musketeer 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_02_Feet_Long.SK_Armor_Blackbeard_Musketeer_02_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Musketeer_Feet_02_Long_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Feet_03", friendlyName="Blackbeard Musketeer Feet 3", setName=nil, availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_03_Feet.SK_Armor_Blackbeard_Musketeer_03_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Musketeer_Feet_03_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Feet_03_Long", friendlyName="Blackbeard Musketeer Feet 3 - Long", setName="Blackbeard Musketeer 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_03_Feet_Long.SK_Armor_Blackbeard_Musketeer_03_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Musketeer_Feet_03_Long_CompositeMeshData" },
    { name="BlackBeard_Sailor_Feet_01", friendlyName="Blackbeard Sailor Feet 1", setName="Blackbeard Sailor 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_01_Feet.SK_Armor_Blackbeard_Sailor_01_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Sailor_Feet_01_CompositeMeshData" },
    { name="BlackBeard_Sailor_Feet_02", friendlyName="Blackbeard Sailor Feet 2", setName="Blackbeard Sailor 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_02_Feet.SK_Armor_Blackbeard_Sailor_02_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Sailor_Feet_02_CompositeMeshData" },
    { name="BlackBeard_Sailor_Feet_03", friendlyName="Blackbeard Sailor Feet 3", setName="Blackbeard Sailor 3", availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_03_Feet_Long.SK_Armor_Blackbeard_Sailor_03_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Sailor_Feet_03_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Feet_01", friendlyName="Blackbeard Sergeant Feet 1", setName="Blackbeard Sergeant 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_01_Feet_Long.SK_Armor_Blackbeard_Sergeant_01_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Sergeant_Feet_01_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Feet_02", friendlyName="Blackbeard Sergeant Feet 2", setName="Blackbeard Sergeant 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_02_Feet_Long.SK_Armor_Blackbeard_Sergeant_02_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Sergeant_Feet_02_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Feet_03", friendlyName="Blackbeard Sergeant Feet 3", setName="Blackbeard Sergeant 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_03_Feet_Long.SK_Armor_Blackbeard_Sergeant_03_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Feets/DA_Armor_Regular_BlackBeard_Sergeant_Feet_03_CompositeMeshData" },
    { name="BlackSmith_Feet", friendlyName="Blacksmith Feet", setName="Blacksmith", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blacksmith/Meshes/SK_Armor_BlackSmith_Feet.SK_Armor_BlackSmith_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset=nil },
    { name="Hero_Brigant_Feet_01", friendlyName="Brigant Feet 1", setName="Brigant 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Male_Feet_Long.SK_Armor_Brigant_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Female_Feet_Long.SK_Armor_Brigant_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Brigant/CompositeMeshData/Feet/DA_Armor_Regular_Hero_Brigant_Feet_01_CompositeMeshData" },
    { name="Combatant_Feet_01", friendlyName="Combatant Feet 1", setName="Combatant 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_01_Feet.SK_Armor_Combatant_01_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Feets/DA_Armor_Regular_Combatant_Feet_01_CompositeMeshData" },
    { name="Combatant_Feet_02", friendlyName="Combatant Feet 2", setName="Combatant 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_02_Feet.SK_Armor_Combatant_02_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Feets/DA_Armor_Regular_Combatant_Feet_02_CompositeMeshData" },
    { name="Combatant_Feet_03", friendlyName="Combatant Feet 3", setName="Combatant 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_03_Feet.SK_Armor_Combatant_03_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Feets/DA_Armor_Regular_Combatant_Feet_03_CompositeMeshData" },
    { name="Hero_Conquistador_Feet_01", friendlyName="Conquistador Feet 1", setName="Conquistador 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_02_Male_Feet_Long.SK_Armor_Conquistador_02_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_02_Female_Feet_Long.SK_Armor_Conquistador_02_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Conquistador/CompositeMeshData/Feet/DA_Armor_Regular_Hero_Conquistador_Feet_01_CompositeMeshData" },
    { name="Dogface_Feet_01", friendlyName="Dogface Feet 1", setName="Dogface 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_01_Male_Feet.SK_Armor_Dogface_01_Male_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Feets/DA_Armor_Regular_Dogface_Feet_01_CompositeMeshData" },
    { name="Dogface_Feet_02", friendlyName="Dogface Feet 2", setName="Dogface 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_02_Male_Feet.SK_Armor_Dogface_02_Male_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Feets/DA_Armor_Regular_Dogface_Feet_02_CompositeMeshData" },
    { name="Dogface_Feet_03", friendlyName="Dogface Feet 3", setName="Dogface 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_03_Male_Feet.SK_Armor_Dogface_03_Male_Feet", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Feets/DA_Armor_Regular_Dogface_Feet_03_CompositeMeshData" },
    { name="Drowned_Armored_Feet_01", friendlyName="Drowned Armored Feet 1", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned_Armored/Meshes/SK_ArmorCreature_Drowned_Armored_01_Feet_Long.SK_ArmorCreature_Drowned_Armored_01_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned_Armored/CompositeMeshData/Feets/DA_Armor_Regular_Drowned_Armored_Feet_01_CompositeMeshData" },
    { name="Drowned_Armored_Feet_02", friendlyName="Drowned Armored Feet 2", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned_Armored/Meshes/SK_ArmorCreature_Drowned_Armored_02_Feet_Long.SK_ArmorCreature_Drowned_Armored_02_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned_Armored/CompositeMeshData/Feets/DA_Armor_Regular_Drowned_Armored_Feet_02_CompositeMeshData" },
    { name="Drowned_Armored_Feet_03", friendlyName="Drowned Armored Feet 3", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned_Armored/Meshes/SK_ArmorCreature_Drowned_Armored_03_Feet_Long.SK_ArmorCreature_Drowned_Armored_03_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned_Armored/CompositeMeshData/Feets/DA_Armor_Regular_Drowned_Armored_Feet_03_CompositeMeshData" },
    { name="Drowned_Feet_01", friendlyName="Drowned Feet 1", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned/Meshes/SK_ArmorCreature_Drowned_01_Feets_Long.SK_ArmorCreature_Drowned_01_Feets_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned/CompositeMeshData/Feets/DA_Armor_Regular_Drowned_Feet_01_CompositeMeshData" },
    { name="Drowned_Feet_02", friendlyName="Drowned Feet 2", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned/Meshes/SK_ArmorCreature_Drowned_02_Feets.SK_ArmorCreature_Drowned_02_Feets", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned/CompositeMeshData/Feets/DA_Armor_Regular_Drowned_Feet_02_CompositeMeshData" },
    { name="Drowned_Feet_03", friendlyName="Drowned Feet 3", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned/Meshes/SK_ArmorCreature_Drowned_03_Feets_Long.SK_ArmorCreature_Drowned_03_Feets_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned/CompositeMeshData/Feets/DA_Armor_Regular_Drowned_Feet_03_CompositeMeshData" },
    { name="Flibustier_Feet_01", friendlyName="Flibustier Feet 1", setName="Flibustier 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Male_Feet_Long.SK_Armor_Flibustier_01_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Female_Feet_Long.SK_Armor_Flibustier_01_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Feet/DA_Armor_Regular_Flibustier_Feet_01_CompositeMeshData" },
    { name="Hero_Flibustier_Feet_01", friendlyName="Flibustier Feet 1 (2)", setName="Flibustier 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Male_Feet_Long.SK_Armor_Flibustier_02_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Female_Feet_Long.SK_Armor_Flibustier_02_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Flibustier/CompositeMeshData/Feet/DA_Armor_Regular_Hero_Flibustier_Feet_01_CompositeMeshData" },
    { name="Flibustier_Feet_02", friendlyName="Flibustier Feet 2", setName="Flibustier 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Male_Feet_Long.SK_Armor_Flibustier_02_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Female_Feet_Long.SK_Armor_Flibustier_02_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Feet/DA_Armor_Regular_Flibustier_Feet_02_CompositeMeshData" },
    { name="DA_Mob_Boneman_Spanish_Ghost_CompositeMeshData_Feet_01", friendlyName="Ghostly Boots", setName="Ghostly", availability="BOTH", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Ghost/Meshes/SK_ArmorCreature_Boneman_Spanish_Ghost_01_Feets.SK_ArmorCreature_Boneman_Spanish_Ghost_01_Feets", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Ghost/CompositeMeshData/Torso/DA_Mob_Boneman_Spanish_Ghost_CompositeMeshData_Feet_01" },
    { name="Jeweler_Feet_01", friendlyName="Jeweler Feet 1", setName="Jeweler 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Male_Feet_Long.SK_Armor_Jeweler_01_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Female_Feet_Long.SK_Armor_Jeweler_01_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Feet/DA_Armor_Regular_Jeweler_Feet_01_CompositeMeshData" },
    { name="Jeweler_Feet_02", friendlyName="Jeweler Feet 2", setName="Jeweler 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Male_Feet_Long.SK_Armor_Jeweler_02_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Female_Feet_Long.SK_Armor_Jeweler_02_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Feet/DA_Armor_Regular_Jeweler_Feet_02_CompositeMeshData" },
    { name="Jeweler_Feet_03", friendlyName="Jeweler Feet 3", setName="Jeweler 3", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Male_Feet_Long.SK_Armor_Jeweler_03_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Female_Feet_Long.SK_Armor_Jeweler_03_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Feet/DA_Armor_Regular_Jeweler_Feet_03_CompositeMeshData" },
    { name="Hero_Mercenary_Feet_01", friendlyName="Mercenary Feet 1", setName="Mercenary 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Male_Feet_Long.SK_Armor_Mercenary_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Female_Feet_Long.SK_Armor_Mercenary_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Mercenary/CompositeMeshData/Feet/DA_Armor_Regular_Hero_Mercenary_Feet_01_CompositeMeshData" },
    { name="Musketeer_Feet_01", friendlyName="Musketeer Feet 1", setName="Musketeer 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer01_Feet_Long.SK_Armor_Musketeer01_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Feets/DA_Armor_Regular_Musketeer_Feet_01_CompositeMeshData" },
    { name="Musketeer_Feet_02", friendlyName="Musketeer Feet 2", setName="Musketeer 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer02_Feet_Long.SK_Armor_Musketeer02_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Feets/DA_Armor_Regular_Musketeer_Feet_02_CompositeMeshData" },
    { name="Musketeer_Feet_03", friendlyName="Musketeer Feet 3", setName="Musketeer 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer03_Feet_Long.SK_Armor_Musketeer03_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Feets/DA_Armor_Regular_Musketeer_Feet_03_CompositeMeshData" },
    { name="Hero_Pikeman_Feet_01", friendlyName="Pikeman Feet 1", setName="Pikeman 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_01_Male_Feet_Long.SK_Armor_Conquistador_01_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_01_Female_Feet_Long.SK_Armor_Conquistador_01_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Pikeman/CompositeMeshData/Feet/DA_Armor_Regular_Hero_Pikeman_Feet_01_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Feet_01", friendlyName="Senkamati Hunter Feather Feet 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_01_Feet_Long.SK_ArmorCreature_Senkamati_Hunter_Feather_01_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Hunter_Feather_Feet_01_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Feet_02", friendlyName="Senkamati Hunter Feather Feet 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_02_Feet_Long.SK_ArmorCreature_Senkamati_Hunter_Feather_02_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Hunter_Feather_Feet_02_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Feet_03", friendlyName="Senkamati Hunter Feather Feet 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_03_Feet_Long.SK_ArmorCreature_Senkamati_Hunter_Feather_03_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Hunter_Feather_Feet_03_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Feet_01", friendlyName="Senkamati Shaman Feather Feet 1", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_01_Feet.SK_ArmorCreature_Senkamati_Witch_Feather_01_Feet", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Shaman_Feather_Feet_01_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Feet_02", friendlyName="Senkamati Shaman Feather Feet 2", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_02_Feet.SK_ArmorCreature_Senkamati_Witch_Feather_02_Feet", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Shaman_Feather_Feet_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Feet_01", friendlyName="Senkamati Thrall Feather Feet 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_01_Feet_Long.SK_ArmorCreature_Senkamati_Thrall_Feather_01_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Thrall_Feather_Feet_01_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Feet_02", friendlyName="Senkamati Thrall Feather Feet 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_02_Feet.SK_ArmorCreature_Senkamati_Thrall_Feather_02_Feet", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Thrall_Feather_Feet_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Feet_03", friendlyName="Senkamati Thrall Feather Feet 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_03_Feet_Long.SK_ArmorCreature_Senkamati_Thrall_Feather_03_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Thrall_Feather_Feet_03_CompositeMeshData" },
    { name="Senkamati_Thrall_Wood_Feet_01", friendlyName="Senkamati Thrall Wood Feet 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Wood_01_Feet_Long.SK_ArmorCreature_Senkamati_Thrall_Wood_01_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Wood/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Thrall_Wood_Feet_01_CompositeMeshData" },
    { name="Senkamati_Thrall_Wood_Feet_02", friendlyName="Senkamati Thrall Wood Feet 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Wood_02_Feet_Long.SK_ArmorCreature_Senkamati_Thrall_Wood_02_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Wood/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Thrall_Wood_Feet_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Wood_Feet_03", friendlyName="Senkamati Thrall Wood Feet 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Wood_03_Feet_Long.SK_ArmorCreature_Senkamati_Thrall_Wood_03_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Wood/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Thrall_Wood_Feet_03_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Feet_01", friendlyName="Senkamati Warrior Feather Feet 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_01_Feet_Long.SK_ArmorCreature_Senkamati_Warrior_Feather_01_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Warrior_Feather_Feet_01_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Feet_02", friendlyName="Senkamati Warrior Feather Feet 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_02_Feet_Long.SK_ArmorCreature_Senkamati_Warrior_Feather_02_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Warrior_Feather_Feet_02_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Feet_03", friendlyName="Senkamati Warrior Feather Feet 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_03_Feet_Long.SK_ArmorCreature_Senkamati_Warrior_Feather_03_Feet_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Feets/DA_Armor_Regular_Senkamati_Warrior_Feather_Feet_03_CompositeMeshData" },
    { name="Hero_Starter_Feet_01", friendlyName="Starter Feet 1", setName="Starter 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Male_Feet_Long.SK_Armor_Starter_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Female_Feet_Long.SK_Armor_Starter_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Starter/CompositeMeshData/Feet/DA_Armor_Regular_Hero_Starter_Feet_01_CompositeMeshData" },
    { name="Hero_Starter_Feet_02", friendlyName="Starter Feet 2", setName="Starter 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Restored/Meshes/SK_Armor_Restored_Male_Feet_Long.SK_Armor_Restored_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Restored/Meshes/SK_Armor_Restored_Female_Feet_Long.SK_Armor_Restored_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Starter/CompositeMeshData/Feet/DA_Armor_Regular_Hero_Starter_Feet_02_CompositeMeshData" },
    { name="NPC_GalenSkelton_Feet", friendlyName="Unique - Galen Skelton Feet", setName=nil, availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/DrGalen/Meshes/SK_Armor_DrGalen_Feet_Long.SK_Armor_DrGalen_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/NPC_GalenSkelton/CompositeMeshData/Feets/DA_Armor_Regular_NPC_GalenSkelton_Feet_CompositeMeshData" },
    { name="NPC_Ksante_Feet", friendlyName="Unique - John Feet", setName=nil, availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Ksante/Meshes/SK_Armor_Ksante_Feet_Long.SK_Armor_Ksante_Feet_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/NPC_Ksante/CompositeMeshData/Feets/DA_Armor_Regular_NPC_Ksante_Feet_CompositeMeshData" },
    { name="Hero_Vanilla_Feet_01", friendlyName="Vanilla Feet 1", setName="Vanilla 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Male_Feet_Long.SK_Armor_Vanilla_Male_Feet_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Female_Feet_Long.SK_Armor_Vanilla_Female_Feet_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Vanilla/CompositeMeshData/Feet/DA_Armor_Regular_Hero_Vanilla_Feet_01_CompositeMeshData" },
  },
  Hands = {
    { name="Hero_Adventurer_Hands_01", friendlyName="Adventurer Hands 1", setName="Adventurer 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_Adventurer_Male_Hands_Long.SK_Armor_Flibustier_Adventurer_Male_Hands_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_Adventurer_Female_Hands_Long.SK_Armor_Flibustier_Adventurer_Female_Hands_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Adventurer/CompositeMeshData/Hands/DA_Armor_Regular_Hero_Adventurer_Hands_01_CompositeMeshData" },
    { name="Hero_Bandit_Hands_01", friendlyName="Bandit Hands 1", setName="Bandit 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Male_Hands_Long.SK_Armor_Bandit_Male_Hands_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Female_Hands_Long.SK_Armor_Bandit_Female_Hands_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Bandit/CompositeMeshData/Hands/DA_Armor_Regular_Hero_Bandit_Hands_01_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Hands_01", friendlyName="Blackbeard Grenadier Hands 1", setName="Blackbeard Grenadier 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_01_Hands_Long.SK_Armor_Blackbeard_Grenadier_01_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Grenadier_Hands_01_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Hands_02", friendlyName="Blackbeard Grenadier Hands 2", setName="Blackbeard Grenadier 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_02_Hands_Long.SK_Armor_Blackbeard_Grenadier_02_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Grenadier_Hands_02_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Hands_03", friendlyName="Blackbeard Grenadier Hands 3", setName="Blackbeard Grenadier 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_03_Hands_Long.SK_Armor_Blackbeard_Grenadier_03_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Grenadier_Hands_03_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Hands_01", friendlyName="Blackbeard Musketeer Hands 1", setName="Blackbeard Musketeer 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_01_Hands_Long.SK_Armor_Blackbeard_Musketeer_01_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Musketeer_Hands_01_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Hands_02", friendlyName="Blackbeard Musketeer Hands 2", setName="Blackbeard Musketeer 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_02_Hands_Long.SK_Armor_Blackbeard_Musketeer_02_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Musketeer_Hands_02_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Hands_03", friendlyName="Blackbeard Musketeer Hands 3", setName="Blackbeard Musketeer 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_03_Hands.SK_Armor_Blackbeard_Musketeer_03_Hands", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Musketeer_Hands_03_CompositeMeshData" },
    { name="BlackBeard_Sailor_Hands_01", friendlyName="Blackbeard Sailor Hands 1", setName="Blackbeard Sailor 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_01_Hands_Long.SK_Armor_Blackbeard_Sailor_01_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Sailor_Hands_01_CompositeMeshData" },
    { name="BlackBeard_Sailor_Hands_02", friendlyName="Blackbeard Sailor Hands 2", setName="Blackbeard Sailor 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_02_Hands_Long.SK_Armor_Blackbeard_Sailor_02_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Sailor_Hands_02_CompositeMeshData" },
    { name="BlackBeard_Sailor_Hands_03", friendlyName="Blackbeard Sailor Hands 3", setName="Blackbeard Sailor 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_03_Hands_Long.SK_Armor_Blackbeard_Sailor_03_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Sailor_Hands_03_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Hands_01", friendlyName="Blackbeard Sergeant Hands 1", setName="Blackbeard Sergeant 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_01_Hands_Long.SK_Armor_Blackbeard_Sergeant_01_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Sergeant_Hands_01_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Hands_02", friendlyName="Blackbeard Sergeant Hands 2", setName="Blackbeard Sergeant 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_02_Hands_Long.SK_Armor_Blackbeard_Sergeant_02_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Sergeant_Hands_02_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Hands_03", friendlyName="Blackbeard Sergeant Hands 3", setName="Blackbeard Sergeant 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_03_Hands_Long.SK_Armor_Blackbeard_Sergeant_03_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Hands/DA_Armor_Regular_BlackBeard_Sergeant_Hands_03_CompositeMeshData" },
    { name="BlackSmith_Hands", friendlyName="Blacksmith Hands", setName="Blacksmith", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blacksmith/Meshes/SK_Armor_BlackSmith_Hands.SK_Armor_BlackSmith_Hands", femaleMesh=nil, unisexMesh=nil, sourceAsset=nil },
    { name="Hero_Brigant_Hands_01", friendlyName="Brigant Hands 1", setName="Brigant 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Male_Hands_Long.SK_Armor_Brigant_Male_Hands_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Female_Hands_Long.SK_Armor_Brigant_Female_Hands_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Brigant/CompositeMeshData/Hands/DA_Armor_Regular_Hero_Brigant_Hands_01_CompositeMeshData" },
    { name="Combatant_Hands_01", friendlyName="Combatant Hands 1", setName="Combatant 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_01_Hands_Long.SK_Armor_Combatant_01_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Hands/DA_Armor_Regular_Combatant_Hands_01_CompositeMeshData" },
    { name="Combatant_Hands_02", friendlyName="Combatant Hands 2", setName="Combatant 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_02_Hands.SK_Armor_Combatant_02_Hands", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Hands/DA_Armor_Regular_Combatant_Hands_02_CompositeMeshData" },
    { name="Combatant_Hands_03", friendlyName="Combatant Hands 3", setName="Combatant 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_03_Hands.SK_Armor_Combatant_03_Hands", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Hands/DA_Armor_Regular_Combatant_Hands_03_CompositeMeshData" },
    { name="Hero_Conquistador_Hands_01", friendlyName="Conquistador Hands 1", setName="Conquistador 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_02_Male_Hands.SK_Armor_Conquistador_02_Male_Hands", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_02_Female_Hands.SK_Armor_Conquistador_02_Female_Hands", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Conquistador/CompositeMeshData/Hands/DA_Armor_Regular_Hero_Conquistador_Hands_01_CompositeMeshData" },
    { name="Drowned_Armored_Hands_01", friendlyName="Drowned Armored Hands 1", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned_Armored/Meshes/SK_ArmorCreature_Drowned_Armored_01_Hands_Long.SK_ArmorCreature_Drowned_Armored_01_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned_Armored/CompositeMeshData/Hands/DA_Armor_Regular_Drowned_Armored_Hands_01_CompositeMeshData" },
    { name="Drowned_Armored_Hands_02", friendlyName="Drowned Armored Hands 2", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned_Armored/Meshes/SK_ArmorCreature_Drowned_Armored_02_Hands_Long.SK_ArmorCreature_Drowned_Armored_02_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned_Armored/CompositeMeshData/Hands/DA_Armor_Regular_Drowned_Armored_Hands_02_CompositeMeshData" },
    { name="Drowned_Armored_Hands_03", friendlyName="Drowned Armored Hands 3", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned_Armored/Meshes/SK_ArmorCreature_Drowned_Armored_03_Hands_Long.SK_ArmorCreature_Drowned_Armored_03_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned_Armored/CompositeMeshData/Hands/DA_Armor_Regular_Drowned_Armored_Hands_03_CompositeMeshData" },
    { name="Drowned_Hands_01", friendlyName="Drowned Hands 1", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned/Meshes/SK_ArmorCreature_Drowned_01_Hands_Long.SK_ArmorCreature_Drowned_01_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned/CompositeMeshData/Hands/DA_Armor_Regular_Drowned_Hands_01_CompositeMeshData" },
    { name="Drowned_Hands_02", friendlyName="Drowned Hands 2", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned/Meshes/SK_ArmorCreature_Drowned_02_Hands.SK_ArmorCreature_Drowned_02_Hands", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned/CompositeMeshData/Hands/DA_Armor_Regular_Drowned_Hands_02_CompositeMeshData" },
    { name="Drowned_Hands_03", friendlyName="Drowned Hands 3", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned/Meshes/SK_ArmorCreature_Drowned_03_Hands_Long.SK_ArmorCreature_Drowned_03_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned/CompositeMeshData/Hands/DA_Armor_Regular_Drowned_Hands_03_CompositeMeshData" },
    { name="Flibustier_Hands_01", friendlyName="Flibustier Hands 1", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Male_Hands.SK_Armor_Flibustier_01_Male_Hands", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Female_Hands.SK_Armor_Flibustier_01_Female_Hands", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Hands/DA_Armor_Regular_Flibustier_Hands_01_CompositeMeshData" },
    { name="Flibustier_Hands_01_Long", friendlyName="Flibustier Hands 1 - Long", setName="Flibustier 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Male_Hands_Long.SK_Armor_Flibustier_01_Male_Hands_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Female_Hands_Long.SK_Armor_Flibustier_01_Female_Hands_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Hands/DA_Armor_Regular_Flibustier_Hands_01_Long_CompositeMeshData" },
    { name="Flibustier_Hands_02", friendlyName="Flibustier Hands 2", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Male_Hands.SK_Armor_Flibustier_02_Male_Hands", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Female_Hands.SK_Armor_Flibustier_02_Female_Hands", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Hands/DA_Armor_Regular_Flibustier_Hands_02_CompositeMeshData" },
    { name="Flibustier_Hands_02_Long", friendlyName="Flibustier Hands 2 - Long", setName="Flibustier 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Male_Hands_Long.SK_Armor_Flibustier_02_Male_Hands_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Female_Hands_Long.SK_Armor_Flibustier_02_Female_Hands_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Hands/DA_Armor_Regular_Flibustier_Hands_02_Long_CompositeMeshData" },
    { name="DA_Mob_Boneman_Spanish_Ghost_CompositeMeshData_Hands_01", friendlyName="Ghostly Gloves", setName="Ghostly", availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Ghost/Meshes/SK_ArmorCreature_Boneman_Spanish_Ghost_02_Hands.SK_ArmorCreature_Boneman_Spanish_Ghost_02_Hands", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Ghost/CompositeMeshData/Torso/DA_Mob_Boneman_Spanish_Ghost_CompositeMeshData_Hands_01" },
    { name="Jeweler_Hands_01", friendlyName="Jeweler Hands 1", setName="Jeweler 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Male_Hands.SK_Armor_Jeweler_01_Male_Hands", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Female_Hands.SK_Armor_Jeweler_01_Female_Hands", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Hands/DA_Armor_Regular_Jeweler_Hands_01_CompositeMeshData" },
    { name="Jeweler_Hands_02", friendlyName="Jeweler Hands 2", setName="Jeweler 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Male_Hands.SK_Armor_Jeweler_02_Male_Hands", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Female_Hands.SK_Armor_Jeweler_02_Female_Hands", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Hands/DA_Armor_Regular_Jeweler_Hands_02_CompositeMeshData" },
    { name="Jeweler_Hands_03", friendlyName="Jeweler Hands 3", setName="Jeweler 3", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Male_Hands.SK_Armor_Jeweler_03_Male_Hands", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Female_Hands.SK_Armor_Jeweler_03_Female_Hands", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Hands/DA_Armor_Regular_Jeweler_Hands_03_CompositeMeshData" },
    { name="Hero_Mercenary_Hands_01", friendlyName="Mercenary Hands 1", setName="Mercenary 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Male_Hands.SK_Armor_Mercenary_Male_Hands", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Female_Hands.SK_Armor_Mercenary_Female_Hands", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Mercenary/CompositeMeshData/Hands/DA_Armor_Regular_Hero_Mercenary_Hands_01_CompositeMeshData" },
    { name="Musketeer_Hands_01", friendlyName="Musketeer Hands 1", setName="Musketeer 1", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer01_Hands_Long.SK_Armor_Musketeer01_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Hands/DA_Armor_Regular_Musketeer_Hands_01_CompositeMeshData" },
    { name="Musketeer_Hands_02", friendlyName="Musketeer Hands 2", setName="Musketeer 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer02_Hands_Long.SK_Armor_Musketeer02_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Hands/DA_Armor_Regular_Musketeer_Hands_02_CompositeMeshData" },
    { name="Musketeer_Hands_03", friendlyName="Musketeer Hands 3", setName="Musketeer 3", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer03_Hands_Long.SK_Armor_Musketeer03_Hands_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Hands/DA_Armor_Regular_Musketeer_Hands_03_CompositeMeshData" },
    { name="Hero_Pikeman_Hands_01", friendlyName="Pikeman Hands 1", setName="Pikeman 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_01_Male_Hands_Long.SK_Armor_Conquistador_01_Male_Hands_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_01_Female_Hands_Long.SK_Armor_Conquistador_01_Female_Hands_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Pikeman/CompositeMeshData/Hands/DA_Armor_Regular_Hero_Pikeman_Hands_01_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Hands_01", friendlyName="Senkamati Hunter Feather Hands 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_01_Hands_Long.SK_ArmorCreature_Senkamati_Hunter_Feather_01_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Hunter_Feather_Hands_01_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Hands_02", friendlyName="Senkamati Hunter Feather Hands 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_02_Hands_Long.SK_ArmorCreature_Senkamati_Hunter_Feather_02_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Hunter_Feather_Hands_02_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Hands_03", friendlyName="Senkamati Hunter Feather Hands 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_03_Hands_Long.SK_ArmorCreature_Senkamati_Hunter_Feather_03_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Hunter_Feather_Hands_03_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Hands_01", friendlyName="Senkamati Shaman Feather Hands 1", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_01_Hands.SK_ArmorCreature_Senkamati_Witch_Feather_01_Hands", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Shaman_Feather_Hands_01_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Hands_02", friendlyName="Senkamati Shaman Feather Hands 2", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_02_Hands.SK_ArmorCreature_Senkamati_Witch_Feather_02_Hands", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Shaman_Feather_Hands_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Hands_01", friendlyName="Senkamati Thrall Feather Hands 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_01_Hands_Long.SK_ArmorCreature_Senkamati_Thrall_Feather_01_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Thrall_Feather_Hands_01_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Hands_02", friendlyName="Senkamati Thrall Feather Hands 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_02_Hands_Long.SK_ArmorCreature_Senkamati_Thrall_Feather_02_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Thrall_Feather_Hands_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Hands_03", friendlyName="Senkamati Thrall Feather Hands 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_03_Hands_Long.SK_ArmorCreature_Senkamati_Thrall_Feather_03_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Thrall_Feather_Hands_03_CompositeMeshData" },
    { name="Senkamati_Thrall_Wood_Hands_01", friendlyName="Senkamati Thrall Wood Hands 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Wood_01_Hands_Long.SK_ArmorCreature_Senkamati_Thrall_Wood_01_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Wood/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Thrall_Wood_Hands_01_CompositeMeshData" },
    { name="Senkamati_Thrall_Wood_Hands_02", friendlyName="Senkamati Thrall Wood Hands 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Wood_02_Hands_Long.SK_ArmorCreature_Senkamati_Thrall_Wood_02_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Wood/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Thrall_Wood_Hands_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Wood_Hands_03", friendlyName="Senkamati Thrall Wood Hands 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Wood_03_Hands_Long.SK_ArmorCreature_Senkamati_Thrall_Wood_03_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Wood/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Thrall_Wood_Hands_03_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Hands_01", friendlyName="Senkamati Warrior Feather Hands 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_01_Hands_Long.SK_ArmorCreature_Senkamati_Warrior_Feather_01_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Warrior_Feather_Hands_01_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Hands_02", friendlyName="Senkamati Warrior Feather Hands 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_02_Hands_Long.SK_ArmorCreature_Senkamati_Warrior_Feather_02_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Warrior_Feather_Hands_02_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Hands_03", friendlyName="Senkamati Warrior Feather Hands 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_03_Hands_Long.SK_ArmorCreature_Senkamati_Warrior_Feather_03_Hands_Long", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Hands/DA_Armor_Regular_Senkamati_Warrior_Feather_Hands_03_CompositeMeshData" },
    { name="Hero_Starter_Hands_01", friendlyName="Starter Hands 1", setName="Starter 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Male_Hands_Long.SK_Armor_Starter_Male_Hands_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Female_Hands_Long.SK_Armor_Starter_Female_Hands_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Starter/CompositeMeshData/Hands/DA_Armor_Regular_Hero_Starter_Hands_01_CompositeMeshData" },
    { name="NPC_Ksante_Hands", friendlyName="Unique - John Hands", setName=nil, availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Ksante/Meshes/SK_Armor_Ksante_Hands.SK_Armor_Ksante_Hands", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/NPC_Ksante/CompositeMeshData/Hands/DA_Armor_Regular_NPC_Ksante_Hands_CompositeMeshData" },
    { name="Hero_Vanilla_Hands_01", friendlyName="Vanilla Hands 1", setName="Vanilla 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Male_Hands_Long.SK_Armor_Vanilla_Male_Hands_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Female_Hands_Long.SK_Armor_Vanilla_Female_Hands_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Vanilla/CompositeMeshData/Hands/DA_Armor_Regular_Hero_Vanilla_Hands_01_CompositeMeshData" },
  },
  Headgear = {
    { name="Hero_Adventurer_Head_01", friendlyName="Adventurer Head 1", setName="Adventurer 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Male_Bandana.SK_Armor_Mercenary_Male_Bandana", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Female_Bandana.SK_Armor_Mercenary_Female_Bandana", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Adventurer/CompositeMeshData/Head/DA_Armor_Regular_Hero_Adventurer_Head_01_CompositeMeshData" },
    { name="Hero_Bandit_Head_01", friendlyName="Bandit Head 1", setName="Bandit 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Male_Hat.SK_Armor_Bandit_Male_Hat", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Female_Hat.SK_Armor_Bandit_Female_Hat", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Bandit/CompositeMeshData/Head/DA_Armor_Regular_Hero_Bandit_Head_01_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Head_01", friendlyName="Blackbeard Grenadier Head 1", setName="Blackbeard Grenadier 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_01_Bandana.SK_Armor_Blackbeard_Grenadier_01_Bandana", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Head/DA_Armor_Regular_BlackBeard_Grenadier_Head_01_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Head_02", friendlyName="Blackbeard Grenadier Head 2", setName="Blackbeard Grenadier 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_02_Bandana.SK_Armor_Blackbeard_Grenadier_02_Bandana", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Head/DA_Armor_Regular_BlackBeard_Grenadier_Head_02_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Head_03", friendlyName="Blackbeard Grenadier Head 3", setName="Blackbeard Grenadier 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_03_Bandana.SK_Armor_Blackbeard_Grenadier_03_Bandana", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Head/DA_Armor_Regular_BlackBeard_Grenadier_Head_03_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Head_01", friendlyName="Blackbeard Musketeer Head 1", setName="Blackbeard Musketeer 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_01_Hat.SK_Armor_Blackbeard_Musketeer_01_Hat", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Head/DA_Armor_Regular_BlackBeard_Musketeer_Head_01_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Head_02", friendlyName="Blackbeard Musketeer Head 2", setName="Blackbeard Musketeer 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_02_Hat.SK_Armor_Blackbeard_Musketeer_02_Hat", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Head/DA_Armor_Regular_BlackBeard_Musketeer_Head_02_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Head_03", friendlyName="Blackbeard Musketeer Head 3", setName="Blackbeard Musketeer 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_03_Hat.SK_Armor_Blackbeard_Musketeer_03_Hat", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Head/DA_Armor_Regular_BlackBeard_Musketeer_Head_03_CompositeMeshData" },
    { name="BlackBeard_Sailor_Head_01", friendlyName="Blackbeard Sailor Head 1", setName="Blackbeard Sailor 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_01_Bandana.SK_Armor_Blackbeard_Sailor_01_Bandana", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Head/DA_Armor_Regular_BlackBeard_Sailor_Head_01_CompositeMeshData" },
    { name="BlackBeard_Sailor_Head_02", friendlyName="Blackbeard Sailor Head 2", setName="Blackbeard Sailor 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_02_Headband.SK_Armor_Blackbeard_Sailor_02_Headband", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Head/DA_Armor_Regular_BlackBeard_Sailor_Head_02_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Head_01", friendlyName="Blackbeard Sergeant Head 1", setName="Blackbeard Sergeant 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_01_Hat.SK_Armor_Blackbeard_Sergeant_01_Hat", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Head/DA_Armor_Regular_BlackBeard_Sergeant_Head_01_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Head_02", friendlyName="Blackbeard Sergeant Head 2", setName="Blackbeard Sergeant 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_02_Hat.SK_Armor_Blackbeard_Sergeant_02_Hat", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Head/DA_Armor_Regular_BlackBeard_Sergeant_Head_02_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Head_03", friendlyName="Blackbeard Sergeant Head 3", setName="Blackbeard Sergeant 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_03_Hat.SK_Armor_Blackbeard_Sergeant_03_Hat", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Head/DA_Armor_Regular_BlackBeard_Sergeant_Head_03_CompositeMeshData" },
    { name="Hero_Brigant_Head_01", friendlyName="Brigant Head 1", setName="Brigant 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Male_Hat.SK_Armor_Brigant_Male_Hat", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Female_Hat.SK_Armor_Brigant_Female_Hat", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Brigant/CompositeMeshData/Head/DA_Armor_Regular_Hero_Brigant_Head_01_CompositeMeshData" },
    { name="Combatant_Head_01", friendlyName="Combatant Head 1", setName="Combatant 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_03_Head.SK_Armor_Combatant_03_Head", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Head/DA_Armor_Regular_Combatant_Head_01_CompositeMeshData" },
    { name="Combatant_Head_02", friendlyName="Combatant Head 2", setName="Combatant 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_02_Head.SK_Armor_Combatant_02_Head", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Head/DA_Armor_Regular_Combatant_Head_02_CompositeMeshData" },
    { name="Combatant_Head_03", friendlyName="Combatant Head 3", setName="Combatant 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_03_Head.SK_Armor_Combatant_03_Head", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Head/DA_Armor_Regular_Combatant_Head_03_CompositeMeshData" },
    { name="Hero_Conquistador_Head_01", friendlyName="Conquistador Head 1", setName="Conquistador 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_03_Male_Helmet.SK_Armor_Conquistador_03_Male_Helmet", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_03_Female_Helmet.SK_Armor_Conquistador_03_Female_Helmet", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Conquistador/CompositeMeshData/Head/DA_Armor_Regular_Hero_Conquistador_Head_01_CompositeMeshData" },
    { name="Dogface_Head_01", friendlyName="Dogface Head 1", setName="Dogface 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_01_Male_Head.SK_Armor_Dogface_01_Male_Head", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Head/DA_Armor_Regular_Dogface_Head_01_CompositeMeshData" },
    { name="Dogface_Head_02", friendlyName="Dogface Head 2", setName="Dogface 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_02_Male_Head.SK_Armor_Dogface_02_Male_Head", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Head/DA_Armor_Regular_Dogface_Head_02_CompositeMeshData" },
    { name="Dogface_Head_03", friendlyName="Dogface Head 3", setName="Dogface 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_03_Male_Head.SK_Armor_Dogface_03_Male_Head", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Head/DA_Armor_Regular_Dogface_Head_03_CompositeMeshData" },
    { name="Drowned_Armored_Head_01", friendlyName="Drowned Armored Head 1", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned_Armored/Meshes/SK_ArmorCreature_Drowned_Armored_01_Head.SK_ArmorCreature_Drowned_Armored_01_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned_Armored/CompositeMeshData/Head/DA_Armor_Regular_Drowned_Armored_Head_01_CompositeMeshData" },
    { name="Drowned_Armored_Head_02", friendlyName="Drowned Armored Head 2", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned_Armored/Meshes/SK_ArmorCreature_Drowned_Armored_02_Head.SK_ArmorCreature_Drowned_Armored_02_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned_Armored/CompositeMeshData/Head/DA_Armor_Regular_Drowned_Armored_Head_02_CompositeMeshData" },
    { name="Drowned_Armored_Head_03", friendlyName="Drowned Armored Head 3", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned_Armored/Meshes/SK_ArmorCreature_Drowned_Armored_03_Head.SK_ArmorCreature_Drowned_Armored_03_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned_Armored/CompositeMeshData/Head/DA_Armor_Regular_Drowned_Armored_Head_03_CompositeMeshData" },
    { name="Drowned_Head_01", friendlyName="Drowned Head 1", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned/Meshes/SK_ArmorCreature_Drowned_01_Head.SK_ArmorCreature_Drowned_01_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned/CompositeMeshData/Head/DA_Armor_Regular_Drowned_Head_01_CompositeMeshData" },
    { name="Drowned_Head_02", friendlyName="Drowned Head 2", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned/Meshes/SK_ArmorCreature_Drowned_02_Head.SK_ArmorCreature_Drowned_02_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned/CompositeMeshData/Head/DA_Armor_Regular_Drowned_Head_02_CompositeMeshData" },
    { name="Drowned_Head_03", friendlyName="Drowned Head 3", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Drowned/Meshes/SK_ArmorCreature_Drowned_03_Head.SK_ArmorCreature_Drowned_03_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Drowned/CompositeMeshData/Head/DA_Armor_Regular_Drowned_Head_03_CompositeMeshData" },
    { name="Flibustier_Head_01", friendlyName="Flibustier Head 1", setName="Flibustier 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Male_Bandana.SK_Armor_Flibustier_01_Male_Bandana", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Female_Bandana.SK_Armor_Flibustier_01_Female_Bandana", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Head/DA_Armor_Regular_Flibustier_Head_01_CompositeMeshData" },
    { name="Flibustier_Head_02", friendlyName="Flibustier Head 2", setName="Flibustier 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Male_Headband.SK_Armor_Flibustier_02_Male_Headband", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Female_Headband.SK_Armor_Flibustier_02_Female_Headband", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Head/DA_Armor_Regular_Flibustier_Head_02_CompositeMeshData" },
    { name="Flibustier_Head_03", friendlyName="Flibustier Head 3", setName="Flibustier 3", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_03_Male_BandanaHat.SK_Armor_Flibustier_03_Male_BandanaHat", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_03_Female_BandanaHat.SK_Armor_Flibustier_03_Female_BandanaHat", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Head/DA_Armor_Regular_Flibustier_Head_03_CompositeMeshData" },
    { name="Flibustier_Head_04", friendlyName="Flibustier Head 4", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Male_BandanaHat.SK_Armor_Flibustier_02_Male_BandanaHat", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Female_BandanaHat.SK_Armor_Flibustier_02_Female_BandanaHat", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Head/DA_Armor_Regular_Flibustier_Head_04_CompositeMeshData" },
    { name="DA_Mob_Boneman_Spanish_Ghost_CompositeMeshData_Head_01", friendlyName="Ghostly Helmet", setName="Ghostly", availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Ghost/Meshes/SK_ArmorCreature_Boneman_Spanish_Ghost_01_Head.SK_ArmorCreature_Boneman_Spanish_Ghost_01_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Ghost/CompositeMeshData/Torso/DA_Mob_Boneman_Spanish_Ghost_CompositeMeshData_Head_01" },
    { name="Jeweler_Head_01", friendlyName="Jeweler Head 1", setName="Jeweler 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Male_BandanaHat_01.SK_Armor_Jeweler_01_Male_BandanaHat_01", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Female_BandanaHat_01.SK_Armor_Jeweler_01_Female_BandanaHat_01", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Head/DA_Armor_Regular_Jeweler_Head_01_CompositeMeshData" },
    { name="Jeweler_Head_02", friendlyName="Jeweler Head 2", setName="Jeweler 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Male_BandanaHat_02.SK_Armor_Jeweler_01_Male_BandanaHat_02", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Female_BandanaHat_02.SK_Armor_Jeweler_01_Female_BandanaHat_02", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Head/DA_Armor_Regular_Jeweler_Head_02_CompositeMeshData" },
    { name="Jeweler_Head_03", friendlyName="Jeweler Head 3", setName="Jeweler 3", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Male_Bandana_01.SK_Armor_Jeweler_02_Male_Bandana_01", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Female_Bandana_01.SK_Armor_Jeweler_02_Female_Bandana_01", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Head/DA_Armor_Regular_Jeweler_Head_03_CompositeMeshData" },
    { name="Jeweler_Head_04", friendlyName="Jeweler Head 4", setName="Jeweler 4", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Male_Bandana_02.SK_Armor_Jeweler_02_Male_Bandana_02", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Female_Bandana_02.SK_Armor_Jeweler_02_Female_Bandana_02", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Head/DA_Armor_Regular_Jeweler_Head_04_CompositeMeshData" },
    { name="Jeweler_Head_07", friendlyName="Jeweler Head 7", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Male_Hat.SK_Armor_Jeweler_03_Male_Hat", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Female_Hat.SK_Armor_Jeweler_03_Female_Hat", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Head/DA_Armor_Regular_Jeweler_Head_07_CompositeMeshData" },
    { name="Hero_Mercenary_Head_Bandana_01", friendlyName="Mercenary Head 1", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Male_Bandana.SK_Armor_Mercenary_Male_Bandana", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Female_Bandana.SK_Armor_Mercenary_Female_Bandana", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Mercenary/CompositeMeshData/Head/DA_Armor_Regular_Hero_Mercenary_Head_Bandana_01_CompositeMeshData" },
    { name="Hero_Mercenary_Head_Headband_01", friendlyName="Mercenary Head 1 (2)", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Male_Headband.SK_Armor_Mercenary_Male_Headband", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Female_Headband.SK_Armor_Mercenary_Female_Headband", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Mercenary/CompositeMeshData/Head/DA_Armor_Regular_Hero_Mercenary_Head_Headband_01_CompositeMeshData" },
    { name="Hero_Mercenary_Head_Hat_01", friendlyName="Mercenary Head Hat 1", setName="Mercenary 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Male_Hat.SK_Armor_Mercenary_Male_Hat", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Female_Hat.SK_Armor_Mercenary_Female_Hat", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Mercenary/CompositeMeshData/Head/DA_Armor_Regular_Hero_Mercenary_Head_Hat_01_CompositeMeshData" },
    { name="Musketeer_Head_01", friendlyName="Musketeer Head 1", setName="Musketeer 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer01_Head.SK_Armor_Musketeer01_Head", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Head/DA_Armor_Regular_Musketeer_Head_01_CompositeMeshData" },
    { name="Musketeer_Head_02", friendlyName="Musketeer Head 2", setName="Musketeer 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer02_Head.SK_Armor_Musketeer02_Head", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Head/DA_Armor_Regular_Musketeer_Head_02_CompositeMeshData" },
    { name="Musketeer_Head_03", friendlyName="Musketeer Head 3", setName="Musketeer 3", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer03_Head.SK_Armor_Musketeer03_Head", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer03_Head.SK_Armor_Musketeer03_Head", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Head/DA_Armor_Regular_Musketeer_Head_03_CompositeMeshData" },
    { name="Hero_Pikeman_Head_01", friendlyName="Pikeman Head 1", setName="Pikeman 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_04_Male_Helmet.SK_Armor_Conquistador_04_Male_Helmet", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_04_Female_Helmet.SK_Armor_Conquistador_04_Female_Helmet", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Pikeman/CompositeMeshData/Head/DA_Armor_Regular_Hero_Pikeman_Head_01_CompositeMeshData" },
    { name="Hero_MaskSenkamati_Head_01", friendlyName="Senkamati Head 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_02_Head.SK_ArmorCreature_Senkamati_Hunter_Feather_02_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/T03_Head_MaskSenkamati/CompositeMeshData/Head/DA_Armor_Regular_Hero_MaskSenkamati_Head_01_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Head_01", friendlyName="Senkamati Hunter Feather Head 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_01_Head.SK_ArmorCreature_Senkamati_Hunter_Feather_01_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Hunter_Feather_Head_01_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Head_02", friendlyName="Senkamati Hunter Feather Head 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_02_Head.SK_ArmorCreature_Senkamati_Hunter_Feather_02_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Hunter_Feather_Head_02_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Head_03", friendlyName="Senkamati Hunter Feather Head 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_03_Head.SK_ArmorCreature_Senkamati_Hunter_Feather_03_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Hunter_Feather_Head_03_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Head_01", friendlyName="Senkamati Shaman Feather Head 1", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_01_Head.SK_ArmorCreature_Senkamati_Witch_Feather_01_Head", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Shaman_Feather_Head_01_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Head_02", friendlyName="Senkamati Shaman Feather Head 2", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_02_Head.SK_ArmorCreature_Senkamati_Witch_Feather_02_Head", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Shaman_Feather_Head_02_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Head_03", friendlyName="Senkamati Shaman Feather Head 3", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_03_Head.SK_ArmorCreature_Senkamati_Witch_Feather_03_Head", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Shaman_Feather_Head_03_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Head_01", friendlyName="Senkamati Thrall Feather Head 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_01_Head.SK_ArmorCreature_Senkamati_Thrall_Feather_01_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Thrall_Feather_Head_01_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Head_02", friendlyName="Senkamati Thrall Feather Head 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_02_Head.SK_ArmorCreature_Senkamati_Thrall_Feather_02_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Thrall_Feather_Head_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Head_03", friendlyName="Senkamati Thrall Feather Head 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_03_Head.SK_ArmorCreature_Senkamati_Thrall_Feather_03_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Thrall_Feather_Head_03_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Head_04", friendlyName="Senkamati Thrall Feather Head 4", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_04_Head.SK_ArmorCreature_Senkamati_Thrall_Feather_04_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Thrall_Feather_Head_04_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Head_01", friendlyName="Senkamati Warrior Feather Head 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_01_Head.SK_ArmorCreature_Senkamati_Warrior_Feather_01_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Warrior_Feather_Head_01_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Head_02", friendlyName="Senkamati Warrior Feather Head 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_02_Head.SK_ArmorCreature_Senkamati_Warrior_Feather_02_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Warrior_Feather_Head_02_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Head_03", friendlyName="Senkamati Warrior Feather Head 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_03_Head.SK_ArmorCreature_Senkamati_Warrior_Feather_03_Head", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Head/DA_Armor_Regular_Senkamati_Warrior_Feather_Head_03_CompositeMeshData" },
    { name="Hero_SoloPlayer_Head_01", friendlyName="Solo Player Head 1", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Strange/SK_ArmorRegular_Strange_Pot_02.SK_ArmorRegular_Strange_Pot_02", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/T01_Head_SoloPlayer/CompositeMeshData/Head/DA_Armor_Regular_Hero_SoloPlayer_Head_01_CompositeMeshData" },
    { name="Hero_SoloPlayer_Head_02", friendlyName="Solo Player Head 2", setName=nil, availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Strange/SK_ArmorRegular_Strange_Pot_01.SK_ArmorRegular_Strange_Pot_01", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/T01_Head_SoloPlayer/CompositeMeshData/Head/DA_Armor_Regular_Hero_SoloPlayer_Head_02_CompositeMeshData" },
    { name="Hero_Starter_Head_01", friendlyName="Starter Head 1", setName="Starter 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Male_Headband.SK_Armor_Starter_Male_Headband", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Female_Headband.SK_Armor_Starter_Female_Headband", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Starter/CompositeMeshData/Head/DA_Armor_Regular_Hero_Starter_Head_01_CompositeMeshData" },
    { name="Hero_Vanilla_Head_01", friendlyName="Vanilla Head 1", setName="Vanilla 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Male_BandanaHat.SK_Armor_Vanilla_Male_BandanaHat", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Female_BandanaHat.SK_Armor_Vanilla_Female_BandanaHat", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Vanilla/CompositeMeshData/Head/DA_Armor_Regular_Hero_Vanilla_Head_01_CompositeMeshData" },
  },
  Legs = {
    { name="Hero_Adventurer_Legs_01", friendlyName="Adventurer Legs 1", setName="Adventurer 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_Adventurer_Male_Legs.SK_Armor_Flibustier_Adventurer_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_Adventurer_Female_Legs.SK_Armor_Flibustier_Adventurer_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Adventurer/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Adventurer_Legs_01_CompositeMeshData" },
    { name="Hero_Bandit_Legs_01", friendlyName="Bandit Legs 1", setName="Bandit 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Male_Legs_Long.SK_Armor_Bandit_Male_Legs_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Female_Legs_Long.SK_Armor_Bandit_Female_Legs_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Bandit/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Bandit_Legs_01_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Legs_01", friendlyName="Blackbeard Grenadier Legs 1", setName="Blackbeard Grenadier 1", availability="MALE", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_01_Legs.SK_Armor_Blackbeard_Grenadier_01_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Grenadier_Legs_01_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Legs_02", friendlyName="Blackbeard Grenadier Legs 2", setName="Blackbeard Grenadier 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_02_Legs.SK_Armor_Blackbeard_Grenadier_02_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Grenadier_Legs_02_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Legs_03", friendlyName="Blackbeard Grenadier Legs 3", setName="Blackbeard Grenadier 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_03_Legs.SK_Armor_Blackbeard_Grenadier_03_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Grenadier_Legs_03_CompositeMeshData" },
    { name="BlackBeard_Huntsman_Legs_01", friendlyName="Blackbeard Huntsman Legs 1", setName="Blackbeard Huntsman 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_WolfTamer/Meshes/SK_Armor_Blackbeard_WolfTamer_01_Legs.SK_Armor_Blackbeard_WolfTamer_01_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Huntsman/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Huntsman_Legs_01_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Legs_01", friendlyName="Blackbeard Musketeer Legs 1", setName="Blackbeard Musketeer 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_01_Legs.SK_Armor_Blackbeard_Musketeer_01_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Musketeer_Legs_01_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Legs_02", friendlyName="Blackbeard Musketeer Legs 2", setName="Blackbeard Musketeer 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_02_Legs.SK_Armor_Blackbeard_Musketeer_02_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Musketeer_Legs_02_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Legs_03", friendlyName="Blackbeard Musketeer Legs 3", setName="Blackbeard Musketeer 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_03_Legs.SK_Armor_Blackbeard_Musketeer_03_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Musketeer_Legs_03_CompositeMeshData" },
    { name="BlackBeard_Sailor_Legs_01", friendlyName="Blackbeard Sailor Legs 1", setName="Blackbeard Sailor 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_01_Legs_Long.SK_Armor_Blackbeard_Sailor_01_Legs_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Sailor_Legs_01_CompositeMeshData" },
    { name="BlackBeard_Sailor_Legs_02", friendlyName="Blackbeard Sailor Legs 2", setName="Blackbeard Sailor 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_02_Legs_Long.SK_Armor_Blackbeard_Sailor_02_Legs_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Sailor_Legs_02_CompositeMeshData" },
    { name="BlackBeard_Sailor_Legs_03", friendlyName="Blackbeard Sailor Legs 3", setName="Blackbeard Sailor 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_03_Legs.SK_Armor_Blackbeard_Sailor_03_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Sailor_Legs_03_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Legs_01", friendlyName="Blackbeard Sergeant Legs 1", setName="Blackbeard Sergeant 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_01_Legs_Long.SK_Armor_Blackbeard_Sergeant_01_Legs_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Sergeant_Legs_01_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Legs_02", friendlyName="Blackbeard Sergeant Legs 2", setName="Blackbeard Sergeant 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_02_Legs.SK_Armor_Blackbeard_Sergeant_02_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Sergeant_Legs_02_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Legs_03", friendlyName="Blackbeard Sergeant Legs 3", setName="Blackbeard Sergeant 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_03_Legs.SK_Armor_Blackbeard_Sergeant_03_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Legs/DA_Armor_Regular_BlackBeard_Sergeant_Legs_03_CompositeMeshData" },
    { name="BlackSmith_Legs", friendlyName="Blacksmith Legs", setName="Blacksmith", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blacksmith/Meshes/SK_Armor_BlackSmith_Legs.SK_Armor_BlackSmith_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset=nil },
    { name="Hero_Brigant_Legs_01", friendlyName="Brigant Legs 1", setName="Brigant 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Male_Legs_Long.SK_Armor_Brigant_Male_Legs_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Female_Legs_Long.SK_Armor_Brigant_Female_Legs_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Brigant/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Brigant_Legs_01_CompositeMeshData" },
    { name="Character_Underwear_Legs", friendlyName="Character Underwear Legs", setName="Character Underwear", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Underwear/Meshes/SK_Armor_Underwear_02_Male_Legs.SK_Armor_Underwear_02_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Underwear/Meshes/SK_Armor_Underwear_02_Female_Legs.SK_Armor_Underwear_02_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Default/CompositeMeshData/DA_Armor_Regular_Character_Underwear_Legs_CompositeMeshData" },
    { name="Combatant_Legs_01", friendlyName="Combatant Legs 1", setName="Combatant 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_01_Legs.SK_Armor_Combatant_01_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Legs/DA_Armor_Regular_Combatant_Legs_01_CompositeMeshData" },
    { name="Combatant_Legs_02", friendlyName="Combatant Legs 2", setName="Combatant 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_02_Legs.SK_Armor_Combatant_02_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Legs/DA_Armor_Regular_Combatant_Legs_02_CompositeMeshData" },
    { name="Combatant_Legs_03", friendlyName="Combatant Legs 3", setName="Combatant 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_03_Legs.SK_Armor_Combatant_03_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Legs/DA_Armor_Regular_Combatant_Legs_03_CompositeMeshData" },
    { name="Hero_Conquistador_Legs_01", friendlyName="Conquistador Legs 1", setName="Conquistador 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_02_Male_Legs.SK_Armor_Conquistador_02_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_02_Female_Legs.SK_Armor_Conquistador_02_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Conquistador/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Conquistador_Legs_01_CompositeMeshData" },
    { name="Crafter_Legs_01", friendlyName="Crafter Legs 1", setName="Crafter 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_01_Male_Legs.SK_Armor_Dogface_01_Male_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Crafter/CompositeMeshData/Legs/DA_Armor_Regular_Crafter_Legs_01_CompositeMeshData" },
    { name="Crafter_Legs_02", friendlyName="Crafter Legs 2", setName="Crafter 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_02_Male_Legs.SK_Armor_Dogface_02_Male_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Crafter/CompositeMeshData/Legs/DA_Armor_Regular_Crafter_Legs_02_CompositeMeshData" },
    { name="Crafter_Legs_03", friendlyName="Crafter Legs 3", setName="Crafter 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_03_Male_Legs.SK_Armor_Dogface_03_Male_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Crafter/CompositeMeshData/Legs/DA_Armor_Regular_Crafter_Legs_03_CompositeMeshData" },
    { name="Dogface_Legs_01", friendlyName="Dogface Legs 1", setName="Dogface 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_01_Male_Legs.SK_Armor_Dogface_01_Male_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Legs/DA_Armor_Regular_Dogface_Legs_01_CompositeMeshData" },
    { name="Dogface_Legs_02", friendlyName="Dogface Legs 2", setName="Dogface 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_02_Male_Legs.SK_Armor_Dogface_02_Male_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Legs/DA_Armor_Regular_Dogface_Legs_02_CompositeMeshData" },
    { name="Dogface_Legs_03", friendlyName="Dogface Legs 3", setName="Dogface 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_03_Male_Legs.SK_Armor_Dogface_03_Male_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Legs/DA_Armor_Regular_Dogface_Legs_03_CompositeMeshData" },
    { name="Flibustier_Legs_01", friendlyName="Flibustier Legs 1", setName="Flibustier 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Male_Legs.SK_Armor_Flibustier_01_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Female_Legs.SK_Armor_Flibustier_01_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Legs/DA_Armor_Regular_Flibustier_Legs_01_CompositeMeshData" },
    { name="Flibustier_Legs_02", friendlyName="Flibustier Legs 2", setName="Flibustier 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Male_Legs.SK_Armor_Flibustier_02_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Female_Legs.SK_Armor_Flibustier_02_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Legs/DA_Armor_Regular_Flibustier_Legs_02_CompositeMeshData" },
    { name="Flibustier_Legs_03", friendlyName="Flibustier Legs 3", setName="Flibustier 3", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Male_Legs.SK_Armor_Flibustier_02_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_03_Female_Legs.SK_Armor_Flibustier_03_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Legs/DA_Armor_Regular_Flibustier_Legs_03_CompositeMeshData" },
    { name="DA_Mob_Boneman_Spanish_Ghost_CompositeMeshData_Legs_01", friendlyName="Ghostly Legs", setName="Ghostly", availability="MALE", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Ghost/Meshes/SK_ArmorCreature_Boneman_Spanish_Ghost_01_Legs.SK_ArmorCreature_Boneman_Spanish_Ghost_01_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Ghost/CompositeMeshData/Torso/DA_Mob_Boneman_Spanish_Ghost_CompositeMeshData_Legs_01" },
    { name="Jeweler_Legs_01", friendlyName="Jeweler Legs 1", setName="Jeweler 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Male_Legs.SK_Armor_Jeweler_01_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Female_Legs.SK_Armor_Jeweler_01_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Legs/DA_Armor_Regular_Jeweler_Legs_01_CompositeMeshData" },
    { name="Jeweler_Legs_02", friendlyName="Jeweler Legs 2", setName="Jeweler 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Male_Legs.SK_Armor_Jeweler_02_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Female_Legs.SK_Armor_Jeweler_02_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Legs/DA_Armor_Regular_Jeweler_Legs_02_CompositeMeshData" },
    { name="Jeweler_Legs_03", friendlyName="Jeweler Legs 3", setName="Jeweler 3", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Male_Legs.SK_Armor_Jeweler_03_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Female_Legs.SK_Armor_Jeweler_03_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Legs/DA_Armor_Regular_Jeweler_Legs_03_CompositeMeshData" },
    { name="Hero_Mercenary_Legs_01", friendlyName="Mercenary Legs 1", setName="Mercenary 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Male_Legs_Long.SK_Armor_Mercenary_Male_Legs_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Female_Legs_Long.SK_Armor_Mercenary_Female_Legs_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Mercenary/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Mercenary_Legs_01_CompositeMeshData" },
    { name="Musketeer_Legs_01", friendlyName="Musketeer Legs 1", setName="Musketeer 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer01_Legs.SK_Armor_Musketeer01_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Legs/DA_Armor_Regular_Musketeer_Legs_01_CompositeMeshData" },
    { name="Musketeer_Legs_02", friendlyName="Musketeer Legs 2", setName="Musketeer 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer02_Legs.SK_Armor_Musketeer02_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Legs/DA_Armor_Regular_Musketeer_Legs_02_CompositeMeshData" },
    { name="Musketeer_Legs_03", friendlyName="Musketeer Legs 3", setName="Musketeer 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer03_Legs.SK_Armor_Musketeer03_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Legs/DA_Armor_Regular_Musketeer_Legs_03_CompositeMeshData" },
    { name="Hero_Pikeman_Legs_01", friendlyName="Pikeman Legs 1", setName="Pikeman 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_01_Male_Legs.SK_Armor_Conquistador_01_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_01_Female_Legs.SK_Armor_Conquistador_01_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Pikeman/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Pikeman_Legs_01_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Legs_01", friendlyName="Senkamati Hunter Feather Legs 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_01_Legs.SK_ArmorCreature_Senkamati_Hunter_Feather_01_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Hunter_Feather_Legs_01_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Legs_02", friendlyName="Senkamati Hunter Feather Legs 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_02_Legs.SK_ArmorCreature_Senkamati_Hunter_Feather_02_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Hunter_Feather_Legs_02_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Legs_03", friendlyName="Senkamati Hunter Feather Legs 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_03_Legs.SK_ArmorCreature_Senkamati_Hunter_Feather_03_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Hunter_Feather_Legs_03_CompositeMeshData" },
    { name="Senkamati_Hunter_Wood_Legs_01", friendlyName="Senkamati Hunter Wood Legs 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Wood_01_Legs.SK_ArmorCreature_Senkamati_Hunter_Wood_01_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Wood/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Hunter_Wood_Legs_01_CompositeMeshData" },
    { name="Senkamati_Hunter_Wood_Legs_02", friendlyName="Senkamati Hunter Wood Legs 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Wood_02_Legs.SK_ArmorCreature_Senkamati_Hunter_Wood_02_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Wood/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Hunter_Wood_Legs_02_CompositeMeshData" },
    { name="Senkamati_Hunter_Wood_Legs_03", friendlyName="Senkamati Hunter Wood Legs 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Wood_03_Legs.SK_ArmorCreature_Senkamati_Hunter_Wood_03_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Wood/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Hunter_Wood_Legs_03_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Legs_01", friendlyName="Senkamati Shaman Feather Legs 1", setName=nil, availability="FEMALE - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_01_Legs.SK_ArmorCreature_Senkamati_Witch_Feather_01_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Shaman_Feather_Legs_01_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Legs_02", friendlyName="Senkamati Shaman Feather Legs 2", setName=nil, availability="FEMALE - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_02_Legs.SK_ArmorCreature_Senkamati_Witch_Feather_02_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Shaman_Feather_Legs_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Legs_01", friendlyName="Senkamati Thrall Feather Legs 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_01_Legs.SK_ArmorCreature_Senkamati_Thrall_Feather_01_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Thrall_Feather_Legs_01_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Legs_02", friendlyName="Senkamati Thrall Feather Legs 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_02_Legs.SK_ArmorCreature_Senkamati_Thrall_Feather_02_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Thrall_Feather_Legs_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Legs_03", friendlyName="Senkamati Thrall Feather Legs 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_03_Legs.SK_ArmorCreature_Senkamati_Thrall_Feather_03_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Thrall_Feather_Legs_03_CompositeMeshData" },
    { name="Senkamati_Thrall_Wood_Legs_01", friendlyName="Senkamati Thrall Wood Legs 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Wood_01_Legs.SK_ArmorCreature_Senkamati_Thrall_Wood_01_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Wood/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Thrall_Wood_Legs_01_CompositeMeshData" },
    { name="Senkamati_Thrall_Wood_Legs_02", friendlyName="Senkamati Thrall Wood Legs 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Wood_02_Legs.SK_ArmorCreature_Senkamati_Thrall_Wood_02_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Wood/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Thrall_Wood_Legs_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Wood_Legs_03", friendlyName="Senkamati Thrall Wood Legs 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Wood_03_Legs.SK_ArmorCreature_Senkamati_Thrall_Wood_03_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Wood/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Thrall_Wood_Legs_03_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Legs_01", friendlyName="Senkamati Warrior Feather Legs 1", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_01_Legs.SK_ArmorCreature_Senkamati_Warrior_Feather_01_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Warrior_Feather_Legs_01_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Legs_02", friendlyName="Senkamati Warrior Feather Legs 2", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_02_Legs.SK_ArmorCreature_Senkamati_Warrior_Feather_02_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Warrior_Feather_Legs_02_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Legs_03", friendlyName="Senkamati Warrior Feather Legs 3", setName=nil, availability="Both (single unisex mesh) - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_03_Legs.SK_ArmorCreature_Senkamati_Warrior_Feather_03_Legs", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Legs/DA_Armor_Regular_Senkamati_Warrior_Feather_Legs_03_CompositeMeshData" },
    { name="Hero_Starter_Legs_01", friendlyName="Starter Legs 1", setName="Starter 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Male_Legs.SK_Armor_Starter_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Female_Legs.SK_Armor_Starter_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Starter/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Starter_Legs_01_CompositeMeshData" },
    { name="Hero_Starter_Legs_02", friendlyName="Starter Legs 2", setName="Starter 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Restored/Meshes/SK_Armor_Restored_Male_Legs.SK_Armor_Restored_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Restored/Meshes/SK_Armor_Restored_Female_Legs.SK_Armor_Restored_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Starter/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Starter_Legs_02_CompositeMeshData" },
    { name="NPC_GalenSkelton_Legs", friendlyName="Unique - Galen Skelton Legs", setName=nil, availability="MALE ONLY - Only with Unlock", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/DrGalen/Meshes/SK_Armor_DrGalen_Legs.SK_Armor_DrGalen_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/NPC_GalenSkelton/CompositeMeshData/Legs/DA_Armor_Regular_NPC_GalenSkelton_Legs_CompositeMeshData" },
    { name="NPC_Ksante_Legs", friendlyName="Unique - John Legs", setName=nil, availability="MALE ONLY - Only with Unlock", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Ksante/Meshes/SK_Armor_Ksante_Legs.SK_Armor_Ksante_Legs", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/NPC_Ksante/CompositeMeshData/Legs/DA_Armor_Regular_NPC_Ksante_Legs_CompositeMeshData" },
    { name="Hero_Vanilla_Legs_01", friendlyName="Vanilla Legs 1", setName="Vanilla 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Male_Legs.SK_Armor_Vanilla_Male_Legs", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Female_Legs.SK_Armor_Vanilla_Female_Legs", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Vanilla/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Vanilla_Legs_01_CompositeMeshData" },
  },
  Mask = {
    { name="BlackBeard_Sailor_Mask_01", friendlyName="Blackbeard Sailor Mask 1", setName=nil, availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_01_Scarf.SK_Armor_Blackbeard_Sailor_01_Scarf", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Mask/DA_Armor_Regular_BlackBeard_Sailor_Mask_01_CompositeMeshData" },
    { name="BlackBeard_Sailor_Mask_02", friendlyName="Blackbeard Sailor Mask 2", setName=nil, availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_02_Scarf.SK_Armor_Blackbeard_Sailor_02_Scarf", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Mask/DA_Armor_Regular_BlackBeard_Sailor_Mask_02_CompositeMeshData" },
    { name="BlackBeard_Sailor_Mask_03", friendlyName="Blackbeard Sailor Mask 3", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_03_Scarf.SK_Armor_Blackbeard_Sailor_03_Scarf", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_03_Scarf.SK_Armor_Blackbeard_Sailor_03_Scarf", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Mask/DA_Armor_Regular_BlackBeard_Sailor_Mask_03_CompositeMeshData" },
  },
  Torso = {
    { name="Hero_Adventurer_Torso_01", friendlyName="Adventurer Torso 1", setName="Adventurer 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_Adventurer_Male_Torso.SK_Armor_Flibustier_Adventurer_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_Adventurer_Female_Torso.SK_Armor_Flibustier_Adventurer_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Adventurer/CompositeMeshData/Torso/DA_Armor_Regular_Hero_Adventurer_Torso_01_CompositeMeshData" },
    { name="Hero_Bandit_Torso_01", friendlyName="Bandit Torso 1", setName="Bandit 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Male_Torso_Long.SK_Armor_Bandit_Male_Torso_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Female_Torso_Long.SK_Armor_Bandit_Female_Torso_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Bandit/CompositeMeshData/Torso/DA_Armor_Regular_Hero_Bandit_Torso_01_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Torso_01", friendlyName="Blackbeard Grenadier Torso 1", setName="Blackbeard Grenadier 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_01_Torso.SK_Armor_Blackbeard_Grenadier_01_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Grenadier_Torso_01_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Torso_02", friendlyName="Blackbeard Grenadier Torso 2", setName="Blackbeard Grenadier 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_02_Torso.SK_Armor_Blackbeard_Grenadier_02_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Grenadier_Torso_02_CompositeMeshData" },
    { name="BlackBeard_Grenadier_Torso_03", friendlyName="Blackbeard Grenadier Torso 3", setName="Blackbeard Grenadier 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Grenadier/Meshes/SK_Armor_Blackbeard_Grenadier_03_Torso.SK_Armor_Blackbeard_Grenadier_03_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Grenadier/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Grenadier_Torso_03_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Torso_01", friendlyName="Blackbeard Musketeer Torso 1", setName="Blackbeard Musketeer 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_01_Torso.SK_Armor_Blackbeard_Musketeer_01_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Musketeer_Torso_01_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Torso_02", friendlyName="Blackbeard Musketeer Torso 2", setName="Blackbeard Musketeer 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_02_Torso.SK_Armor_Blackbeard_Musketeer_02_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Musketeer_Torso_02_CompositeMeshData" },
    { name="BlackBeard_Musketeer_Torso_03", friendlyName="Blackbeard Musketeer Torso 3", setName="Blackbeard Musketeer 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Musketeer/Meshes/SK_Armor_Blackbeard_Musketeer_03_Torso.SK_Armor_Blackbeard_Musketeer_03_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Musketeer/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Musketeer_Torso_03_CompositeMeshData" },
    { name="BlackBeard_Sailor_Torso_01", friendlyName="Blackbeard Sailor Torso 1", setName="Blackbeard Sailor 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_01_Torso.SK_Armor_Blackbeard_Sailor_01_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Sailor_Torso_01_CompositeMeshData" },
    { name="BlackBeard_Sailor_Torso_02", friendlyName="Blackbeard Sailor Torso 2", setName="Blackbeard Sailor 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_02_Torso.SK_Armor_Blackbeard_Sailor_02_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Sailor_Torso_02_CompositeMeshData" },
    { name="BlackBeard_Sailor_Torso_03", friendlyName="Blackbeard Sailor Torso 3", setName="Blackbeard Sailor 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_03_Torso_Long.SK_Armor_Blackbeard_Sailor_03_Torso_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Sailor_Torso_03_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Torso_01", friendlyName="Blackbeard Sergeant Torso 1", setName="Blackbeard Sergeant 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_01_Torso.SK_Armor_Blackbeard_Sergeant_01_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Sergeant_Torso_01_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Torso_02", friendlyName="Blackbeard Sergeant Torso 2", setName="Blackbeard Sergeant 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_02_Torso.SK_Armor_Blackbeard_Sergeant_02_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Sergeant_Torso_02_CompositeMeshData" },
    { name="BlackBeard_Sergeant_Torso_03", friendlyName="Blackbeard Sergeant Torso 3", setName="Blackbeard Sergeant 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/BlackBeard_Sergeant/Meshes/SK_Armor_Blackbeard_Sergeant_03_Torso.SK_Armor_Blackbeard_Sergeant_03_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sergeant/CompositeMeshData/Torso/DA_Armor_Regular_BlackBeard_Sergeant_Torso_03_CompositeMeshData" },
    { name="Hero_Brigant_Torso_01", friendlyName="Brigant Torso 1", setName="Brigant 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Male_Torso.SK_Armor_Brigant_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Female_Torso.SK_Armor_Brigant_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Brigant/CompositeMeshData/Torso/DA_Armor_Regular_Hero_Brigant_Torso_01_CompositeMeshData" },
    { name="Character_Underwear_Torso_Female", friendlyName="Character Underwear Torso", setName="Character Underwear", availability="FEMALE ONLY", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Underwear/Meshes/SK_Armor_Underwear_01_Female_Torso.SK_Armor_Underwear_01_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Default/CompositeMeshData/DA_Armor_Regular_Character_Underwear_Torso_Female_CompositeMeshData" },
    { name="Combatant_Torso_01", friendlyName="Combatant Torso 1", setName="Combatant 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_01_Torso.SK_Armor_Combatant_01_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Torso/DA_Armor_Regular_Combatant_Torso_01_CompositeMeshData" },
    { name="Combatant_Torso_02", friendlyName="Combatant Torso 2", setName="Combatant 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_02_Torso.SK_Armor_Combatant_02_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Torso/DA_Armor_Regular_Combatant_Torso_02_CompositeMeshData" },
    { name="Combatant_Torso_03", friendlyName="Combatant Torso 3", setName="Combatant 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Combatant/Meshes/SK_Armor_Combatant_03_Torso.SK_Armor_Combatant_03_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Combatant/CompositeMeshData/Torso/DA_Armor_Regular_Combatant_Torso_03_CompositeMeshData" },
    { name="Hero_Conquistador_Torso_01", friendlyName="Conquistador Torso 1", setName="Conquistador 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_03_Male_Torso.SK_Armor_Conquistador_03_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_03_Female_Torso.SK_Armor_Conquistador_03_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Conquistador/CompositeMeshData/Torso/DA_Armor_Regular_Hero_Conquistador_Torso_01_CompositeMeshData" },
    { name="Dogface_Torso_01", friendlyName="Dogface Torso 1", setName="Dogface 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_01_Male_Torso.SK_Armor_Dogface_01_Male_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Torso/DA_Armor_Regular_Dogface_Torso_01_CompositeMeshData" },
    { name="Dogface_Torso_02", friendlyName="Dogface Torso 2", setName="Dogface 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_02_Male_Torso.SK_Armor_Dogface_02_Male_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Torso/DA_Armor_Regular_Dogface_Torso_02_CompositeMeshData" },
    { name="Dogface_Torso_03", friendlyName="Dogface Torso 3", setName="Dogface 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Dogface/Meshes/SK_Armor_Dogface_03_Male_Torso.SK_Armor_Dogface_03_Male_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Dogface/CompositeMeshData/Torso/DA_Armor_Regular_Dogface_Torso_03_CompositeMeshData" },
    { name="Flibustier_Torso_01", friendlyName="Flibustier Torso 1", setName="Flibustier 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Male_Torso.SK_Armor_Flibustier_01_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Female_Torso_Long.SK_Armor_Flibustier_01_Female_Torso_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Torso/DA_Armor_Regular_Flibustier_Torso_01_CompositeMeshData" },
    { name="Flibustier_Torso_02", friendlyName="Flibustier Torso 2", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Male_Torso.SK_Armor_Flibustier_02_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Female_Torso.SK_Armor_Flibustier_02_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Torso/DA_Armor_Regular_Flibustier_Torso_02_CompositeMeshData" },
    { name="Flibustier_Torso_02_Long", friendlyName="Flibustier Torso 2 - Long", setName="Flibustier 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Male_Torso_Long.SK_Armor_Flibustier_02_Male_Torso_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_02_Female_Torso_Long.SK_Armor_Flibustier_02_Female_Torso_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Torso/DA_Armor_Regular_Flibustier_Torso_02_Long_CompositeMeshData" },
    { name="Flibustier_Torso_03", friendlyName="Flibustier Torso 3", setName="Flibustier 3", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_03_Male_Torso.SK_Armor_Flibustier_03_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_03_Female_Torso.SK_Armor_Flibustier_03_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Torso/DA_Armor_Regular_Flibustier_Torso_03_CompositeMeshData" },
    { name="Flibustier_Torso_04", friendlyName="Flibustier Torso 4", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_04_Male_Torso.SK_Armor_Flibustier_04_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_04_Female_Torso.SK_Armor_Flibustier_04_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Torso/DA_Armor_Regular_Flibustier_Torso_04_CompositeMeshData" },
    { name="Flibustier_Torso_05", friendlyName="Flibustier Torso 5", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_05_Male_Torso.SK_Armor_Flibustier_05_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_05_Female_Torso.SK_Armor_Flibustier_05_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Torso/DA_Armor_Regular_Flibustier_Torso_05_CompositeMeshData" },
    { name="DA_Mob_Boneman_Spanish_Ghost_CompositeMeshData_Torso_01", friendlyName="Ghostly Torso", setName="Ghostly", availability="MALE", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Ghost/Meshes/SK_ArmorCreature_Boneman_Spanish_Ghost_02_Torso.SK_ArmorCreature_Boneman_Spanish_Ghost_02_Torso", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Ghost/CompositeMeshData/Torso/DA_Mob_Boneman_Spanish_Ghost_CompositeMeshData_Torso_01" },
    { name="Jeweler_Torso_01", friendlyName="Jeweler Torso 1", setName="Jeweler 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Male_Torso_Long.SK_Armor_Jeweler_01_Male_Torso_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Female_Torso_Long_01.SK_Armor_Jeweler_01_Female_Torso_Long_01", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Torso/DA_Armor_Regular_Jeweler_Torso_01_CompositeMeshData" },
    { name="Jeweler_Torso_02", friendlyName="Jeweler Torso 2", setName="Jeweler 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Male_Torso_Long.SK_Armor_Jeweler_01_Male_Torso_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Female_Torso_Long_02.SK_Armor_Jeweler_01_Female_Torso_Long_02", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Torso/DA_Armor_Regular_Jeweler_Torso_02_CompositeMeshData" },
    { name="Jeweler_Torso_03", friendlyName="Jeweler Torso 3", setName="Jeweler 3", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Male_Torso_Long.SK_Armor_Jeweler_02_Male_Torso_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Female_Torso_Long_01.SK_Armor_Jeweler_02_Female_Torso_Long_01", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Torso/DA_Armor_Regular_Jeweler_Torso_03_CompositeMeshData" },
    { name="Jeweler_Torso_04", friendlyName="Jeweler Torso 4", setName="Jeweler 4", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Male_Torso_Long.SK_Armor_Jeweler_02_Male_Torso_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Female_Torso_Long_02.SK_Armor_Jeweler_02_Female_Torso_Long_02", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Torso/DA_Armor_Regular_Jeweler_Torso_04_CompositeMeshData" },
    { name="Jeweler_Torso_05", friendlyName="Jeweler Torso 5", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Male_Torso_Long.SK_Armor_Jeweler_02_Male_Torso_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Female_Torso_Long_03.SK_Armor_Jeweler_02_Female_Torso_Long_03", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Torso/DA_Armor_Regular_Jeweler_Torso_05_CompositeMeshData" },
    { name="Jeweler_Torso_06", friendlyName="Jeweler Torso 6", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Male_Torso_Long.SK_Armor_Jeweler_03_Male_Torso_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Female_Torso_Long_01.SK_Armor_Jeweler_03_Female_Torso_Long_01", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Torso/DA_Armor_Regular_Jeweler_Torso_06_CompositeMeshData" },
    { name="Jeweler_Torso_07", friendlyName="Jeweler Torso 7", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Male_Torso_Long.SK_Armor_Jeweler_03_Male_Torso_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Female_Torso_Long_02.SK_Armor_Jeweler_03_Female_Torso_Long_02", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Torso/DA_Armor_Regular_Jeweler_Torso_07_CompositeMeshData" },
    { name="Jeweler_Torso_08", friendlyName="Jeweler Torso 8", setName=nil, availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Male_Torso_Long.SK_Armor_Jeweler_03_Male_Torso_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_03_Female_Torso_Long_03.SK_Armor_Jeweler_03_Female_Torso_Long_03", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Torso/DA_Armor_Regular_Jeweler_Torso_08_CompositeMeshData" },
    { name="Hero_Mercenary_Torso_01", friendlyName="Mercenary Torso 1", setName="Mercenary 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Male_Torso_Long.SK_Armor_Mercenary_Male_Torso_Long", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Female_Torso_Long.SK_Armor_Mercenary_Female_Torso_Long", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Mercenary/CompositeMeshData/Torso/DA_Armor_Regular_Hero_Mercenary_Torso_01_CompositeMeshData" },
    { name="Musketeer_Torso_01", friendlyName="Musketeer Torso 1", setName="Musketeer 1", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer01_Torso_Long.SK_Armor_Musketeer01_Torso_Long", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Torso/DA_Armor_Regular_Musketeer_Torso_01_CompositeMeshData" },
    { name="Musketeer_Torso_02", friendlyName="Musketeer Torso 2", setName="Musketeer 2", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer02_Torso.SK_Armor_Musketeer02_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Torso/DA_Armor_Regular_Musketeer_Torso_02_CompositeMeshData" },
    { name="Musketeer_Torso_03", friendlyName="Musketeer Torso 3", setName="Musketeer 3", availability="MALE ONLY", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Musketeer/Meshes/SK_Armor_Musketeer03_Torso.SK_Armor_Musketeer03_Torso", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Musketeer/CompositeMeshData/Torso/DA_Armor_Regular_Musketeer_Torso_03_CompositeMeshData" },
    { name="Hero_Pikeman_Torso_01", friendlyName="Pikeman Torso 1", setName="Pikeman 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_01_Male_Torso.SK_Armor_Conquistador_01_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_01_Female_Torso.SK_Armor_Conquistador_01_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Pikeman/CompositeMeshData/Torso/DA_Armor_Regular_Hero_Pikeman_Torso_01_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Torso_01", friendlyName="Senkamati Hunter Feather Torso 1", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_01_Torso.SK_ArmorCreature_Senkamati_Hunter_Feather_01_Torso", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Torso/DA_Armor_Regular_Senkamati_Hunter_Feather_Torso_01_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Torso_02", friendlyName="Senkamati Hunter Feather Torso 2", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_02_Torso.SK_ArmorCreature_Senkamati_Hunter_Feather_02_Torso", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Torso/DA_Armor_Regular_Senkamati_Hunter_Feather_Torso_02_CompositeMeshData" },
    { name="Senkamati_Hunter_Feather_Torso_03", friendlyName="Senkamati Hunter Feather Torso 3", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Hunter/Meshes/SK_ArmorCreature_Senkamati_Hunter_Feather_03_Torso.SK_ArmorCreature_Senkamati_Hunter_Feather_03_Torso", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Hunter_Feather/CompositeMeshData/Torso/DA_Armor_Regular_Senkamati_Hunter_Feather_Torso_03_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Torso_01", friendlyName="Senkamati Shaman Feather Torso 1", setName=nil, availability="FEMALE - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_01_Torso.SK_ArmorCreature_Senkamati_Witch_Feather_01_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Torso/DA_Armor_Regular_Senkamati_Shaman_Feather_Torso_01_CompositeMeshData" },
    { name="Senkamati_Shaman_Feather_Torso_02", friendlyName="Senkamati Shaman Feather Torso 2", setName=nil, availability="FEMALE - Only with Unlock", maleMesh=nil, femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Witch/Mesh/SK_ArmorCreature_Senkamati_Witch_Feather_02_Torso.SK_ArmorCreature_Senkamati_Witch_Feather_02_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Shaman_Feather/CompositeMeshData/Torso/DA_Armor_Regular_Senkamati_Shaman_Feather_Torso_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Torso_01", friendlyName="Senkamati Thrall Feather Torso 1", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_01_Torso.SK_ArmorCreature_Senkamati_Thrall_Feather_01_Torso", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Torso/DA_Armor_Regular_Senkamati_Thrall_Feather_Torso_01_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Torso_02", friendlyName="Senkamati Thrall Feather Torso 2", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_02_Torso.SK_ArmorCreature_Senkamati_Thrall_Feather_02_Torso", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Torso/DA_Armor_Regular_Senkamati_Thrall_Feather_Torso_02_CompositeMeshData" },
    { name="Senkamati_Thrall_Feather_Torso_03", friendlyName="Senkamati Thrall Feather Torso 3", setName=nil, availability="BOTH - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Thrall/Meshes/SK_ArmorCreature_Senkamati_Thrall_Feather_03_Torso.SK_ArmorCreature_Senkamati_Thrall_Feather_03_Torso", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Thrall_Feather/CompositeMeshData/Torso/DA_Armor_Regular_Senkamati_Thrall_Feather_Torso_03_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Torso_01", friendlyName="Senkamati Warrior Feather Torso 1", setName=nil, availability="MALE ONLY - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_01_Torso.SK_ArmorCreature_Senkamati_Warrior_Feather_01_Torso", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Torso/DA_Armor_Regular_Senkamati_Warrior_Feather_Torso_01_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Torso_02", friendlyName="Senkamati Warrior Feather Torso 2", setName=nil, availability="MALE ONLY - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_02_Torso.SK_ArmorCreature_Senkamati_Warrior_Feather_02_Torso", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Torso/DA_Armor_Regular_Senkamati_Warrior_Feather_Torso_02_CompositeMeshData" },
    { name="Senkamati_Warrior_Feather_Torso_03", friendlyName="Senkamati Warrior Feather Torso 3", setName=nil, availability="MALE ONLY - Only with Unlock", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorCreature/Senkamati_Warrior/Meshes/SK_ArmorCreature_Senkamati_Warrior_Feather_03_Torso.SK_ArmorCreature_Senkamati_Warrior_Feather_03_Torso", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Senkamati_Warrior_Feather/CompositeMeshData/Torso/DA_Armor_Regular_Senkamati_Warrior_Feather_Torso_03_CompositeMeshData" },
    { name="Hero_Starter_Torso_01", friendlyName="Starter Torso 1", setName="Starter 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Male_Torso.SK_Armor_Starter_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Female_Torso.SK_Armor_Starter_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Starter/CompositeMeshData/Torso/DA_Armor_Regular_Hero_Starter_Torso_01_CompositeMeshData" },
    { name="Hero_Starter_Torso_02", friendlyName="Starter Torso 2", setName="Starter 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Restored/Meshes/SK_Armor_Restored_Male_Torso.SK_Armor_Restored_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Restored/Meshes/SK_Armor_Restored_Female_Torso.SK_Armor_Restored_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Starter/CompositeMeshData/Torso/DA_Armor_Regular_Hero_Starter_Torso_02_CompositeMeshData" },
    { name="Hero_Vanilla_Torso_01", friendlyName="Vanilla Torso 1", setName="Vanilla 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Male_Torso.SK_Armor_Vanilla_Male_Torso", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Female_Torso.SK_Armor_Vanilla_Female_Torso", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Vanilla/CompositeMeshData/Torso/DA_Armor_Regular_Hero_Vanilla_Torso_01_CompositeMeshData" },
  },
  Waist = {
    { name="Hero_Adventurer_Waist_01", friendlyName="Adventurer Waist 1", setName="Adventurer 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_Adventurer_Male_Waist.SK_Armor_Flibustier_Adventurer_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_Adventurer_Female_Torso_Waist.SK_Armor_Flibustier_Adventurer_Female_Torso_Waist", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Adventurer/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Adventurer_Waist_01_CompositeMeshData" },
    { name="Hero_Bandit_Waist_01", friendlyName="Bandit Waist 1", setName="Bandit 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Male_Waist.SK_Armor_Bandit_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Bandit/Meshes/SK_Armor_Bandit_Female_Waist.SK_Armor_Bandit_Female_Waist", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Bandit/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Bandit_Waist_01_CompositeMeshData" },
    { name="BlackBeard_Sailor_Waist_02", friendlyName="Blackbeard Sailor Waist 2", setName="Blackbeard Sailor 2", availability="BOTH", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_02_Waist.SK_Armor_Blackbeard_Sailor_02_Waist", femaleMesh=nil, unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Waist/DA_Armor_Regular_BlackBeard_Sailor_Waist_02_CompositeMeshData" },
    { name="BlackBeard_Sailor_Waist_03", friendlyName="Blackbeard Sailor Waist 3", setName="Blackbeard Sailor 3", availability="Both (single unisex mesh)", maleMesh=nil, femaleMesh=nil, unisexMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Blackbeard_Sailor/Meshes/SK_Armor_Blackbeard_Sailor_03_Waist.SK_Armor_Blackbeard_Sailor_03_Waist", sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/BlackBeard_Sailor/CompositeMeshData/Waist/DA_Armor_Regular_BlackBeard_Sailor_Waist_03_CompositeMeshData" },
    { name="Hero_Brigant_Waist_01", friendlyName="Brigant Waist 1", setName="Brigant 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Male_Waist.SK_Armor_Brigant_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Brigant/Meshes/SK_Armor_Brigant_Female_Waist.SK_Armor_Brigant_Female_Waist", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Brigant/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Brigant_Waist_01_CompositeMeshData" },
    { name="Flibustier_Waist_01", friendlyName="Flibustier Waist 1", setName="Flibustier 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Male_Waist.SK_Armor_Flibustier_01_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Flibustier/Meshes/SK_Armor_Flibustier_01_Female_Waist.SK_Armor_Flibustier_01_Female_Waist", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Flibustier/CompositeMeshData/Waist/DA_Armor_Regular_Flibustier_Waist_01_CompositeMeshData" },
    { name="Jeweler_Waist_01", friendlyName="Jeweler Waist 1", setName="Jeweler 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Male_Waist.SK_Armor_Jeweler_01_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Female_Waist_01.SK_Armor_Jeweler_01_Female_Waist_01", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Waist/DA_Armor_Regular_Jeweler_Waist_01_CompositeMeshData" },
    { name="Jeweler_Waist_02", friendlyName="Jeweler Waist 2", setName="Jeweler 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Male_Waist.SK_Armor_Jeweler_01_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_01_Female_Waist_02.SK_Armor_Jeweler_01_Female_Waist_02", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Waist/DA_Armor_Regular_Jeweler_Waist_02_CompositeMeshData" },
    { name="Jeweler_Waist_03", friendlyName="Jeweler Waist 3", setName="Jeweler 3", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Male_Waist.SK_Armor_Jeweler_02_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Jeweler/Meshes/SK_Armor_Jeweler_02_Female_Waist_01.SK_Armor_Jeweler_02_Female_Waist_01", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Jeweler/CompositeMeshData/Waist/DA_Armor_Regular_Jeweler_Waist_03_CompositeMeshData" },
    { name="Hero_Mercenary_Waist_01", friendlyName="Mercenary Waist 1", setName="Mercenary 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Male_Waist.SK_Armor_Mercenary_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Mercenary/Meshes/SK_Armor_Mercenary_Female_Waist.SK_Armor_Mercenary_Female_Waist", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Mercenary/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Mercenary_Waist_01_CompositeMeshData" },
    { name="Hero_Pikeman_Waist_01", friendlyName="Pikeman Waist 1", setName="Pikeman 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_Male_Waist.SK_Armor_Conquistador_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Conquistador/Meshes/SK_Armor_Conquistador_Female_Waist.SK_Armor_Conquistador_Female_Waist", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Pikeman/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Pikeman_Waist_01_CompositeMeshData" },
    { name="Hero_Starter_Waist_01", friendlyName="Starter Waist 1", setName="Starter 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Male_Waist.SK_Armor_Starter_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Starter/Meshes/SK_Armor_Starter_Female_Waist.SK_Armor_Starter_Female_Waist", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Starter/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Starter_Waist_01_CompositeMeshData" },
    { name="Hero_Starter_Waist_02", friendlyName="Starter Waist 2", setName="Starter 2", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Restored/Meshes/SK_Armor_Restored_Male_Waist.SK_Armor_Restored_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Restored/Meshes/SK_Armor_Restored_Female_Waist.SK_Armor_Restored_Female_Waist", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Starter/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Starter_Waist_02_CompositeMeshData" },
    { name="Hero_Vanilla_Waist_01", friendlyName="Vanilla Waist 1", setName="Vanilla 1", availability="Both (distinct Male + Female art)", maleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Male_Waist.SK_Armor_Vanilla_Male_Waist", femaleMesh="/Game/Character/Skeletal_Meshes/Armor/ArmorRegular/Vanilla/Meshes/SK_Armor_Vanilla_Female_Waist.SK_Armor_Vanilla_Female_Waist", unisexMesh=nil, sourceAsset="/Game/Gameplay/Character/Customization/Regular/Armor/Set_Vanilla/CompositeMeshData/Legs/DA_Armor_Regular_Hero_Vanilla_Waist_01_CompositeMeshData" },
  },
}

-- Config.CLOTHES_OUTFITS -- generated by gen_clothes_lua.py from the same workbook's "Clothing
-- Outfits" sheet, grouped by Set Name. Each entry's `pieces` maps a bodyPart to the exact
-- friendlyName to look up in Config.CLOTHES_ITEMS[bodyPart] (no mesh data duplicated here --
-- single source of truth). Applying an outfit only SETS the slots it defines and leaves every
-- other slot untouched (RedFalcon, 2026-09-14: "only set the items, but do not clear any others").
-- Sorted alphabetically by setName -- this order IS the Outfit dropdown order. Outfits missing
-- Feets/Torso/Legs are DROPPED ENTIRELY (RedFalcon: "Any outfit should contain at least feet,
-- torso and legs. If it doesn't, dont create that outfit") -- see this script's own stderr
-- output for which sets were skipped and why, if re-run.
Config.CLOTHES_OUTFITS = {
  { setName="Adventurer 1", pieces = { Feets="Adventurer Feet 1", Hands="Adventurer Hands 1", Headgear="Adventurer Head 1", Legs="Adventurer Legs 1", Torso="Adventurer Torso 1", Waist="Adventurer Waist 1" } },
  { setName="Bandit 1", pieces = { Feets="Bandit Feet 1", Hands="Bandit Hands 1", Headgear="Bandit Head 1", Legs="Bandit Legs 1", Torso="Bandit Torso 1", Waist="Bandit Waist 1" } },
  { setName="Blackbeard Grenadier 1", pieces = { Feets="Blackbeard Grenadier Feet 1", Hands="Blackbeard Grenadier Hands 1", Headgear="Blackbeard Grenadier Head 1", Legs="Blackbeard Grenadier Legs 1", Torso="Blackbeard Grenadier Torso 1" } },
  { setName="Blackbeard Grenadier 2", pieces = { Feets="Blackbeard Grenadier Feet 2", Hands="Blackbeard Grenadier Hands 2", Headgear="Blackbeard Grenadier Head 2", Legs="Blackbeard Grenadier Legs 2", Torso="Blackbeard Grenadier Torso 2" } },
  { setName="Blackbeard Grenadier 3", pieces = { Feets="Blackbeard Grenadier Feet 3", Hands="Blackbeard Grenadier Hands 3", Headgear="Blackbeard Grenadier Head 3", Legs="Blackbeard Grenadier Legs 3", Torso="Blackbeard Grenadier Torso 3" } },
  { setName="Blackbeard Musketeer 1", pieces = { Feets="Blackbeard Musketeer Feet 1 - Long", Hands="Blackbeard Musketeer Hands 1", Headgear="Blackbeard Musketeer Head 1", Legs="Blackbeard Musketeer Legs 1", Torso="Blackbeard Musketeer Torso 1" } },
  { setName="Blackbeard Musketeer 2", pieces = { Feets="Blackbeard Musketeer Feet 2 - Long", Hands="Blackbeard Musketeer Hands 2", Headgear="Blackbeard Musketeer Head 2", Legs="Blackbeard Musketeer Legs 2", Torso="Blackbeard Musketeer Torso 2" } },
  { setName="Blackbeard Musketeer 3", pieces = { Feets="Blackbeard Musketeer Feet 3 - Long", Hands="Blackbeard Musketeer Hands 3", Headgear="Blackbeard Musketeer Head 3", Legs="Blackbeard Musketeer Legs 3", Torso="Blackbeard Musketeer Torso 3" } },
  { setName="Blackbeard Sailor 1", pieces = { Feets="Blackbeard Sailor Feet 1", Hands="Blackbeard Sailor Hands 1", Headgear="Blackbeard Sailor Head 1", Legs="Blackbeard Sailor Legs 1", Torso="Blackbeard Sailor Torso 1" } },
  { setName="Blackbeard Sailor 2", pieces = { Feets="Blackbeard Sailor Feet 2", Hands="Blackbeard Sailor Hands 2", Headgear="Blackbeard Sailor Head 2", Legs="Blackbeard Sailor Legs 2", Torso="Blackbeard Sailor Torso 2", Waist="Blackbeard Sailor Waist 2" } },
  { setName="Blackbeard Sailor 3", pieces = { Feets="Blackbeard Sailor Feet 3", Hands="Blackbeard Sailor Hands 3", Legs="Blackbeard Sailor Legs 3", Torso="Blackbeard Sailor Torso 3", Waist="Blackbeard Sailor Waist 3" } },
  { setName="Blackbeard Sergeant 1", pieces = { Feets="Blackbeard Sergeant Feet 1", Hands="Blackbeard Sergeant Hands 1", Headgear="Blackbeard Sergeant Head 1", Legs="Blackbeard Sergeant Legs 1", Torso="Blackbeard Sergeant Torso 1" } },
  { setName="Blackbeard Sergeant 2", pieces = { Feets="Blackbeard Sergeant Feet 2", Hands="Blackbeard Sergeant Hands 2", Headgear="Blackbeard Sergeant Head 2", Legs="Blackbeard Sergeant Legs 2", Torso="Blackbeard Sergeant Torso 2" } },
  { setName="Blackbeard Sergeant 3", pieces = { Feets="Blackbeard Sergeant Feet 3", Hands="Blackbeard Sergeant Hands 3", Headgear="Blackbeard Sergeant Head 3", Legs="Blackbeard Sergeant Legs 3", Torso="Blackbeard Sergeant Torso 3" } },
  { setName="Brigant 1", pieces = { Feets="Brigant Feet 1", Hands="Brigant Hands 1", Headgear="Brigant Head 1", Legs="Brigant Legs 1", Torso="Brigant Torso 1", Waist="Brigant Waist 1" } },
  { setName="Combatant 1", pieces = { Feets="Combatant Feet 1", Hands="Combatant Hands 1", Headgear="Combatant Head 1", Legs="Combatant Legs 1", Torso="Combatant Torso 1" } },
  { setName="Combatant 2", pieces = { Feets="Combatant Feet 2", Hands="Combatant Hands 2", Headgear="Combatant Head 2", Legs="Combatant Legs 2", Torso="Combatant Torso 2" } },
  { setName="Combatant 3", pieces = { Feets="Combatant Feet 3", Hands="Combatant Hands 3", Headgear="Combatant Head 3", Legs="Combatant Legs 3", Torso="Combatant Torso 3" } },
  { setName="Conquistador 1", pieces = { Cape="Conquistador Cape 1", Feets="Conquistador Feet 1", Hands="Conquistador Hands 1", Headgear="Conquistador Head 1", Legs="Conquistador Legs 1", Torso="Conquistador Torso 1" } },
  { setName="Dogface 1", pieces = { Feets="Dogface Feet 1", Headgear="Dogface Head 1", Legs="Dogface Legs 1", Torso="Dogface Torso 1" } },
  { setName="Dogface 2", pieces = { Feets="Dogface Feet 2", Headgear="Dogface Head 2", Legs="Dogface Legs 2", Torso="Dogface Torso 2" } },
  { setName="Dogface 3", pieces = { Feets="Dogface Feet 3", Headgear="Dogface Head 3", Legs="Dogface Legs 3", Torso="Dogface Torso 3" } },
  { setName="Flibustier 1", pieces = { Feets="Flibustier Feet 1 (2)", Hands="Flibustier Hands 1 - Long", Headgear="Flibustier Head 1", Legs="Flibustier Legs 1", Torso="Flibustier Torso 1", Waist="Flibustier Waist 1" } },
  { setName="Flibustier 2", pieces = { Cape="Flibustier Cape 2", Feets="Flibustier Feet 2", Hands="Flibustier Hands 2 - Long", Headgear="Flibustier Head 2", Legs="Flibustier Legs 2", Torso="Flibustier Torso 2 - Long" } },
  { setName="Ghostly", pieces = { Feets="Ghostly Boots", Hands="Ghostly Gloves", Headgear="Ghostly Helmet", Legs="Ghostly Legs", Torso="Ghostly Torso" } },
  { setName="Jeweler 1", pieces = { Feets="Jeweler Feet 1", Hands="Jeweler Hands 1", Headgear="Jeweler Head 1", Legs="Jeweler Legs 1", Torso="Jeweler Torso 1", Waist="Jeweler Waist 1" } },
  { setName="Jeweler 2", pieces = { Cape="Jeweler Cape 2", Feets="Jeweler Feet 2", Hands="Jeweler Hands 2", Headgear="Jeweler Head 2", Legs="Jeweler Legs 2", Torso="Jeweler Torso 2", Waist="Jeweler Waist 2" } },
  { setName="Jeweler 3", pieces = { Cape="Jeweler Cape 3", Feets="Jeweler Feet 3", Hands="Jeweler Hands 3", Headgear="Jeweler Head 3", Legs="Jeweler Legs 3", Torso="Jeweler Torso 3", Waist="Jeweler Waist 3" } },
  { setName="Mercenary 1", pieces = { Feets="Mercenary Feet 1", Hands="Mercenary Hands 1", Headgear="Mercenary Head Hat 1", Legs="Mercenary Legs 1", Torso="Mercenary Torso 1", Waist="Mercenary Waist 1" } },
  { setName="Musketeer 1", pieces = { Feets="Musketeer Feet 1", Hands="Musketeer Hands 1", Headgear="Musketeer Head 1", Legs="Musketeer Legs 1", Torso="Musketeer Torso 1" } },
  { setName="Musketeer 2", pieces = { Feets="Musketeer Feet 2", Hands="Musketeer Hands 2", Headgear="Musketeer Head 2", Legs="Musketeer Legs 2", Torso="Musketeer Torso 2" } },
  { setName="Musketeer 3", pieces = { Feets="Musketeer Feet 3", Hands="Musketeer Hands 3", Headgear="Musketeer Head 3", Legs="Musketeer Legs 3", Torso="Musketeer Torso 3" } },
  { setName="Pikeman 1", pieces = { Cape="Pikeman Torso 1", Feets="Pikeman Feet 1", Hands="Pikeman Hands 1", Headgear="Pikeman Head 1", Legs="Pikeman Legs 1", Torso="Pikeman Torso 1", Waist="Pikeman Waist 1" } },
  { setName="Starter 1", pieces = { Feets="Starter Feet 1", Hands="Starter Hands 1", Headgear="Starter Head 1", Legs="Starter Legs 1", Torso="Starter Torso 1", Waist="Starter Waist 1" } },
  { setName="Starter 2", pieces = { Feets="Starter Feet 2", Legs="Starter Legs 2", Torso="Starter Torso 2", Waist="Starter Waist 2" } },
  { setName="Vanilla 1", pieces = { Feets="Vanilla Feet 1", Hands="Vanilla Hands 1", Headgear="Vanilla Head 1", Legs="Vanilla Legs 1", Torso="Vanilla Torso 1", Waist="Vanilla Waist 1" } },
}

-- Config.NPC_LOOK_CLASSIFICATION (2026-09-17) -- generated from Other/randomized.xlsx via
-- gen_npc_customization_reference.py. Keyed by a spawn menu DISPLAY LABEL (the label BEFORE
-- Spawner.NextInstanceLabel appends its own trailing instance number, e.g. "BotC Merchant 4"
-- not "BotC Merchant 4 1") -- "randomized" means every customization category on this
-- archetype rerolls at build time (only Color is a real, worth-saving edit); "modified" means
-- this look is already assembled by redressing a different base class (Letty/Marita/the
-- walking women, Senkamati Upright/Wild mask variants, etc.) -- RedFalcon: "already being
-- processed so the detect doesn't make sense", better alternatives still TBD, not yet gated.
-- Absent from this table = "normal": a single fixed look, safe to diff/save/restore
-- per-category using Config.NPC_CUSTOM_BASELINE below.
Config.NPC_LOOK_CLASSIFICATION = {
  ["BotC Musketeer"] = "randomized",
  ["BotC Sailor 1"] = "randomized",
  ["BotC Sailor 2"] = "randomized",
  ["BotC Sailor 3"] = "randomized",
  ["BotC Sailor 4"] = "randomized",
  ["BotC Sailor Chatting"] = "randomized",
  ["BotC Sailor Fire"] = "randomized",
  ["BotC Sailor Leaning"] = "randomized",
  ["BotC Sergeant Fire"] = "randomized",
  ["BotC Sergeant Leaning"] = "randomized",
  ["BotC Woman 1"] = "randomized",
  ["BotC Woman Sitting 1"] = "randomized",
  ["BotC Woman Sitting 2"] = "randomized",
  ["BotC Woman Sitting 3"] = "randomized",
  ["Botc Musketeer Chest"] = "randomized",
  ["Botc Musketeer Sitting 1"] = "randomized",
  ["Botc Musketeer Sitting 2"] = "randomized",
  ["Botc Sailor"] = "randomized",
  ["Botc Sailor Craft"] = "randomized",
  ["Botc Sailor Sitting 1"] = "randomized",
  ["Bucc Infantry Leaning"] = "randomized",
  ["Bucc Infantry Sitting"] = "randomized",
  ["Bucc Infantry Squat"] = "randomized",
  ["Bucc Musketman Fire"] = "randomized",
  ["Bucc Musketman Sitting 1"] = "randomized",
  ["Bucc Musketman Sitting 2"] = "randomized",
  ["Bucc Trapper"] = "randomized",
  ["Bucc Trapper Chest"] = "randomized",
  ["Bucc Trapper Craft"] = "randomized",
  ["Bucc Trapper Leaning"] = "randomized",
  ["Bucc Trapper Sitting 1"] = "randomized",
  ["Bucc Trapper Sitting 2"] = "randomized",
  ["Bucc Trapper Squat"] = "randomized",
  ["Foppish Crew"] = "randomized",
  ["Hunter (Idle) - Masked"] = "modified",
  ["Hunter (Idle) - No Mask"] = "modified",
  ["Hunter (Mobile) - Masked"] = "modified",
  ["Hunter (Mobile) - No Mask"] = "modified",
  ["Letty - Body 1 (Mobile)"] = "modified",
  ["Letty - Body 2 (Mobile)"] = "modified",
  ["Marita Suares - Body 1 (Mobile)"] = "modified",
  ["Marita Suares - Body 2 (Mobile)"] = "modified",
  ["Merchant Woman - Body 1"] = "modified",
  ["Merchant Woman - Body 2"] = "modified",
  ["Musketeer Crew"] = "randomized",
  ["Player Crew"] = "randomized",
  ["Sailor Crew"] = "randomized",
  ["Sergeant Crew"] = "randomized",
  ["Smug Hitman Sitting 1"] = "randomized",
  ["Smug Leaning 1"] = "randomized",
  ["Smug Leaning 2"] = "randomized",
  ["Smug Runner Sitting 2"] = "randomized",
  ["Smug Theif Chest"] = "randomized",
  ["Smug Theif Fire"] = "randomized",
  ["Smug Theif Squat"] = "randomized",
  ["Smug Thief 1"] = "randomized",
  ["Smug Thief 2"] = "randomized",
  ["Smug Thief Sitting 3"] = "randomized",
  ["Smug Thief Sleep"] = "randomized",
  ["Tort Canoneer Sitting"] = "randomized",
  ["Tort Citizen 1"] = "randomized",
  ["Tort Citizen 2"] = "randomized",
  ["Tort Citizen 3"] = "randomized",
  ["Tort Citizen 4"] = "randomized",
  ["Tort Citizen 5"] = "randomized",
  ["Tort Citizen Chest"] = "randomized",
  ["Tort Citizen Craft"] = "randomized",
  ["Tort Citizen Fire"] = "randomized",
  ["Tort Citizen Sitting"] = "randomized",
  ["Tort Citizen Sleep"] = "randomized",
  ["Tort Combatant 1"] = "randomized",
  ["Tort Combatant 2"] = "randomized",
  ["Tort Combatant Fire"] = "randomized",
  ["Tort Combatant Leaning"] = "randomized",
  ["Tort Combatant Sitting 1"] = "randomized",
  ["Tort Combatant Sitting 2"] = "randomized",
  ["Tort Combatant Sleep"] = "randomized",
  ["Tort Musketeer"] = "randomized",
  ["Tort Musketeer 1 Sitting"] = "randomized",
  ["Tort Musketeer 2 Sitting"] = "randomized",
  ["Tort Musketeer Chest"] = "randomized",
  ["Tort Musketman Fire"] = "randomized",
  ["Tort Sailor"] = "randomized",
  ["Town Walker"] = "randomized",
  ["Town Worker"] = "randomized",
  ["Warrior (Idle) - Masked"] = "modified",
  ["Warrior (Mobile) - Masked"] = "modified",
  ["Warrior (Mobile) - No Mask"] = "modified",
  ["Witch (Idle) - Masked"] = "modified",
  ["Witch (Idle) - No Mask"] = "modified",
  ["Witch (Mobile) - Masked"] = "modified",
  ["Witch (Mobile) - No Mask"] = "modified",
  ["Witch - Body 1 (Idle) - Masked"] = "modified",
  ["Witch - Body 1 (Idle) - No Mask"] = "modified",
  ["Witch - Body 1 (Mobile) - Masked"] = "modified",
  ["Witch - Body 1 (Mobile) - No Mask"] = "modified",
  ["Witch - Body 2 (Idle) - Masked"] = "modified",
  ["Witch - Body 2 (Idle) - No Mask"] = "modified",
  ["Witch - Body 2 (Mobile) - Masked"] = "modified",
  ["Witch - Body 2 (Mobile) - No Mask"] = "modified",
  ["Woman - Body 1"] = "modified",
  ["Woman - Body 2"] = "modified",
}

-- Config.NPC_CUSTOM_BASELINE (2026-09-17, revised) -- generated from npc_preset_scan.txt's
-- RAW*:... lines (Spawner.CaptureRawCustomBaseline) via gen_npc_customization_reference.py.
-- Keyed by the FULL DefaultParams asset path (readable off any live actor via
-- Spawner.ReadDefaultParamsSummary). Each entry is the archetype's REAL default state --
-- raw mesh/material names, not friendly names, so a Config.*_ITEMS catalog rename never
-- breaks this comparison -- captured on a genuinely fresh/untouched HOME-scanned actor.
-- BuildCustomStateLines diffs the current read against this per-category so Save only
-- writes a line for what ACTUALLY changed. Only covers archetypes RedFalcon has actually
-- HOME-scanned -- an actor whose DefaultParams isn't a key here just skips this diff
-- entirely (falls back to always-write, current unrestricted behavior).
Config.NPC_CUSTOM_BASELINE = {
  ["/Game/Gameplay/Character/AI/Mob/SenkamatiCorrupted/Regular_Hunter/CompositeMesh/DA_Mob_Senkamati_Regular_Hunter_CompositeMeshComponentParams.DA_Mob_Senkamati_Regular_Hunter_CompositeMeshComponentParams"] = { colors={["FEET"]={"7","13","21"}, ["HANDS"]={"20","3","1"}, ["HAT"]={"0","6","18"}, ["LEGS"]={"1","5","12"}, ["TORSO"]={"4","8","0"}, ["WAIST"]={"22","23","8"}}, hairColors={["Beard"]=1, ["Eyebrows"]=1, ["Hairs"]=1, ["Mustache"]=1, ["Whiskers"]=1}, hair={["Hairs"]="SK_Hair_Shag_06_Default"}, eyeColor="3", skinTone="MI_Senkamati_Feather_Male_Medium", pose="A_Regular_SenkamatiCorrupted_Thrall_Agressive_Idle" },
  ["/Game/Gameplay/Character/AI/Mob/SenkamatiCorrupted/Regular_Shaman_Caster/CompositeMesh/DA_Mob_Senkamati_Regular_Shaman_Caster_CompositeMeshComponentParams.DA_Mob_Senkamati_Regular_Shaman_Caster_CompositeMeshComponentParams"] = { colors={["FEET"]={"8","18","11"}, ["HANDS"]={"15","6","7"}, ["HAT"]={"2","12","16"}, ["LEGS"]={"22","13","19"}, ["TORSO"]={"16","12","1"}, ["WAIST"]={"1","14","0"}}, hairColors={["Beard"]=5, ["Eyebrows"]=5, ["Hairs"]=5, ["Mustache"]=5, ["Whiskers"]=5}, eyeColor="GLOWING", skinTone="MI_Senkamati_Female_Medium", belts={["Sling"]="SK_ArmorCreature_Senkamati_Witch_Feather_01_Neck"}, pose="A_Regular_SenkamatiCorrupted_Thrall_Agressive_Idle" },
  ["/Game/Gameplay/Character/AI/Mob/SenkamatiCorrupted/Regular_Thrall/CompositeMesh/DA_Mob_Senkamati_Regular_Thrall_CompositeMeshComponentParams.DA_Mob_Senkamati_Regular_Thrall_CompositeMeshComponentParams"] = { colors={["FEET"]={"6","9","5"}, ["HANDS"]={"21","20","7"}, ["HAT"]={"21","18","9"}, ["LEGS"]={"22","23","10"}, ["TORSO"]={"1","5","11"}, ["WAIST"]={"8","1","8"}}, hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Hairs"]="SK_Hair_Braid_01_VARIANT_Male"}, eyeColor="3", skinTone="MI_Senkamati_Feather_Male_Large", pose="A_Regular_SenkamatiCorrupted_Thrall_Agressive_Idle" },
  ["/Game/Gameplay/Character/AI/Mob/SenkamatiCorrupted/Regular_Warrior/CompositeMesh/DA_Mob_Senkamati_Regular_Warrior_CompositeMeshComponentParams.DA_Mob_Senkamati_Regular_Warrior_CompositeMeshComponentParams"] = { colors={["FEET"]={"20","1","20"}, ["HANDS"]={"4","2","7"}, ["HAT"]={"16","8","21"}, ["LEGS"]={"10","19","23"}, ["TORSO"]={"19","16","9"}, ["WAIST"]={"8","19","13"}}, hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Hairs"]="SK_Hair_Wavy_03_VARIANT_Male"}, eyeColor="3", skinTone="MI_Senkamati_Feather_Male_Medium", pose="A_SenkamatiCorrupted_Regular_Warrior_Agressive_Idle" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/BrethrenOfTheCoast/CompositeMesh/BenjaminHornigold/DA_NPC_QuestStatic_BotC_BenjaminHornigold_CompositeMeshComponentParams.DA_NPC_QuestStatic_BotC_BenjaminHornigold_CompositeMeshComponentParams"] = { colors={["FEET"]={"3","23","6"}, ["HANDS"]={"6","6","6"}, ["HAT"]={"3","6","11"}, ["LEGS"]={"2","2","5"}, ["TORSO"]={"5","8","6"}, ["WAIST"]={"6","6","6"}}, physique="Medium", hairColors={["Beard"]=1, ["Eyebrows"]=1, ["Hairs"]=1, ["Mustache"]=1, ["Whiskers"]=1}, hair={["Beard"]="SK_Beard_Shag_04", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_Wavy_02_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_02", ["Whiskers"]="SK_Whiskers_Shag_03"}, eyeColor="6", clothes={["Feets"]="SK_Armor_Flibustier_01_Male_Feet_Long", ["Hands"]="SK_Armor_Flibustier_01_Male_Hands", ["Headgear"]="SK_Armor_Flibustier_02_Male_BandanaHat", ["Legs"]="SK_Armor_Vanilla_Male_Legs", ["Torso"]="SK_Armor_Flibustier_02_Male_Torso_Long"}, skinTone="MI_Albion_Male_Medium", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_01_Male", ["Sling"]="SK_Sling_01_Male"}, accessories={["beltSlot_01_rSocket"]="SM_Belt_Misc_Holster_01", ["soc_Sling03F"]="SM_Belt_Misc_Powder_01", ["soc_Sling04F"]="SM_Belt_Misc_HolderCommon_01", ["soc_Strap01F"]="SM_Belt_Misc_Pouch_01", ["soc_belt02B_l"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["soc_beltB"]="SM_Belt_Misc_Pouch_02", ["soc_beltB02_l"]="SM_Belt_Misc_Gloves_01_FL", ["soc_beltStrapB"]="SM_Belt_Misc_Bandoleer_01_Tripple03"}, pose="A_AnimatedActor_Regular_Male_Idle_Standing_06" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/BrethrenOfTheCoast/CompositeMesh/FrancoisArno/DA_NPC_QuestStatic_BotC_FrancoisArno_CompositeMeshComponentParams.DA_NPC_QuestStatic_BotC_FrancoisArno_CompositeMeshComponentParams"] = { colors={["FEET"]={"3","3","23"}, ["HANDS"]={"4","4","4"}, ["HAT"]={"6","8","3"}, ["LEGS"]={"2","2","2"}, ["TORSO"]={"8","8","6"}, ["WAIST"]={"6","6","6"}}, physique="Large", hairColors={["Beard"]=7, ["Eyebrows"]=7, ["Hairs"]=7, ["Mustache"]=7, ["Whiskers"]=7}, hair={["Beard"]="SK_Beard_Bristle_01", ["Eyebrows"]="SK_Eyebrows_Male_03", ["Hairs"]="SK_Hair_Shag_04_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_03", ["Whiskers"]="SK_Whiskers_Hungover"}, eyeColor="6", clothes={["Feets"]="SK_Armor_Bandit_Male_Feet_Long", ["Hands"]="SK_Armor_Flibustier_02_Male_Hands", ["Headgear"]="SK_Armor_Musketeer01_Head", ["Legs"]="SK_Armor_Mercenary_Male_Legs", ["Torso"]="SK_Armor_Flibustier_01_Male_Torso"}, skinTone="MI_Scum_Male_Large", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male", ["Sling"]="SK_Sling_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Drop_PistolT02_03", ["soc_Sling03F"]="SM_Drop_PistolT02_03", ["soc_Strap01F"]="SM_Drop_PistolT02_01", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_02_Simple", ["soc_beltB02_l"]="SM_Belt_Misc_Bandoleer_01_FR", ["soc_beltStrapB"]="SM_Drop_PistolT02_03"}, pose="A_AnimatedActor_Regular_Male_Idle_Standing_05" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/BrethrenOfTheCoast/CompositeMesh/Merchant/DA_NPC_AnimatedActor_BotC_Merchant_01_CompositeMeshComponentParams.DA_NPC_AnimatedActor_BotC_Merchant_01_CompositeMeshComponentParams"] = { colors={["CAPE"]={"1","9","2"}, ["FEET"]={"22","4","6"}, ["HANDS"]={"2","3","2"}, ["HAT"]={"22","2","3"}, ["LEGS"]={"2","6","2"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"8","5","6"}, ["WAIST"]={"6","6","6"}}, physique="Large", hairColors={["Beard"]=7, ["Eyebrows"]=7, ["Hairs"]=7, ["Mustache"]=7, ["Whiskers"]=7}, hair={["Beard"]="SK_Beard_Hungover", ["Eyebrows"]="SK_Eyebrows_Male_03", ["Hairs"]="SK_Hair_Afro_03_VARIANT_Male", ["Mustache"]="SK_Mustaches_Hungover", ["Whiskers"]="SK_Whiskers_Jag_01"}, eyeColor="6", clothes={["Feets"]="SK_Armor_Musketeer03_Feet_Long", ["Hands"]="SK_Armor_Vanilla_Male_Hands_Long", ["Headgear"]="SK_Armor_Combatant_02_Head", ["Legs"]="SK_Armor_Musketeer02_Legs", ["Torso"]="SK_Armor_Combatant_01_Torso"}, skinTone="MI_African_Male_Large", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_01_Male", ["Sling"]="SK_Sling_01_Male"}, accessories={["beltSlot_01_rSocket"]="SM_Belt_Misc_Holster_01", ["soc_Sling03F"]="SM_Belt_Misc_Powder_01", ["soc_Sling04F"]="SM_Belt_Misc_HolderCommon_01", ["soc_Strap01F"]="SM_Belt_Misc_Pouch_01", ["soc_belt02B_l"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["soc_beltB"]="SM_Belt_Misc_Pouch_02", ["soc_beltB02_l"]="SM_Belt_Misc_Gloves_01_FL", ["soc_beltStrapB"]="SM_Belt_Misc_Bandoleer_01_Tripple03", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Vengeful"}, pose="A_AnimatedActor_Regular_Male_Idle_Standing_07" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/BrethrenOfTheCoast/CompositeMesh/Merchant/DA_NPC_AnimatedActor_BotC_Merchant_02_CompositeMeshComponentParams.DA_NPC_AnimatedActor_BotC_Merchant_02_CompositeMeshComponentParams"] = { colors={["CAPE"]={"21","0","17"}, ["FEET"]={"4","2","4"}, ["HANDS"]={"2","2","2"}, ["HAT"]={"4","4","3"}, ["LEGS"]={"2","4","6"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"8","4","6"}, ["WAIST"]={"16","8","12"}}, physique="Medium", hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Beard"]="SK_Beard_Bristle_01", ["Eyebrows"]="SK_Eyebrows_Male_02", ["Hairs"]="SK_Hair_Shag_VARIANT_Male", ["Whiskers"]="SK_Whiskers_Sparse_01"}, eyeColor="12", clothes={["Feets"]="SK_Armor_Dogface_03_Male_Feet", ["Hands"]="SK_Armor_Combatant_03_Hands", ["Headgear"]="SK_Armor_Mercenary_Male_Headband", ["Legs"]="SK_Armor_Vanilla_Male_Legs", ["Torso"]="SK_Armor_Dogface_01_Male_Torso"}, skinTone="MI_Native_Male_Medium", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male", ["Sling"]="SK_Sling_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Drop_PistolT02_01", ["soc_Sling01F"]="SM_Belt_Misc_Knife_01", ["soc_Strap01F"]="SM_Belt_Misc_Knife_03", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple03", ["soc_beltB"]="SM_Belt_Misc_BeltBag_01_Simple", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Forged"}, pose="A_AnimatedActor_Regular_Male_Idle_Standing_05" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/BrethrenOfTheCoast/CompositeMesh/Merchant/DA_NPC_AnimatedActor_BotC_Merchant_03_CompositeMeshComponentParams.DA_NPC_AnimatedActor_BotC_Merchant_03_CompositeMeshComponentParams"] = { colors={["CAPE"]={"4","17","14"}, ["FEET"]={"4","23","7"}, ["HANDS"]={"2","2","2"}, ["HAT"]={"4","4","3"}, ["LEGS"]={"2","4","6"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"8","4","6"}, ["WAIST"]={"7","7","7"}}, physique="Large", hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Beard"]="SK_Beard_Shag_04", ["Eyebrows"]="SK_Eyebrows_Male_04", ["Hairs"]="SK_Hair_Wavy_02_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_03", ["Whiskers"]="SK_Whiskers_Sparse_03"}, eyeColor="7", clothes={["Feets"]="SK_Armor_Flibustier_02_Male_Feet_Long", ["Headgear"]="SK_Armor_Brigant_Male_Hat", ["Legs"]="SK_Armor_Mercenary_Male_Legs", ["Torso"]="SK_Armor_Jeweler_01_Male_Torso_Long"}, skinTone="MI_Adventurer_Male_Large", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male"}, accessories={["Axe2h_backsocket"]="SK_WeaponMelee_Greatsword_Slicer", ["beltSlot_01_lSocket"]="SM_Drop_WeaponRange_Pistol_Faithful", ["soc_Lantern"]="SM_Drop_WeaponRange_Pistol_Baneful", ["soc_Strap01F"]="SM_Drop_WeaponRange_Pistol_Blank", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_01_Simple", ["soc_beltStrapB"]="SM_Belt_Misc_Pouch_01"}, pose="A_Regular_Male_Vendor05" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/BrethrenOfTheCoast/CompositeMesh/Merchant/DA_NPC_AnimatedActor_BotC_Merchant_04_CompositeMeshComponentParams.DA_NPC_AnimatedActor_BotC_Merchant_04_CompositeMeshComponentParams"] = { colors={["CAPE"]={"23","14","18"}, ["FEET"]={"3","2","2"}, ["HANDS"]={"2","2","2"}, ["HAT"]={"4","4","3"}, ["LEGS"]={"2","2","2"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"8","4","6"}, ["WAIST"]={"7","7","7"}}, physique="Medium", hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_LayeredBob_01_VARIANT_Male"}, eyeColor="7", clothes={["Feets"]="SK_Armor_Jeweler_03_Male_Feet_Long", ["Headgear"]="SK_Armor_Flibustier_02_Male_BandanaHat", ["Legs"]="SK_Armor_Flibustier_01_Male_Legs", ["Torso"]="SK_Armor_Jeweler_03_Male_Torso_Long"}, skinTone="MI_Fable_Male_Medium", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male", ["Sling"]="SK_Sling_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Drop_PistolT02_03", ["soc_Sling03F"]="SM_Drop_PistolT02_03", ["soc_Strap01F"]="SM_Drop_PistolT02_01", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_02_Simple", ["soc_beltB02_l"]="SM_Belt_Misc_Bandoleer_01_FR", ["soc_beltStrapB"]="SM_Drop_PistolT02_03", ["swordSlot_lSocket"]="SK_WeaponMelee_Rapier_Swift"}, pose="A_AnimatedActor_Regular_Female_Idle_Standing_03" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/Buccaneers/CompositeMesh/HenriBoucher/DA_NPC_QuestStatic_Buccaneers_HenriBoucher_CompositeMeshComponentParams.DA_NPC_QuestStatic_Buccaneers_HenriBoucher_CompositeMeshComponentParams"] = { colors={["FEET"]={"3","23","4"}, ["HANDS"]={"3","3","3"}, ["HAT"]={"21","4","4"}, ["LEGS"]={"19","11","21"}, ["TORSO"]={"9","3","5"}, ["WAIST"]={"16","8","12"}}, physique="Large", hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Beard"]="SK_Beard_Hungover", ["Eyebrows"]="SK_Eyebrows_Male_04", ["Hairs"]="SK_Hair_Bun_01_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_05", ["Whiskers"]="SK_Whiskers_Shag_05"}, eyeColor="12", clothes={["Cape"]="SK_Armor_Flibustier_01_Male_Cape", ["Feets"]="SK_Armor_Conquistador_02_Male_Feet_Long", ["Hands"]="SK_Armor_Musketeer03_Hands_Long", ["Headgear"]="SK_Armor_Mercenary_Male_Hat", ["Legs"]="SK_Armor_Conquistador_01_Male_Legs", ["Torso"]="SK_Armor_Blackbeard_Musketeer_02_Torso"}, skinTone="MI_Adventurer_Male_Large", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male", ["Sling"]="SK_Sling_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Drop_PistolT02_03", ["soc_Lantern"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Sling01F"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["soc_Sling03F"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Sling04B"]="SM_Drop_MusketT02_03", ["soc_Sling04F"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Strap01F"]="SM_Drop_PistolT02_03", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_02_Simple", ["soc_beltB02_l"]="SM_Belt_Misc_Bandoleer_01_FR", ["soc_beltStrapB"]="SM_Drop_PistolT02_03"}, pose="A_Regular_Carpenter_Idle" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/Buccaneers/CompositeMesh/Merchant/DA_NPC_AnimatedActor_Buccaneers_Merchant_01_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Buccaneers_Merchant_01_CompositeMeshComponentParams"] = { colors={["CAPE"]={"6","9","6"}, ["FEET"]={"4","2","4"}, ["HANDS"]={"9","3","3"}, ["HAT"]={"10","8","3"}, ["LEGS"]={"5","9","23"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"9","9","19"}, ["WAIST"]={"5","5","5"}}, physique="Large", hairColors={["Beard"]=1, ["Eyebrows"]=1, ["Hairs"]=1, ["Mustache"]=1, ["Whiskers"]=1}, hair={["Eyebrows"]="SK_Eyebrows_Female_01", ["Hairs"]="SK_Hair_ShortBob_VARIANT_Female"}, eyeColor="5", clothes={["Feets"]="SK_Armor_Mercenary_Female_Feet_Long", ["Hands"]="SK_Armor_Flibustier_02_Female_Hands_Long", ["Headgear"]="SK_Armor_Musketeer03_Head", ["Legs"]="SK_Armor_Conquistador_02_Male_Legs", ["Torso"]="SK_Armor_Flibustier_04_Female_Torso"}, skinTone="MI_Orient_Female_Large", belts={["Belt"]="SK_Belt_03_Female", ["Frog"]="SK_Frog_03_Female", ["Sling"]="SK_Sling_03_Female", ["Strap"]="SK_Strap_04_Female"}, accessories={["soc_Lantern"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Sling01B"]="SM_Belt_Misc_Bag_01", ["soc_Sling01F"]="SM_Belt_Misc_Knife_01", ["soc_Sling04B"]="SM_Drop_MusketT02_01", ["soc_Sling04F"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Strap01F"]="SM_Belt_Misc_Bottle_01", ["soc_Strap03F"]="SM_Belt_Misc_Bandoleer_01_Rope", ["soc_Strap04F"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Strap_r"]="SM_Belt_Misc_BeltBag_01_Simple", ["soc_beltB"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB02_l"]="SM_Belt_Misc_Bandoleer_01_FR", ["soc_beltStrapB"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Baneful"}, pose="A_AnimatedActor_Regular_Female_Idle_Standing_01" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/Buccaneers/CompositeMesh/Merchant/DA_NPC_AnimatedActor_Buccaneers_Merchant_02_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Buccaneers_Merchant_02_CompositeMeshComponentParams"] = { colors={["CAPE"]={"20","11","6"}, ["FEET"]={"5","23","5"}, ["HANDS"]={"9","3","3"}, ["HAT"]={"10","8","3"}, ["LEGS"]={"5","9","23"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"9","22","19"}, ["WAIST"]={"5","5","5"}}, physique="Small", hairColors={["Beard"]=7, ["Eyebrows"]=7, ["Hairs"]=7, ["Mustache"]=7, ["Whiskers"]=7}, hair={["Beard"]="SK_Beard_Hungover", ["Eyebrows"]="SK_Eyebrows_Male_02", ["Hairs"]="SK_Hair_Shag_06_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_05", ["Whiskers"]="SK_Whiskers_Shag_05"}, eyeColor="5", clothes={["Feets"]="SK_Armor_Flibustier_01_Male_Feet_Long", ["Hands"]="SK_Armor_Musketeer03_Hands_Long", ["Headgear"]="SK_Armor_Starter_Male_Headband", ["Legs"]="SK_Armor_Mercenary_Male_Legs", ["Torso"]="SK_Armor_Flibustier_04_Male_Torso"}, skinTone="MI_Albion_Male_Small", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male", ["Sling"]="SK_Sling_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Drop_PistolT01_03", ["soc_Lantern"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Sling01F"]="SM_Belt_Misc_Knife_01", ["soc_Sling03F"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Sling04B"]="SM_Drop_MusketT02_01", ["soc_Sling04F"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Strap01F"]="SM_Belt_Misc_Bandoleer_01_FR", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB02_l"]="SM_Belt_Misc_Bandoleer_01_FR", ["soc_beltStrapB"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Baneful"}, pose="A_Regular_Male_Vendor04" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/Buccaneers/CompositeMesh/Merchant/DA_NPC_AnimatedActor_Buccaneers_Merchant_03_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Buccaneers_Merchant_03_CompositeMeshComponentParams"] = { colors={["CAPE"]={"20","11","6"}, ["FEET"]={"5","23","5"}, ["HANDS"]={"21","5","9"}, ["HAT"]={"5","9","9"}, ["LEGS"]={"5","9","23"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"9","22","19"}, ["WAIST"]={"19","19","19"}}, physique="Large", hairColors={["Beard"]=5, ["Eyebrows"]=5, ["Hairs"]=5, ["Mustache"]=5, ["Whiskers"]=5}, hair={["Beard"]="SK_Beard_Bristle_01", ["Eyebrows"]="SK_Eyebrows_Male_03", ["Hairs"]="SK_Hair_Afro_02_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_03", ["Whiskers"]="SK_Whiskers_Hungover"}, eyeColor="19", clothes={["Feets"]="SK_Armor_Conquistador_01_Male_Feet_Long", ["Hands"]="SK_Armor_Conquistador_01_Male_Hands_Long", ["Headgear"]="SK_Armor_Bandit_Male_Hat", ["Legs"]="SK_Armor_Vanilla_Male_Legs", ["Torso"]="SK_Armor_Flibustier_05_Male_Torso"}, skinTone="MI_Adventurer_Male_Large", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Drop_WeaponRange_Pistol_Faithful", ["soc_Lantern"]="SM_Drop_WeaponRange_Pistol_Baneful", ["soc_Strap01F"]="SM_Drop_WeaponRange_Pistol_Blank", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_01_Simple", ["soc_beltStrapB"]="SM_Belt_Misc_Pouch_01", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Boarding"}, pose="A_Regular_Male_Vendor05" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/Buccaneers/CompositeMesh/Merchant/DA_NPC_AnimatedActor_Buccaneers_Merchant_04_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Buccaneers_Merchant_04_CompositeMeshComponentParams"] = { colors={["CAPE"]={"20","11","6"}, ["FEET"]={"5","23","9"}, ["HANDS"]={"21","5","9"}, ["HAT"]={"5","9","9"}, ["LEGS"]={"5","9","23"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"9","9","19"}, ["WAIST"]={"19","19","19"}}, physique="Small", hairColors={["Beard"]=0, ["Eyebrows"]=0, ["Hairs"]=0, ["Mustache"]=0, ["Whiskers"]=0}, hair={["Beard"]="SK_Beard_Sparse_01", ["Eyebrows"]="SK_Eyebrows_Male_04", ["Hairs"]="SK_Hair_PartialDreadlocks_01_VARIANT_Male", ["Mustache"]="SK_Mustache_Sparse_01", ["Whiskers"]="SK_Whiskers_Sparse_01"}, eyeColor="19", clothes={["Feets"]="SK_Armor_Combatant_02_Feet", ["Hands"]="SK_Armor_Starter_Male_Hands_Long", ["Headgear"]="SK_Armor_Mercenary_Male_Bandana", ["Legs"]="SK_Armor_Bandit_Male_Legs", ["Torso"]="SK_Armor_Blackbeard_Sailor_03_Torso"}, skinTone="MI_Native_Male_Small", belts={["Frog"]="SK_Frog_01_Male", ["Sling"]="SK_Sling_03_Male", ["Strap"]="SK_Strap_03_Male"}, accessories={["soc_Sling01B"]="SM_Belt_Misc_Bag_01", ["soc_Sling01F"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Sling03F"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Sling04F"]="SM_Belt_Misc_Bandoleer_01_FL", ["soc_Strap03F"]="SM_Belt_Misc_Pouch_02", ["soc_Strap04F"]="SM_Belt_Misc_Bandoleer_01_Rope", ["soc_Strap_r"]="SM_Belt_Misc_BeltBag_01_Simple", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Relentless"}, pose="A_AnimatedActor_Regular_Male_Idle_Standing_08" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/Smugglers/CompositeMesh/MaritaSuares/DA_NPC_QuestStatic_Smugglers_MaritaSuares_CompositeMeshComponentParams.DA_NPC_QuestStatic_Smugglers_MaritaSuares_CompositeMeshComponentParams"] = { colors={["FEET"]={"5","23","11"}, ["HANDS"]={"3","3","3"}, ["HAT"]={"5","14","11"}, ["LEGS"]={"4","3","4"}, ["TORSO"]={"14","2","11"}, ["WAIST"]={"11","11","11"}}, physique="Medium", hairColors={["Beard"]=0, ["Eyebrows"]=0, ["Hairs"]=0, ["Mustache"]=0, ["Whiskers"]=0}, hair={["Eyebrows"]="SK_Eyebrows_Female_03", ["Hairs"]="SK_Hair_Wig_02_VARIANT_Female"}, eyeColor="11", clothes={["Feets"]="SK_Armor_Conquistador_02_Female_Feet_Long", ["Hands"]="SK_Armor_Flibustier_01_Female_Hands", ["Headgear"]="SK_Armor_Flibustier_03_Female_BandanaHat", ["Legs"]="SK_Armor_Flibustier_03_Female_Legs", ["Torso"]="SK_Armor_Flibustier_01_Female_Torso_Long"}, skinTone="MI_Fable_Female_Medium", belts={["Belt"]="SK_Belt_03_Female", ["Frog"]="SK_Frog_03_Female"}, accessories={["beltSlot_01_lSocket"]="SM_Drop_WeaponRange_Pistol_Rusty", ["soc_Lantern"]="SM_Drop_WeaponRange_Pistol_Devastating", ["soc_Strap01F"]="SM_Drop_WeaponRange_Pistol_DrakesDoom", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_01_Simple", ["soc_beltStrapB"]="SM_Belt_Misc_Pouch_01"}, pose="A_AnimatedActor_Regular_Female_Idle_Standing_03" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/Smugglers/CompositeMesh/Merchant/DA_NPC_AnimatedActor_Smugglers_Merchant_01_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Smugglers_Merchant_01_CompositeMeshComponentParams"] = { colors={["CAPE"]={"19","19","9"}, ["FEET"]={"4","2","4"}, ["HANDS"]={"14","21","5"}, ["HAT"]={"14","23","23"}, ["LEGS"]={"23","23","21"}, ["MASK"]={"14","14","14"}, ["TORSO"]={"11","14","14"}, ["WAIST"]={"14","14","14"}}, physique="Small", hairColors={["Beard"]=1, ["Eyebrows"]=1, ["Hairs"]=1, ["Mustache"]=1, ["Whiskers"]=1}, hair={["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_Bun_01_VARIANT_Male"}, eyeColor="14", clothes={["Feets"]="SK_Armor_Musketeer01_Feet_Long", ["Hands"]="SK_Armor_Combatant_02_Hands", ["Headgear"]="SK_Armor_Musketeer01_Head", ["Legs"]="SK_Armor_Musketeer01_Legs", ["Mask"]="SK_Armor_Blackbeard_Sailor_03_Scarf", ["Torso"]="SK_Armor_Musketeer01_Torso_Long"}, skinTone="MI_Orient_Male_Small", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male", ["Sling"]="SK_Sling_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Belt_Misc_Knife_04", ["soc_Sling01F"]="SM_Drop_PistolT02_04", ["soc_Sling04F"]="SM_Belt_Misc_Knife_01", ["soc_Strap01F"]="SM_Belt_Misc_Knife_01", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["soc_beltStrapB"]="SM_Belt_Misc_Knife_04", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Graceful"}, pose="A_AnimatedActor_Regular_Male_Idle_Standing_05" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/Smugglers/CompositeMesh/Merchant/DA_NPC_AnimatedActor_Smugglers_Merchant_02_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Smugglers_Merchant_02_CompositeMeshComponentParams"] = { colors={["CAPE"]={"19","19","9"}, ["FEET"]={"4","2","4"}, ["HANDS"]={"14","21","5"}, ["HAT"]={"14","23","23"}, ["LEGS"]={"23","23","21"}, ["MASK"]={"14","14","14"}, ["TORSO"]={"11","14","14"}, ["WAIST"]={"14","14","14"}}, physique="Small", hairColors={["Beard"]=1, ["Eyebrows"]=1, ["Hairs"]=1, ["Mustache"]=1, ["Whiskers"]=1}, hair={["Eyebrows"]="SK_Eyebrows_Male_02", ["Hairs"]="SK_Hair_Bun_01_VARIANT_Male"}, eyeColor="14", clothes={["Feets"]="SK_Armor_Jeweler_02_Male_Feet_Long", ["Hands"]="SK_Armor_Conquistador_01_Male_Hands_Long", ["Headgear"]="SK_Armor_Jeweler_01_Male_BandanaHat_01", ["Legs"]="SK_Armor_Jeweler_03_Male_Legs", ["Mask"]="SK_Armor_Blackbeard_Sailor_03_Scarf", ["Torso"]="SK_Armor_Combatant_01_Torso"}, skinTone="MI_Fable_Male_Small", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male", ["Sling"]="SK_Sling_03_Male"}, accessories={["Axe2h_backsocket"]="SK_WeaponMelee_Greatsword_Savage", ["beltSlot_01_lSocket"]="SM_Belt_Misc_Knife_04", ["soc_Sling01F"]="SM_Drop_PistolT02_04", ["soc_Sling04F"]="SM_Belt_Misc_Knife_01", ["soc_Strap01F"]="SM_Belt_Misc_Knife_03", ["soc_belt02B_l"]="SM_Belt_Misc_Bag_01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["soc_beltSlingB"]="SM_Belt_Misc_Knife_01", ["soc_beltStrapB"]="SM_Belt_Misc_Pouch_02"}, pose="A_Regular_Male_Vendor02" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/Smugglers/CompositeMesh/Merchant/DA_NPC_AnimatedActor_Smugglers_Merchant_03_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Smugglers_Merchant_03_CompositeMeshComponentParams"] = { colors={["CAPE"]={"9","9","14"}, ["FEET"]={"3","2","13"}, ["HANDS"]={"21","14","11"}, ["HAT"]={"23","11","2"}, ["LEGS"]={"23","22","5"}, ["MASK"]={"14","14","14"}, ["TORSO"]={"14","11","14"}, ["WAIST"]={"14","14","14"}}, physique="Small", hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_Bun_01_VARIANT_Male"}, eyeColor="14", clothes={["Cape"]="SK_Armor_Jeweler_01_Male_Cape_03", ["Feets"]="SK_Armor_Blackbeard_Musketeer_03_Feet_Long", ["Hands"]="SK_Armor_Flibustier_02_Male_Hands_Long", ["Headgear"]="SK_Armor_Bandit_Male_Hat", ["Legs"]="SK_Armor_Flibustier_01_Male_Legs", ["Mask"]="SK_Armor_Blackbeard_Sailor_03_Scarf", ["Torso"]="SK_Armor_Vanilla_Male_Torso"}, skinTone="MI_Orient_Male_Small", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male", ["Sling"]="SK_Sling_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Belt_Misc_Knife_04", ["soc_Sling01F"]="SM_Drop_PistolT02_04", ["soc_Sling04F"]="SM_Belt_Misc_Knife_01", ["soc_Strap01F"]="SM_Belt_Misc_Knife_01", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["soc_beltStrapB"]="SM_Belt_Misc_Knife_04", ["swordSlot_lSocket"]="SK_WeaponMelee_Rapier_Swift"}, pose="A_Regular_Male_Vendor03" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/Smugglers/CompositeMesh/Merchant/DA_NPC_AnimatedActor_Smugglers_Merchant_04_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Smugglers_Merchant_04_CompositeMeshComponentParams"] = { colors={["CAPE"]={"9","9","14"}, ["FEET"]={"3","2","13"}, ["HANDS"]={"21","14","11"}, ["HAT"]={"23","5","11"}, ["LEGS"]={"23","22","5"}, ["MASK"]={"3","14","14"}, ["TORSO"]={"14","11","5"}, ["WAIST"]={"14","14","14"}}, physique="Small", hairColors={["Beard"]=4, ["Eyebrows"]=4, ["Hairs"]=4, ["Mustache"]=4, ["Whiskers"]=4}, hair={["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_Wavy_02_VARIANT_Male"}, eyeColor="14", clothes={["Feets"]="SK_Armor_Jeweler_01_Male_Feet_Long", ["Hands"]="SK_Armor_Combatant_02_Hands", ["Headgear"]="SK_Armor_Flibustier_02_Male_BandanaHat", ["Legs"]="SK_Armor_Musketeer01_Legs", ["Mask"]="SK_Armor_Blackbeard_Sailor_03_Scarf", ["Torso"]="SK_Armor_Jeweler_03_Male_Torso"}, skinTone="MI_Scum_Male_Small", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male", ["Sling"]="SK_Sling_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Belt_Misc_Knife_04", ["soc_Sling01F"]="SM_Drop_PistolT02_04", ["soc_Sling04F"]="SM_Belt_Misc_Knife_01", ["soc_Strap01F"]="SM_Belt_Misc_Knife_01", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["soc_beltStrapB"]="SM_Belt_Misc_Knife_04", ["swordSlot_lSocket"]="SK_WeaponMelee_Rapier_Loyal"}, pose="A_AnimatedActor_Regular_Female_Idle_SittingOnChair_02" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/TortugaCitizen/CompositeMesh/CharlieSharp/DA_NPC_QuestStatic_TortugaCitizen_CharlieSharp_CompositeMeshComponentParams.DA_NPC_QuestStatic_TortugaCitizen_CharlieSharp_CompositeMeshComponentParams"] = { colors={["CAPE"]={"7","13","13"}, ["FEET"]={"5","5","4"}, ["HANDS"]={"22","6","4"}, ["HAT"]={"16","16","15"}, ["LEGS"]={"5","5","16"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"2","23","16"}, ["WAIST"]={"6","6","6"}}, physique="Medium", hairColors={["Beard"]=7, ["Eyebrows"]=7, ["Hairs"]=7, ["Mustache"]=7, ["Whiskers"]=7}, hair={["Beard"]="SK_Beard_Bristle_01", ["Eyebrows"]="SK_Eyebrows_Male_02", ["Hairs"]="SK_Hair_Shag_VARIANT_Male", ["Mustache"]="SK_Mustache_Jag_01", ["Whiskers"]="SK_Whiskers_Shag_03"}, eyeColor="6", clothes={["Feets"]="SK_Armor_Musketeer02_Feet_Long", ["Hands"]="SK_Armor_Combatant_03_Hands", ["Headgear"]="SK_Armor_Starter_Male_Headband", ["Legs"]="SK_Armor_Restored_Male_Legs", ["Torso"]="SK_Armor_Brigant_Male_Torso"}, skinTone="MI_Native_Male_Medium", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male", ["Sling"]="SK_Sling_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Belt_Misc_Knife_04", ["soc_Sling01F"]="SM_Drop_PistolT02_04", ["soc_Sling04F"]="SM_Belt_Misc_Knife_01", ["soc_Strap01F"]="SM_Belt_Misc_Knife_01", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["soc_beltStrapB"]="SM_Belt_Misc_Knife_04"}, pose="A_AnimatedActor_Regular_Male_Idle_Standing_07" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/TortugaCitizen/CompositeMesh/Letty/DA_NPC_QuestStatic_TortugaCitizen_Letty_CompositeMeshComponentParams.DA_NPC_QuestStatic_TortugaCitizen_Letty_CompositeMeshComponentParams"] = { colors={["FEET"]={"5","23","2"}, ["HANDS"]={"3","3","3"}, ["HAT"]={"5","14","11"}, ["LEGS"]={"16","3","4"}, ["TORSO"]={"23","2","2"}, ["WAIST"]={"11","11","11"}}, physique="Medium", hairColors={["Beard"]=0, ["Eyebrows"]=0, ["Hairs"]=0, ["Mustache"]=0, ["Whiskers"]=0}, hair={["Eyebrows"]="SK_Eyebrows_Female_03", ["Hairs"]="SK_Hair_Ponytail_01_VARIANT_Female"}, eyeColor="11", clothes={["Feets"]="SK_Armor_Brigant_Female_Feet_Long", ["Hands"]="SK_Armor_Flibustier_01_Female_Hands", ["Legs"]="SK_Armor_Mercenary_Female_Legs", ["Torso"]="SK_Armor_Starter_Female_Torso"}, skinTone="MI_Adventurer_Female_Medium", pose="A_AnimatedActor_Regular_Female_Idle_Standing_02" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/TortugaCitizen/CompositeMesh/LongBen/DA_NPC_QuestStatic_TortugaCitizen_LongBen_CompositeMeshComponentParams.DA_NPC_QuestStatic_TortugaCitizen_LongBen_CompositeMeshComponentParams"] = { colors={["FEET"]={"23","22","4"}, ["HANDS"]={"3","3","3"}, ["HAT"]={"2","22","22"}, ["LEGS"]={"4","2","11"}, ["TORSO"]={"0","6","3"}, ["WAIST"]={"6","6","6"}}, physique="Medium", hairColors={["Beard"]=1, ["Eyebrows"]=1, ["Hairs"]=1, ["Mustache"]=1, ["Whiskers"]=1}, hair={["Beard"]="SK_Beard_Hungover", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Mustache"]="SK_Mustache_Shag_02", ["Whiskers"]="SK_Whiskers_Hungover"}, eyeColor="6", clothes={["Feets"]="SK_Armor_Bandit_Male_Feet_Long", ["Hands"]="SK_Armor_Vanilla_Male_Hands_Long", ["Legs"]="SK_Armor_Mercenary_Male_Legs", ["Torso"]="SK_Armor_Blackbeard_Musketeer_01_Torso"}, skinTone="MI_Albion_Male_Medium", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male"}, accessories={["soc_Lantern"]="SM_Belt_Misc_Pouch_02", ["soc_Strap01F"]="SM_Belt_Misc_Knife_01", ["soc_belt02B_l"]="SM_Belt_Misc_SkullWolf_01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_01_Simple", ["soc_beltStrapB"]="SM_Belt_Misc_Pouch_01"} },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/TortugaCitizen/CompositeMesh/Merchant/DA_NPC_AnimatedActor_TortugaCitizen_Merchant_01_CompositeMeshComponentParams.DA_NPC_AnimatedActor_TortugaCitizen_Merchant_01_CompositeMeshComponentParams"] = { colors={["CAPE"]={"12","18","10"}, ["FEET"]={"3","2","21"}, ["HANDS"]={"21","6","2"}, ["HAT"]={"19","16","2"}, ["LEGS"]={"16","16","16"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"2","21","3"}, ["WAIST"]={"2","2","2"}}, physique="Medium", hairColors={["Beard"]=4, ["Eyebrows"]=4, ["Hairs"]=4, ["Mustache"]=4, ["Whiskers"]=4}, hair={["Beard"]="SK_Beard_Shag_05", ["Eyebrows"]="SK_Eyebrows_Male_03", ["Hairs"]="SK_Hair_Braid_01_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_05", ["Whiskers"]="SK_Whiskers_Shag_05"}, eyeColor="2", clothes={["Feets"]="SK_Armor_Combatant_01_Feet", ["Hands"]="SK_Armor_Flibustier_02_Male_Hands_Long", ["Headgear"]="SK_Armor_Mercenary_Male_Bandana", ["Legs"]="SK_Armor_Combatant_02_Legs", ["Torso"]="SK_Armor_Blackbeard_Sailor_01_Torso"}, skinTone="MI_Albion_Male_Medium", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male", ["Sling"]="SK_Sling_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Drop_PistolT02_03", ["soc_Sling03F"]="SM_Drop_PistolT02_03", ["soc_Strap01F"]="SM_Drop_PistolT02_01", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_02_Simple", ["soc_beltB02_l"]="SM_Belt_Misc_Bandoleer_01_FR", ["soc_beltStrapB"]="SM_Drop_PistolT02_03", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Severe"}, pose="A_Regular_Male_Vendor01" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/TortugaCitizen/CompositeMesh/Merchant/DA_NPC_AnimatedActor_TortugaCitizen_Merchant_02_CompositeMeshComponentParams.DA_NPC_AnimatedActor_TortugaCitizen_Merchant_02_CompositeMeshComponentParams"] = { colors={["CAPE"]={"10","22","15"}, ["FEET"]={"3","3","2"}, ["HANDS"]={"5","6","2"}, ["HAT"]={"22","2","16"}, ["LEGS"]={"16","3","3"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"5","5","5"}, ["WAIST"]={"3","3","2"}}, physique="Large", hairColors={["Beard"]=0, ["Eyebrows"]=0, ["Hairs"]=0, ["Mustache"]=0, ["Whiskers"]=0}, hair={["Beard"]="SK_Beard_Bristle_01", ["Eyebrows"]="SK_Eyebrows_Male_04", ["Hairs"]="SK_Hair_Bristle_01_VARIANT_Male", ["Mustache"]="SK_Mustache_Jag_02"}, eyeColor="2", clothes={["Feets"]="SK_Armor_Conquistador_01_Male_Feet_Long", ["Hands"]="SK_Armor_Combatant_03_Hands", ["Headgear"]="SK_Armor_Combatant_03_Head", ["Legs"]="SK_Armor_Conquistador_01_Male_Legs", ["Waist"]="SK_Armor_Mercenary_Male_Waist"}, skinTone="MI_African_Male_Large", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male"}, accessories={["soc_Strap01F"]="SM_Belt_Misc_Knife_01", ["soc_belt02B_l"]="SM_Belt_Misc_Pouch_02", ["soc_beltB"]="SM_Belt_Misc_BeltBag_01_Simple", ["soc_beltStrapB"]="SM_Belt_Misc_Knife_02", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Savage"}, pose="A_Regular_Male_Vendor02" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/TortugaCitizen/CompositeMesh/Merchant/DA_NPC_AnimatedActor_TortugaCitizen_Merchant_03_CompositeMeshComponentParams.DA_NPC_AnimatedActor_TortugaCitizen_Merchant_03_CompositeMeshComponentParams"] = { colors={["CAPE"]={"4","20","17"}, ["FEET"]={"21","2","21"}, ["HANDS"]={"21","6","2"}, ["HAT"]={"2","2","2"}, ["LEGS"]={"16","16","16"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"21","2","2"}, ["WAIST"]={"2","2","2"}}, physique="Medium", hairColors={["Beard"]=4, ["Eyebrows"]=4, ["Hairs"]=4, ["Mustache"]=4, ["Whiskers"]=4}, hair={["Beard"]="SK_Beard_Shag_05", ["Eyebrows"]="SK_Eyebrows_Male_02", ["Hairs"]="SK_Hair_Braid_01_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_05", ["Whiskers"]="SK_Whiskers_Shag_05"}, eyeColor="2", clothes={["Feets"]="SK_Armor_Combatant_01_Feet", ["Hands"]="SK_Armor_Bandit_Male_Hands_Long", ["Headgear"]="SK_Armor_Musketeer01_Head", ["Legs"]="SK_Armor_Bandit_Male_Legs_Long", ["Torso"]="SK_Armor_Flibustier_05_Male_Torso"}, skinTone="MI_Adventurer_Male_Medium", belts={["Sling"]="SK_Frog_04_Male"}, accessories={["soc_Strap01F"]="SM_Belt_Misc_Bottle_01", ["soc_belt02B_l"]="SM_Belt_Misc_Pouch_01", ["soc_beltB"]="SM_Belt_Misc_Knife_01", ["soc_beltStrapB"]="SM_Belt_Misc_Gloves_01_FL", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Boarding"}, pose="A_Regular_Male_Vendor04" },
  ["/Game/Gameplay/Character/AI/NPC/FactionActors/TortugaCitizen/CompositeMesh/Merchant/DA_NPC_AnimatedActor_TortugaCitizen_Merchant_04_CompositeMeshComponentParams.DA_NPC_AnimatedActor_TortugaCitizen_Merchant_04_CompositeMeshComponentParams"] = { colors={["CAPE"]={"0","13","6"}, ["FEET"]={"22","2","16"}, ["HANDS"]={"21","6","2"}, ["HAT"]={"22","22","1"}, ["LEGS"]={"16","3","3"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"22","2","2"}, ["WAIST"]={"2","2","2"}}, physique="Large", hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Eyebrows"]="SK_Eyebrows_Male_02", ["Hairs"]="SK_Hair_LayeredBobDecor_01_VARIANT_Male"}, eyeColor="2", clothes={["Feets"]="SK_Armor_Jeweler_03_Male_Feet_Long", ["Hands"]="SK_Armor_Brigant_Male_Hands_Long", ["Headgear"]="SK_Armor_Brigant_Male_Hat", ["Legs"]="SK_Armor_Vanilla_Male_Legs", ["Torso"]="SK_Armor_Flibustier_Adventurer_Male_Torso"}, skinTone="MI_Native_Male_Large", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Drop_WeaponRange_Pistol_Faithful", ["soc_Lantern"]="SM_Drop_WeaponRange_Pistol_Baneful", ["soc_Strap01F"]="SM_Drop_WeaponRange_Pistol_Blank", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_01_Simple", ["soc_beltStrapB"]="SM_Belt_Misc_Pouch_01", ["swordSlot_lSocket"]="SK_WeaponMelee_Rapier_Blank"}, pose="A_AnimatedActor_Regular_Female_Idle_Standing_01" },
  ["/Game/Gameplay/Character/AI/NPC/Handyman/Handyman_Farmer/Params/Customization/DA_NPC_Handyman_Farmer_CompositeMeshComponentParams.DA_NPC_Handyman_Farmer_CompositeMeshComponentParams"] = { colors={["FEET"]={"23","2","4"}, ["HANDS"]={"4","4","4"}, ["HAT"]={"4","8","3"}, ["LEGS"]={"23","2","2"}, ["TORSO"]={"21","3","0"}, ["WAIST"]={"3","3","9"}}, physique="Small", hairColors={["Beard"]=2, ["Eyebrows"]=2, ["Hairs"]=2, ["Mustache"]=2, ["Whiskers"]=2}, hair={["Beard"]="SK_Beard_Shag_05", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_Shag_05_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_05", ["Whiskers"]="SK_Whiskers_Shag_05"}, eyeColor="9", clothes={["Feets"]="SK_Armor_Blackbeard_Sailor_03_Feet_Long", ["Legs"]="SK_Armor_Dogface_01_Male_Legs", ["Waist"]="SK_Armor_Restored_Male_Waist"}, skinTone="MI_Scum_Male_Small" },
  ["/Game/Gameplay/Character/AI/NPC/Handyman/Handyman_Gatherer/Params/Customization/DA_NPC_Handyman_Gatherer_CompositeMeshComponentParams.DA_NPC_Handyman_Gatherer_CompositeMeshComponentParams"] = { colors={["FEET"]={"21","23","7"}, ["HANDS"]={"17","16","21"}, ["HAT"]={"13","18","18"}, ["LEGS"]={"4","15","8"}, ["TORSO"]={"23","4","2"}, ["WAIST"]={"7","19","8"}}, physique="Small", hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Eyebrows"]="SK_Eyebrows_Female_01", ["Hairs"]="SK_Hair_Shag_02_VARIANT_Female"}, eyeColor="8", clothes={["Feets"]="SK_Armor_Blackbeard_Sailor_03_Feet_Long", ["Legs"]="SK_Armor_Underwear_02_Female_Legs", ["Torso"]="SK_Armor_Brigant_Female_Torso"}, skinTone="MI_Adventurer_Female_Small" },
  ["/Game/Gameplay/Character/AI/NPC/Handyman/Handyman_Herbalist/Params/Customization/DA_NPC_Handyman_Herbalist_CompositeMeshComponentParams.DA_NPC_Handyman_Herbalist_CompositeMeshComponentParams"] = { colors={["FEET"]={"23","2","4"}, ["HANDS"]={"4","4","4"}, ["HAT"]={"4","8","3"}, ["LEGS"]={"23","2","2"}, ["TORSO"]={"21","3","0"}, ["WAIST"]={"3","3","14"}}, physique="Large", hairColors={["Beard"]=4, ["Eyebrows"]=4, ["Hairs"]=4, ["Mustache"]=4, ["Whiskers"]=4}, hair={["Eyebrows"]="SK_Eyebrows_Female_01", ["Hairs"]="SK_Hair_PartialDreadlocks_01_VARIANT_Female"}, eyeColor="14", clothes={["Feets"]="SK_Armor_Blackbeard_Sailor_03_Feet_Long", ["Legs"]="SK_Armor_Underwear_02_Female_Legs", ["Torso"]="SK_Armor_Underwear_01_Female_Torso", ["Waist"]="SK_Armor_Bandit_Female_Waist"}, skinTone="MI_Adventurer_Female_Large", belts={["Belt"]="SK_Belt_01_Female", ["Frog"]="SK_Frog_01_Female"} },
  ["/Game/Gameplay/Character/AI/NPC/Handyman/Handyman_Hunter/Params/Customization/DA_NPC_Handyman_Hunter_CompositeMeshData.DA_NPC_Handyman_Hunter_CompositeMeshData"] = { colors={["FEET"]={"23","2","4"}, ["HANDS"]={"4","4","4"}, ["HAT"]={"4","8","3"}, ["LEGS"]={"5","2","2"}, ["TORSO"]={"8","23","3"}, ["WAIST"]={"3","3","14"}}, physique="Medium", hairColors={["Beard"]=2, ["Eyebrows"]=2, ["Hairs"]=2, ["Mustache"]=2, ["Whiskers"]=2}, eyeColor="14", clothes={["Feets"]="SK_Armor_Blackbeard_Musketeer_02_Feet", ["Hands"]="SK_Armor_Blackbeard_Musketeer_02_Hands_Long", ["Legs"]="SK_Armor_Blackbeard_Sergeant_01_Legs_Long", ["Torso"]="SK_Armor_Blackbeard_Musketeer_01_Torso"}, skinTone="MI_African_Male_Medium", belts={["Belt"]="SK_Belt_01_Male", ["Frog"]="SK_Frog_01_Male", ["Sling"]="SK_Sling_01_Male", ["Strap"]="SK_Strap_01_Male"} },
  ["/Game/Gameplay/Character/AI/NPC/Handyman/Handyman_Miner/Params/Customization/DA_NPC_Handyman_Miner_CompositeMeshComponentParams.DA_NPC_Handyman_Miner_CompositeMeshComponentParams"] = { colors={["FEET"]={"2","6","8"}, ["HANDS"]={"13","9","6"}, ["HAT"]={"5","21","3"}, ["LEGS"]={"16","23","16"}, ["TORSO"]={"11","8","14"}, ["WAIST"]={"16","2","2"}}, physique="Medium", hairColors={["Beard"]=0, ["Eyebrows"]=0, ["Hairs"]=0, ["Mustache"]=0, ["Whiskers"]=0}, hair={["Beard"]="SK_Beard_Hungover", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Mustache"]="SK_Mustache_Shag_03", ["Whiskers"]="SK_Whiskers_Shag_03"}, eyeColor="2", clothes={["Feets"]="SK_Armor_Blackbeard_Sailor_03_Feet_Long", ["Headgear"]="SK_Armor_Mercenary_Male_Headband", ["Legs"]="SK_Armor_Dogface_03_Male_Legs", ["Waist"]="SK_Armor_Mercenary_Male_Waist"}, skinTone="MI_Scum_Male_Medium" },
  ["/Game/Gameplay/Character/AI/NPC/Handyman/Handyman_Woodman/Params/Customization/DA_NPC_Handyman_Woodman_CompositeMeshComponentParams.DA_NPC_Handyman_Woodman_CompositeMeshComponentParams"] = { colors={["FEET"]={"2","6","8"}, ["HANDS"]={"13","9","6"}, ["HAT"]={"5","21","3"}, ["LEGS"]={"2","23","16"}, ["TORSO"]={"9","16","3"}, ["WAIST"]={"16","2","2"}}, physique="Medium", hairColors={["Beard"]=5, ["Eyebrows"]=5, ["Hairs"]=5, ["Mustache"]=5, ["Whiskers"]=5}, hair={["Beard"]="REMOVED", ["Eyebrows"]="REMOVED", ["Hairs"]="REMOVED", ["Mustache"]="REMOVED", ["Whiskers"]="REMOVED"}, eyeColor="2", clothes={["Feets"]="REMOVED", ["Legs"]="REMOVED", ["Torso"]="REMOVED"}, skinTone="MI_Scum_Male_Medium" },

  ["/Game/Gameplay/Character/AI/Bosses/Boatswain/Params/DA_Boss_Boatswain_CompositeMesh.DA_Boss_Boatswain_CompositeMesh"] = { colors={["FEET"]={"21","10","12"}, ["HANDS"]={"7","8","22"}, ["HAT"]={"14","21","4"}, ["LEGS"]={"12","18","0"}, ["TORSO"]={"23","0","4"}, ["WAIST"]={"6","23","19"}}, physique="Medium", hairColors={["Beard"]=2, ["Eyebrows"]=2, ["Hairs"]=2, ["Mustache"]=2, ["Whiskers"]=2}, eyeColor="19", clothes={["Feets"]="SK_ArmorHeavy_CoastJungle_Feet_Long", ["Headgear"]="SK_ArmorHeavy_CoastJungle_Head", ["Legs"]="SK_ArmorHeavy_CoastJungle_Legs", ["Torso"]="SK_ArmorHeavy_CoastJungle_Torso_Long"}, skinTone="MI_AfricanHeavy_Male_Medium", belts={["Belt"]="SK_HeavyBelt_01"}, pose="A_Boss_Boatswain_Attack_Idle" },

  ["/Game/Gameplay/Character/AI/NPC/FactionActors/BrethrenOfTheCoast/CompositeMesh/Common/DA_NPC_AnimatedActor_BotC_Common_Female_01_CompositeMeshComponentParams.DA_NPC_AnimatedActor_BotC_Common_Female_01_CompositeMeshComponentParams"] = { colors={["CAPE"]={"23","0","19"}, ["FEET"]={"22","4","2"}, ["HANDS"]={"2","3","2"}, ["HAT"]={"22","4","3"}, ["LEGS"]={"2","2","2"}, ["MASK"]={"4","19","19"}, ["TORSO"]={"8","5","6"}, ["WAIST"]={"6","6","6"}}, hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=5, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Eyebrows"]="SK_Eyebrows_Female_04", ["Hairs"]="SK_Hair_Wavy_01_VARIANT_Female"}, eyeColor="6", clothes={["Feets"]="SK_Armor_Jeweler_03_Female_Feet_Long", ["Hands"]="SK_Armor_Flibustier_02_Female_Hands_Long", ["Headgear"]="SK_Armor_Jeweler_02_Female_Bandana_02", ["Legs"]="SK_Armor_Flibustier_02_Female_Legs", ["Torso"]="SK_Armor_Flibustier_01_Female_Torso_Long"}, belts={["Belt"]="SK_Belt_03_Female", ["Frog"]="SK_Frog_03_Female", ["Sling"]="SK_Sling_03_Female"}, accessories={["beltSlot_01_lSocket"]="SM_Belt_Misc_Knife_04", ["soc_Sling01F"]="SM_Drop_PistolT02_04", ["soc_Sling04F"]="SM_Belt_Misc_Knife_01", ["soc_Strap01F"]="SM_Belt_Misc_Knife_01", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["soc_beltStrapB"]="SM_Belt_Misc_Knife_04", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Severe"}, pose="AM_Regular_Male_Hero_Axe_Armed_G1" },

  ["/Game/Gameplay/Character/AI/NPC/FactionActors/QuestAnimatedActor/CompositeMesh/DA_NPC_QuestStatic_BotC_JacquesArno_CompositeMeshComponentParams.DA_NPC_QuestStatic_BotC_JacquesArno_CompositeMeshComponentParams"] = { colors={["CAPE"]={"8","15","12"}, ["FEET"]={"4","5","5"}, ["HANDS"]={"2","2","2"}, ["HAT"]={"4","2","3"}, ["LEGS"]={"2","4","2"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"8","8","6"}, ["WAIST"]={"6","6","6"}}, physique="Small", hairColors={["Beard"]=0, ["Eyebrows"]=0, ["Hairs"]=0, ["Mustache"]=0, ["Whiskers"]=0}, hair={["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_Shag_02_VARIANT_Male", ["Mustache"]="SK_Mustaches_Hungover"}, eyeColor="6", clothes={["Feets"]="SK_Armor_Combatant_01_Feet", ["Headgear"]="SK_Armor_Mercenary_Male_Bandana", ["Legs"]="SK_Armor_Bandit_Male_Legs_Long", ["Torso"]="SK_Armor_Flibustier_03_Male_Torso"}, skinTone="MI_Fable_Male_Small", pose="A_Regular_Male_Hero_SleepOnGround_Loop" },

  ["/Game/Gameplay/Character/AI/NPC/GalenSkelton/Params/Customization/DA_NPC_GalenSkelton_CompositeMeshComponentParams.DA_NPC_GalenSkelton_CompositeMeshComponentParams"] = { colors={["CAPE"]={"3","3","3"}, ["FEET"]={"21","9","4"}, ["HANDS"]={"3","3","3"}, ["HAT"]={"3","3","3"}, ["LEGS"]={"22","22","21"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"16","0","9"}, ["WAIST"]={"10","10","10"}}, hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Beard"]="SK_Beard_Hungover_GalenSkelton", ["Eyebrows"]="SK_Eyebrows_01_GalenSkelton", ["Hairs"]="SK_Undercut_01_VARIANT_GalenSkelton", ["Mustache"]="SK_Mustaches_Hungover_GalenSkelton", ["Whiskers"]="SK_Whiskers_Hungover_GalenSkelton"}, eyeColor="10", clothes={["Cape"]="SK_Armor_DrGalen_Cape", ["Feets"]="SK_Armor_DrGalen_Feet_Long", ["Legs"]="SK_Armor_DrGalen_Legs", ["Torso"]="SK_Armor_DrGalen_Torso_Long"}, belts={["Belt"]="SK_Belt_01_Male", ["Frog"]="SK_Frog_01_Male"}, accessories={["soc_Strap01F"]="SM_Belt_Misc_BagDrGalen_01", ["soc_beltSlingB"]="SM_Belt_Misc_Knife_01", ["soc_beltStrapB"]="SM_Belt_Misc_BeltBag_01_Simple", ["swordSlot_lSocket"]="SK_SaberT02_01"}, pose="A_AnimatedActor_Regular_Male_Idle_SittingOnChair_For_Galen" },

  ["/Game/Gameplay/Character/AI/NPC/Handyman/Base/Params/DA_NPC_Handyman_CompositeMeshData.DA_NPC_Handyman_CompositeMeshData"] = { colors={["FEET"]={"4","22","17"}, ["HANDS"]={"6","4","17"}, ["HAT"]={"6","21","13"}, ["LEGS"]={"1","22","17"}, ["TORSO"]={"11","9","23"}, ["WAIST"]={"6","7","15"}}, physique="Small", hairColors={["Beard"]=7, ["Eyebrows"]=7, ["Hairs"]=7, ["Mustache"]=7, ["Whiskers"]=7}, hair={["Beard"]="SK_Beard_BlackSmith", ["Eyebrows"]="SK_Eyebrow_BlackSmith", ["Hairs"]="SK_Hair_Wick_01_VARIANT_Male", ["Mustache"]="SK_Mustache_BlackSmith"}, eyeColor="15", clothes={["Feets"]="SK_Armor_BlackSmith_Feet", ["Hands"]="SK_Armor_BlackSmith_Hands", ["Legs"]="SK_Armor_BlackSmith_Legs"}, skinTone="MI_Adventurer_Male_Small" },

  ["/Game/Gameplay/Character/AI/NPC/Ksant/Params/Customization/DA_NPC_Ksant_CompositeMeshComponentParams.DA_NPC_Ksant_CompositeMeshComponentParams"] = { colors={["CAPE"]={"15","14","0"}, ["FEET"]={"4","3","4"}, ["HANDS"]={"5","5","6"}, ["HAT"]={"3","3","3"}, ["LEGS"]={"4","4","4"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"3","4","7"}, ["WAIST"]={"7","7","7"}}, hairColors={["Beard"]=1, ["Eyebrows"]=1, ["Hairs"]=1, ["Mustache"]=1, ["Whiskers"]=1}, hair={["Beard"]="SK_Beard_Jag_01_Ksante", ["Eyebrows"]="SK_Eyebrows_01_Ksante", ["Hairs"]="SK_Hair_Wick_02_VARIANT_Ksante", ["Mustache"]="SK_Mustache_Jag_01_Ksante", ["Whiskers"]="SK_Whiskers_Jag_01_Ksante"}, eyeColor="7", clothes={["Cape"]="SK_Armor_Ksante_Cape_01", ["Feets"]="SK_Armor_Ksante_Feet_Long", ["Hands"]="SK_Armor_Ksante_Hands", ["Legs"]="SK_Armor_Ksante_Legs", ["Torso"]="SK_Armor_Ksante_Torso_Torn"}, belts={["Belt"]="SK_Belt_01_Male", ["Frog"]="SK_Frog_01_Male", ["Sling"]="SK_Sling_01_Male"}, accessories={["soc_Sling01F"]="SM_Belt_Misc_SkullMetal_Roped_01", ["soc_Strap01F"]="SM_Belt_Misc_Wallet_01", ["soc_beltSlingB"]="SM_Belt_Misc_Fan", ["soc_beltStrapB"]="SM_Belt_Misc_Book_01"}, pose="A_Regular_PrisonerStand2_Idle" },

  ["/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Buyer/Customization/DA_NPC_AnimatedActor_Buyer_01_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Buyer_01_CompositeMeshComponentParams"] = { colors={["CAPE"]={"22","17","19"}, ["FEET"]={"22","4","6"}, ["HANDS"]={"2","3","2"}, ["HAT"]={"22","2","3"}, ["LEGS"]={"2","6","2"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"8","5","6"}, ["WAIST"]={"6","6","6"}}, physique="Large", hairColors={["Beard"]=7, ["Eyebrows"]=7, ["Hairs"]=7, ["Mustache"]=7, ["Whiskers"]=7}, hair={["Beard"]="SK_Beard_Hungover", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_Afro_03_VARIANT_Male", ["Mustache"]="SK_Mustaches_Hungover", ["Whiskers"]="SK_Whiskers_Jag_01"}, eyeColor="6", clothes={["Feets"]="SK_Armor_Musketeer03_Feet_Long", ["Hands"]="SK_Armor_Flibustier_01_Male_Hands", ["Headgear"]="SK_Armor_Mercenary_Male_Bandana", ["Legs"]="SK_Armor_Musketeer02_Legs", ["Torso"]="SK_Armor_Dogface_02_Male_Torso"}, skinTone="MI_Albion_Male_Large", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male"}, accessories={["soc_Strap01F"]="SM_Belt_Misc_Knife_01", ["soc_belt02B_l"]="SM_Belt_Misc_Pouch_02", ["soc_beltB"]="SM_Belt_Misc_BeltBag_01_Simple", ["soc_beltStrapB"]="SM_Belt_Misc_Knife_02", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Vengeful"}, pose="A_AnimatedActor_Regular_Male_Idle_Standing_08" },

  ["/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Buyer/Customization/DA_NPC_AnimatedActor_Buyer_02_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Buyer_02_CompositeMeshComponentParams"] = { colors={["CAPE"]={"6","22","6"}, ["FEET"]={"23","23","2"}, ["HANDS"]={"3","3","3"}, ["HAT"]={"3","8","3"}, ["LEGS"]={"21","23","23"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"14","23","2"}, ["WAIST"]={"2","2","2"}}, physique="Medium", hairColors={["Beard"]=2, ["Eyebrows"]=2, ["Hairs"]=2, ["Mustache"]=2, ["Whiskers"]=2}, hair={["Eyebrows"]="SK_Eyebrows_Female_04", ["Hairs"]="SK_Hair_Bun_02_VARIANT_Female"}, eyeColor="2", clothes={["Feets"]="SK_Armor_Conquistador_01_Female_Feet_Long", ["Headgear"]="SK_Armor_Flibustier_02_Female_Headband", ["Legs"]="SK_Armor_Flibustier_03_Female_Legs", ["Torso"]="SK_Armor_Brigant_Female_Torso"}, skinTone="MI_Native_Female_Medium", belts={["Frog"]="SK_Frog_01_Female"}, accessories={["soc_Strap01F"]="SM_Belt_Misc_BeltBag_02_Powder_FR2", ["soc_beltSlingB"]="SM_Belt_Misc_Knife_01", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Forged"}, pose="A_Regular_Male_Vendor01" },

  ["/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Buyer/Customization/DA_NPC_AnimatedActor_Buyer_03_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Buyer_03_CompositeMeshComponentParams"] = { colors={["CAPE"]={"4","17","14"}, ["FEET"]={"4","23","7"}, ["HANDS"]={"2","2","2"}, ["HAT"]={"4","4","3"}, ["LEGS"]={"2","4","6"}, ["MASK"]={"0","0","0"}, ["TORSO"]={"8","4","6"}, ["WAIST"]={"7","7","7"}}, physique="Large", hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Beard"]="SK_Beard_Shag_04", ["Eyebrows"]="SK_Eyebrows_Male_02", ["Hairs"]="SK_Hair_Wavy_02_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_03", ["Whiskers"]="SK_Whiskers_Sparse_03"}, eyeColor="7", clothes={["Feets"]="SK_Armor_Flibustier_02_Male_Feet_Long", ["Headgear"]="SK_Armor_Jeweler_01_Male_BandanaHat_01", ["Legs"]="SK_Armor_Mercenary_Male_Legs", ["Torso"]="SK_Armor_Flibustier_05_Male_Torso"}, skinTone="MI_African_Male_Large", belts={["Belt"]="SK_Belt_03_Male", ["Frog"]="SK_Frog_03_Male"}, accessories={["beltSlot_01_lSocket"]="SM_Drop_WeaponRange_Pistol_Faithful", ["soc_Lantern"]="SM_Drop_WeaponRange_Pistol_Baneful", ["soc_Strap01F"]="SM_Drop_WeaponRange_Pistol_Blank", ["soc_belt02B_l"]="SM_Belt_Misc_Bandoleer_01_Tripple01", ["soc_beltB"]="SM_Belt_Misc_BeltBag_01_Simple", ["soc_beltStrapB"]="SM_Belt_Misc_Pouch_01", ["swordSlot_lSocket"]="SK_WeaponMelee_Saber_Reliable"}, pose="A_AnimatedActor_Regular_Male_Idle_Standing_05" },

  ["/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Buyer/Customization/DA_NPC_Merchant_Buyer_CompositeMeshComponentParams.DA_NPC_Merchant_Buyer_CompositeMeshComponentParams"] = { colors={["FEET"]={"4","2","4"}, ["HANDS"]={"8","7","10"}, ["HAT"]={"15","0","3"}, ["LEGS"]={"5","23","6"}, ["TORSO"]={"0","16","0"}, ["WAIST"]={"16","21","4"}}, physique="Medium", hairColors={["Beard"]=0, ["Eyebrows"]=0, ["Hairs"]=0, ["Mustache"]=0, ["Whiskers"]=0}, hair={["Beard"]="SK_Beard_Hungover", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_Slickback_VARIANT_Male", ["Whiskers"]="SK_Whiskers_Shag_03"}, eyeColor="4", clothes={["Feets"]="SK_Armor_Bandit_Male_Feet_Long", ["Hands"]="SK_Armor_Starter_Male_Hands_Long", ["Headgear"]="SK_Armor_Blackbeard_Musketeer_01_Hat", ["Legs"]="SK_Armor_Mercenary_Male_Legs", ["Torso"]="SK_Armor_Blackbeard_Musketeer_02_Torso"}, skinTone="MI_Albion_Male_Medium", belts={["Belt"]="SK_Belt_01_Male", ["Frog"]="SK_Frog_01_Male", ["Sling"]="SK_Sling_01_Male"}, pose="A_Regular_Blacksmith_Attackable" },

  ["/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_DeputyGovernor/Customization/DA_NPC_Merchant_DeputyGovernor_CompositeMeshComponentParams.DA_NPC_Merchant_DeputyGovernor_CompositeMeshComponentParams"] = { colors={["FEET"]={"5","4","4"}, ["HANDS"]={"3","3","3"}, ["HAT"]={"4","21","12"}, ["LEGS"]={"3","3","8"}, ["TORSO"]={"7","7","19"}}, physique="Large", hairColors={["Beard"]=6, ["Eyebrows"]=6, ["Hairs"]=6, ["Mustache"]=6, ["Whiskers"]=6}, hair={["Beard"]="SK_Beard_Shag_04", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_LayeredBobDecor_01_VARIANT_Male", ["Whiskers"]="SK_Whiskers_Shag_03"}, clothes={["Feets"]="SK_Armor_Vanilla_Male_Feet_Long", ["Headgear"]="SK_Armor_Bandit_Male_Hat", ["Legs"]="SK_Armor_Brigant_Male_Legs", ["Torso"]="SK_Armor_Mercenary_Male_Torso_Long"}, skinTone="MI_Albion_Male_Large", belts={["Belt"]="SK_Belt_01_Male", ["Frog"]="SK_Frog_01_Male", ["Sling"]="SK_Sling_01_Male", ["Strap"]="SK_Strap_01_Male"}, pose="A_Regular_Blacksmith_Attackable" },

  ["/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Employees/Customization/DA_NPC_Merchant_Employees_CompositeMeshComponentParams.DA_NPC_Merchant_Employees_CompositeMeshComponentParams"] = { colors={["FEET"]={"4","2","4"}, ["HANDS"]={"8","7","10"}, ["HAT"]={"16","23","3"}, ["LEGS"]={"5","23","6"}, ["TORSO"]={"0","16","0"}, ["WAIST"]={"8","21","4"}}, physique="Medium", hairColors={["Beard"]=0, ["Eyebrows"]=0, ["Hairs"]=0, ["Mustache"]=0, ["Whiskers"]=0}, hair={["Beard"]="SK_Beard_Hungover", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_PartialDreadlocks_01_VARIANT_Male", ["Mustache"]="SK_Mustaches_Hungover", ["Whiskers"]="SK_Whiskers_Hungover"}, eyeColor="4", clothes={["Feets"]="SK_Armor_Blackbeard_Musketeer_02_Feet", ["Headgear"]="SK_Armor_Mercenary_Male_Headband", ["Legs"]="SK_Armor_Blackbeard_Sergeant_01_Legs_Long", ["Waist"]="SK_Armor_Restored_Male_Waist"}, skinTone="MI_African_Male_Medium", belts={["Belt"]="SK_Belt_01_Male", ["Frog"]="SK_Frog_01_Male", ["Sling"]="SK_Sling_01_Male", ["Strap"]="SK_Strap_01_Male"}, pose="A_Regular_Blacksmith_Attackable" },

  ["/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Generalist/Customization/DA_NPC_Merchant_Generalist_CompositeMeshComponentParams.DA_NPC_Merchant_Generalist_CompositeMeshComponentParams"] = { colors={["FEET"]={"3","10","4"}, ["HANDS"]={"4","3","10"}, ["HAT"]={"21","10","3"}, ["LEGS"]={"5","23","6"}, ["TORSO"]={"4","3","10"}, ["WAIST"]={"8","21","4"}}, physique="Small", hairColors={["Beard"]=5, ["Eyebrows"]=5, ["Hairs"]=5, ["Mustache"]=5, ["Whiskers"]=5}, hair={["Eyebrows"]="SK_Eyebrows_Female_01", ["Hairs"]="SK_Hair_PartialDreadlocks_01_VARIANT_Female"}, eyeColor="4", clothes={["Feets"]="SK_Armor_Mercenary_Female_Feet_Long", ["Hands"]="SK_Armor_Brigant_Female_Hands_Long", ["Headgear"]="SK_Armor_Mercenary_Female_Bandana", ["Legs"]="SK_Armor_Starter_Female_Legs", ["Torso"]="SK_Armor_Starter_Female_Torso"}, skinTone="MI_Adventurer_Female_Small", pose="A_Regular_Blacksmith_Attackable" },

  ["/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Herbalist/Customization/DA_NPC_Merchant_Herbalist_CompositeMeshComponentParams.DA_NPC_Merchant_Herbalist_CompositeMeshComponentParams"] = { colors={["FEET"]={"4","2","4"}, ["HANDS"]={"8","7","10"}, ["HAT"]={"21","0","3"}, ["LEGS"]={"5","23","6"}, ["TORSO"]={"0","16","0"}, ["WAIST"]={"8","21","4"}}, physique="Small", hairColors={["Beard"]=3, ["Eyebrows"]=3, ["Hairs"]=3, ["Mustache"]=3, ["Whiskers"]=3}, hair={["Beard"]="SK_Beard_Shag_02", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_Shag_02_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_04", ["Whiskers"]="SK_Whiskers_Shag_04"}, eyeColor="4", clothes={["Feets"]="SK_Armor_Blackbeard_Musketeer_01_Feet", ["Headgear"]="SK_Armor_Blackbeard_Sailor_01_Bandana", ["Legs"]="SK_Armor_Dogface_02_Male_Legs", ["Waist"]="SK_Armor_Bandit_Male_Waist"}, skinTone="MI_Adventurer_Male_Small", pose="A_Regular_Blacksmith_Attackable" },

  ["/Game/Gameplay/Character/AI/NPC/Merchant/Merchant_Ship/Customization/DA_NPC_Merchant_Ship_CompositeMeshComponentParams.DA_NPC_Merchant_Ship_CompositeMeshComponentParams"] = { colors={["FEET"]={"22","3","9"}, ["HANDS"]={"8","7","10"}, ["HAT"]={"4","2","2"}, ["LEGS"]={"3","4","3"}, ["TORSO"]={"10","9","9"}, ["WAIST"]={"16","21","4"}}, physique="Large", hairColors={["Beard"]=5, ["Eyebrows"]=5, ["Hairs"]=5, ["Mustache"]=5, ["Whiskers"]=5}, hair={["Beard"]="SK_Beard_Shag_02", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_ShortBob_VARIANT_Male", ["Mustache"]="SK_Mustaches_Hungover", ["Whiskers"]="SK_Whiskers_Shag_03"}, eyeColor="4", clothes={["Feets"]="SK_Armor_Brigant_Male_Feet_Long", ["Headgear"]="SK_Armor_Blackbeard_Sergeant_01_Hat", ["Legs"]="SK_Armor_Restored_Male_Legs", ["Torso"]="SK_Armor_Blackbeard_Musketeer_02_Torso"}, skinTone="MI_Albion_Male_Large", pose="A_Regular_Blacksmith_Attackable" },

  ["/Game/Gameplay/Character/AI/NPC/Prisoners/CompositeMesh/DA_NPC_AnimatedActor_Prisoner_01_CompositeMeshComponentParams.DA_NPC_AnimatedActor_Prisoner_01_CompositeMeshComponentParams"] = { colors={["CAPE"]={"23","3","6"}, ["FEET"]={"22","4","6"}, ["HANDS"]={"2","3","2"}, ["HAT"]={"22","2","3"}, ["LEGS"]={"2","2","2"}, ["MASK"]={"4","19","19"}, ["TORSO"]={"8","5","6"}, ["WAIST"]={"6","6","6"}}, physique="Medium", hairColors={["Beard"]=4, ["Eyebrows"]=4, ["Hairs"]=4, ["Mustache"]=4, ["Whiskers"]=4}, hair={["Beard"]="SK_Beard_Shag_04", ["Eyebrows"]="SK_Eyebrows_Male_04", ["Hairs"]="SK_Hair_Shag_03_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_02", ["Whiskers"]="SK_Whiskers_Shag_05"}, eyeColor="6", clothes={["Headgear"]="SK_Armor_Dogface_01_Male_Head", ["Legs"]="SK_Armor_Dogface_02_Male_Legs", ["Waist"]="SK_Armor_Bandit_Male_Waist"}, skinTone="MI_Scum_Male_Medium", pose="A_Regular_Blacksmith_Attackable" },

  ["/Game/Gameplay/Character/AI/NPC/RecruitTortuga/Params/Customization/DA_NPC_Alchemist_CompositeMeshComponentParams.DA_NPC_Alchemist_CompositeMeshComponentParams"] = { colors={["FEET"]={"4","2","4"}, ["HANDS"]={"4","4","4"}, ["HAT"]={"4","4","3"}, ["LEGS"]={"2","15","15"}, ["TORSO"]={"8","10","15"}, ["WAIST"]={"6","18","13"}}, physique="Small", hairColors={["Beard"]=2, ["Eyebrows"]=2, ["Hairs"]=2, ["Mustache"]=2, ["Whiskers"]=2}, hair={["Eyebrows"]="SK_Eyebrows_Female_01", ["Hairs"]="SK_Hair_Shag_03_VARIANT_Female"}, eyeColor="13", clothes={["Feets"]="SK_Armor_Blackbeard_Sailor_03_Feet_Long", ["Headgear"]="SK_Armor_Mercenary_Female_Headband", ["Legs"]="SK_Armor_Mercenary_Female_Legs", ["Torso"]="SK_Armor_Restored_Female_Torso"}, skinTone="MI_Adventurer_Female_Small", belts={["Belt"]="SK_Belt_01_Female"} },

  ["/Game/Gameplay/Character/AI/NPC/RecruitTortuga/Params/Customization/DA_NPC_Canoneer_CompositeMeshComponentParams.DA_NPC_Canoneer_CompositeMeshComponentParams"] = { colors={["FEET"]={"23","14","23"}, ["HANDS"]={"21","21","2"}, ["HAT"]={"4","14","3"}, ["LEGS"]={"2","23","6"}, ["TORSO"]={"1","2","2"}, ["WAIST"]={"21","5","16"}}, physique="Medium", hairColors={["Beard"]=0, ["Eyebrows"]=0, ["Hairs"]=0, ["Mustache"]=0, ["Whiskers"]=0}, hair={["Beard"]="SK_Beard_Shag_05", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_LayeredBob_01_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_05", ["Whiskers"]="SK_Whiskers_Shag_05"}, eyeColor="16", clothes={["Feets"]="SK_Armor_Blackbeard_Musketeer_03_Feet", ["Hands"]="SK_Armor_Blackbeard_Sergeant_02_Hands_Long", ["Headgear"]="SK_Armor_Mercenary_Male_Bandana", ["Legs"]="SK_Armor_Dogface_01_Male_Legs", ["Waist"]="SK_Armor_Bandit_Male_Waist"}, skinTone="MI_Scum_Male_Medium", belts={["Belt"]="SK_Belt_01_Male", ["Frog"]="SK_Frog_01_Male", ["Sling"]="SK_Sling_01_Male"} },

  ["/Game/Gameplay/Character/AI/NPC/RecruitTortuga/Params/Customization/DA_NPC_Woodcutter_CompositeMeshComponentParams.DA_NPC_Woodcutter_CompositeMeshComponentParams"] = { colors={["FEET"]={"23","21","4"}, ["HANDS"]={"4","4","4"}, ["HAT"]={"3","8","3"}, ["LEGS"]={"2","2","6"}, ["TORSO"]={"15","15","2"}, ["WAIST"]={"9","22","20"}}, physique="Medium", hairColors={["Beard"]=5, ["Eyebrows"]=5, ["Hairs"]=5, ["Mustache"]=5, ["Whiskers"]=5}, hair={["Beard"]="SK_Beard_Hungover", ["Eyebrows"]="SK_Eyebrows_Male_01", ["Hairs"]="SK_Hair_Shag_02_VARIANT_Male", ["Mustache"]="SK_Mustache_Shag_02", ["Whiskers"]="SK_Whiskers_Shag_04"}, eyeColor="20", clothes={["Feets"]="SK_Armor_Restored_Male_Feet_Long", ["Hands"]="SK_Armor_Blackbeard_Musketeer_01_Hands_Long", ["Headgear"]="SK_Armor_Blackbeard_Sailor_01_Bandana", ["Legs"]="SK_Armor_Blackbeard_Sergeant_03_Legs", ["Torso"]="SK_Armor_Dogface_01_Male_Torso"}, skinTone="MI_Native_Male_Medium" },
}
