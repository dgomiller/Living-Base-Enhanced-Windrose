...
-- Spawner.ShieldStructure(actor) — make one building block invulnerable. Building damage runs through a
-- GameplayEffectProxy/damageable component gated by the actor's bCanBeDamaged flag (same flag that makes
-- our set-dressing NPCs immortal) and the building's IsDamageable() check. Flip both: clear bCanBeDamaged
-- AND set the shared BuildingItem's bIsDamageable=false. Runtime-only (resets on reload), so no save risk.
--------------------------------------------------------------------
...
-- unshielded until the next raid) and we never re-scan the whole UObject list to catch them. Hooking the
