...
### 19bf. Ghost-state save/restore, a RestoreCustomState concurrency race, and the real fix for wrong-sex-clothes-on-restore Barbies (2026-09-22)

Three related restore-path bugs found and fixed the same session. **(1) Ghost state never survived a reload.** RedFalcon: "i dont think saving custom tracks if someone is changed to ghost." `Spawner.MakeGhost` is a pure one-way material swap with no other readable state anywhere, so `Spawner.BuildCustomStateLines`/`Spawner.RestoreCustomState` had no way to know an actor was ghosted. Fixed by having `MakeGhost` set `e.isGhost = true` on the actor's own `Spawner.spawned` entry (the same entry `BuildCustomStateLines` already scans), emitting a `GHOST:1` line on save and replaying it via `Spawner.MakeGhost(say, true, actor)` on restore -- only ever written true (there's no "un-ghost" action to diff against), so a missing line correctly means "not ghosted," matching every other category's convention. `MakeGhost` gained an `actorOverride` param (via the existing `resolveTargetOrActor` helper, not the old lockedTarget-bypass path) so restore can call it directly on a specific actor.

**(2) Restoring many customized actors together corrupted their outfits.** RedFalcon: "so i have a lot of custom people and i think too many were trying to reconfigure at once and some women ended up topless and some men ended up with womens clothes." Root cause in `Spawner.RestoreCustomState`: the next actor's own step used to be scheduled via `ExecuteWithDelay` immediately after the current actor's apply-lines work was merely QUEUED via `ExecuteInGameThread`, not after it actually ran -- a fixed real-world timer racing independently against a queued game-thread callback whose true execution timing this function never controlled. Under load, that let two actors' own composite-rebuild-triggering apply calls land on the same or adjacent ticks -- the exact "touching two composites' builds concurrently" hazard `Spawner.RunSerialized` exists elsewhere in this file to prevent, just never applied here. Fixed by scheduling the next actor's step from INSIDE the current actor's own `ExecuteInGameThread` callback, after its lines have genuinely finished running, not from outside it.

**(3) The REAL wrong-sex-clothes-on-restore bug, separate from (2) and confirmed by RedFalcon as restore-specific** ("miner F African is spawning with mens clothes and Mercer M Native is spawning in wome's clothes" -- "a fresh spawn is correct"). Traced by reading the two spawn paths side by side: a fresh Barbie spawn goes through `Spawner.SwapBodyType`, which sets up `compositeLook` (archetype/params/sex/bodyTypes) BEFORE spawning, then -- critically -- calls the local `pollForBuildThenApplyBodySwap` AFTER the composite build finishes to do the REAL correction (swap `DefaultParams` to the matching `DA_Custom_BarbieDefaultParams_Regular_<Sex>` outfit, then the real `comp:SwapBodySex()` native rebuild). `restoreOne` calls `Spawner.Spawn` with the same pre-build `compositeLook` fields (the saved `sex`/`bodyTypes` data was never wrong) but never ran that post-build correction, so the saved sex flag never actually took on the rebuilt actor. Fixed by exposing `pollForBuildThenApplyBodySwap` as `Spawner.RestoreApplySexCorrection` (a Spawner-table field, NOT a new file-level local -- this file is already at Lua's 200-local ceiling and `restoreOne` sits textually above the function's own definition, so a bare local would hit this project's own forward-reference trap; a table index resolves at call time, so declaration order doesn't matter) and calling it from `restoreOne`'s success branch, gated on `look.bodyTypes` being present (not just `look.sex`) so it only ever fires for the real Barbie/bodyTypesOverride flow, leaving other reskin mechanisms (e.g. the Senkamati crew, which also sets `compositeLook.sex` but never `bodyTypes`) untouched. `currentSex` is read LIVE off the just-spawned actor (same as `SwapBodyType`'s own `doSpawnNow`), so a same-sex Barbie correctly skips the `SwapBodySex()` rebuild instead of running it unconditionally -- no new crash exposure on top of the existing, already-flagged cross-sex rebuild risk.

**Files touched**: `spawner.lua` (`Spawner.MakeGhost`'s `actorOverride` param + `e.isGhost` write; `Spawner.BuildCustomStateLines`'/`CS.applyLine`'s GHOST line; `Spawner.RestoreCustomState`'s scheduling-order fix; `pollForBuildThenApplyBodySwap` exposed as `Spawner.RestoreApplySexCorrection`; `restoreOne`'s new post-spawn correction call).

### 19bg. Resource-node decor (shipwrecks, boulders, craft-station campfires) could still be mined/chopped/camera-blocked despite existing invulnerability code -- two separate native-component discoveries, one dead-end abandoned after it froze the game (2026-09-23/24)

RedFalcon: "some of the decor can be broken or chopped down like trees and ship wrecks" and, separately, "when I get near them it acts like its blocking the camera and i lose view of my character" for craft-station props (Kiln/Furnace/Cooking Station). Both turned out to be the SAME class of bug: `makeSetDressing`'s `bCanBeDamaged=false` and `Spawner.StripInteraction` are both ACTOR-level or interaction-component-level protections, but the real mechanics live on OTHER, independent native components neither of them touches.

**Mining/salvage damage.** A new `Spawner.DumpAllComponents` probe helper (walks every attached `ActorComponent` via the same `K2_GetComponentsByClass`+`RemoteUnrealParam` unwrap pattern `stripComponentsOfClass` already proved safe, calling the existing `dumpObjectProperties` on each -- wired into `lbprobedump`'s own output) found that shipwrecks and mineral-node boulders both carry one or more `R5MineralNodeClusterComponent` children, each with its OWN independent `Health` float (confirmed 120.0 on a fresh boulder) that a pick/axe hit decrements directly -- completely bypassing the actor's own damage system. Fixed with `Spawner.ProtectResourceNodeHealth(actor)`, called from `Spawner.Spawn`'s non-combatant set-dressing block, forcing every such cluster's `Health` to 999999.0 right after spawn.

**Camera occlusion from a small, harmless-looking fire.** `Spawner.DumpAllComponents` was extended to also drill each component's `BodyInstance.CollisionProfileName`/`CollisionEnabled`/`ObjectType` (previously only printed as an opaque `ScriptStruct` -- same struct-drill technique as `R5SegmentTreeSpec` below, reused on `/Script/Engine.BodyInstance`). Every Kiln/Furnace/Cooking Station carries an `R5HearthVolumeComponent` ("R5HearthVolume_T3", a `CapsuleComponent` sized to the fire's WARMTH radius -- several meters, much larger than the visible mesh) with `CollisionEnabled=1` (QueryOnly). QueryOnly doesn't physically push the player, but it DOES respond to trace queries -- and a third-person camera's own collision-avoidance check is exactly that, a query. Confirmed live: a fresh scan of a small campfire-style prop showed the same component/values, and the symptom (camera pulling in ~3m away from a small object) matched exactly. Fixed with `Spawner.DisableHearthVolumeCollision(actor)` (same Spawner.Spawn hook point), calling the real `SetCollisionEnabled(0)` UFUNCTION on the component (not a direct `BodyInstance` sub-field write -- struct sub-field writes on a live component have proven unreliable elsewhere in this file, see 19bh below) with a readback to confirm it actually took. This gameplay-warmth trigger isn't needed on a placed decor prop anyway, matching this project's existing "placed items are pure set-dressing" precedent (`Spawner.StripInteraction`).

**Trees are a separate, unsolved case -- DO NOT re-attempt the struct-manipulation approach.** A `R5SegmentTree` (the palm/tree decor family) has no plain numeric Health anywhere; it tracks chop progress via `Spec` (an `R5SegmentTreeSpec` struct VALUE) and `ActiveSegmentInterval` (a plain `FInt32Interval`, read as `{min=0, max=8}` against 9 total `Segments` on a fresh tree -- read via the same struct-type-resolve-then-`ForEachProperty` drill as `CustomizationRecordID` before it, added as `Spawner.DumpTreeChopState`). Two live write attempts to collapse/expand this interval were both genuinely dangerous: an INVERTED range (`min=999999, max=-999999`) instakilled the tree on the very first hit (the width apparently reads as a live "hits remaining" count, and an inverted range overflowed to something the game treated as already-fully-depleted); a non-inverted but oversized range (`min` unchanged, `max=999999`) instead FROZE THE ENTIRE GAME, reproducibly, on a clean restart -- most likely a native loop iterating the segment range that never expected six-figure values. `Spawner.TestFreezeTreeChop` is left in the codebase disabled (prints a warning instead of touching anything) purely as a documented dead end, matching this project's other "DO NOT re-attempt" precedents. RedFalcon's call after seeing both failure modes: not worth the risk for a cosmetic/anti-cheat concern -- "if someone wants to use it to cheat, its their game so be it." A real fix, if ever revisited, needs a fundamentally different lever (e.g. the tree's own `Collision` component's response to whichever trace channel the chop hit-detection actually uses), not further struct manipulation.

**Files touched**: `spawner.lua` (`Spawner.DumpAllComponents` + its `BodyInstance` drill, `Spawner.DumpTreeChopState`, `Spawner.TestFreezeTreeChop` [disabled], `Spawner.ProtectResourceNodeHealth`, `Spawner.DisableHearthVolumeCollision`, all wired into `Spawner.Spawn`'s set-dressing block and/or `probeDumpPropertiesBody`); `main.lua` (`lbtestfreezetree` console command, now a no-op).

### 19bh. Restore-load visibility: per-phase progress toasts, and a `RestoreCustomState` pacing knob split off from the movers' own cadence (2026-09-24)

RedFalcon: "it would be nice to have a status update for the progress of the load. like 19/25... with larger bases it obviously takes longer to load," refined a turn later to "i'd really like some status of the various steps. like when its processing movers, how many are done, etc. less of a black box." First pass added a single combined counter across `Spawner.RestoreFromPersist`'s statics+movers phases; revised on the spot into a `phaseLabel` parameter threaded through the shared `spawnList()` helper so each phase (`"Restoring decor/statues"`, `"Restoring movers"`) reports its own `X/Y` independently, plus a per-item toast added to `scheduleRestorePostProcess` (`"Post-processing movers: X/Y"`, previously ZERO feedback for a phase that can run many seconds on a big base) and to `Spawner.RestoreCustomState` (`"Restoring customizations: X/Y"`). The fast statics phase (as low as 40ms/item) is throttled to at most one toast/second (`Spawner.Toast` always ADDS a new stacking widget, it never updates one in place, so an untoasted per-item call would leave dozens on screen at once); the slower movers/post-process/customization phases (hundreds of ms per item) toast every item with no throttling needed.

**Separately, "i also think we can reduce the time to custom restores."** `Spawner.RestoreCustomState` was pacing at 350ms per customized actor by reusing `Config.RESTORE_STAGGER_MS` -- the SAME value used to pace spawning brand-new AI movers (wake/possess an AIController), copied over purely by analogy when the ordering race in 19bf above got fixed, never independently tuned for this lighter workload (no AI wake, just composite/clothing property writes on an already-built actor). Split into its own `Config.CUSTOM_RESTORE_STAGGER_MS` (200ms, between the cheap statics rate of 150 and the AI-pawn mover rate of 350 -- not a wild guess, a value already used elsewhere in this same file for a comparable-weight operation) so it can be tuned without touching the movers' proven-safe cadence. If a large base with many customized actors ever hitches/crashes on load after this, raise `CUSTOM_RESTORE_STAGGER_MS` back toward 350 first.

**Files touched**: `spawner.lua` (`spawnList`'s new `phaseLabel` param + throttled toast; `scheduleRestorePostProcess`'s new per-item toast; `Spawner.RestoreCustomState`'s toast + `Config.CUSTOM_RESTORE_STAGGER_MS` switch); `config.lua` (`Config.CUSTOM_RESTORE_STAGGER_MS = 200`, new).

### 19bi. Spawn-menu tree reorg: Ghost Pirate combined into one subcategory, Senkamati split three ways, both alphabetized -- a direct `spawn_menu.ini` edit, not a generator rewrite (2026-09-24)

RedFalcon wanted three changes to the Spawn tree, clarified over a couple of rounds (an initial screenshot used informal category notation -- e.g. "People > Senkamati > Upright" -- that didn't match the ini's real current paths, so the first pass was grounded against a fresh `grep` of the live `spawn_menu.ini` rather than trusting the screenshot literally): **(1)** `Monsterous > Standing > Ghost Pirate` and `Monsterous > Walkers > Ghost Pirate (Mobile)` (two separate subcategories, an artifact of the Idle/Mobile roster split) merged into one `Monsterous > Ghost Pirate` holding both, with the Idle entry's label changed to "Ghost Pirate (Idle)" to match the Mobile one's naming. **(2)** The `Senkamati` top-level category (three subcategories: `Plagued`/`Upright`/`Wild`, all already siblings under one node, not actually split across Monsterous/People as RedFalcon's own reference table implied) split apart on purpose: `Plagued` (RedFalcon's own "Corrupted" terminology) moved to `Monsterous > Corrupted Senkamati`; `Upright` and `Wild` moved to `People > Senkamati > Upright`/`People > Senkamati > Wild` respectively (44 entries total, `Mask On`/`Mask Off` sub-structure preserved). **(3)** All four touched groups alphabetized (a follow-up "small nitpick").

**Direct ini edit, confirmed safe by the file's own design.** `spawn_menu.ini`'s own header states re-running the generator only ADDS missing roster/index entries, never touches or removes anything already there -- and the generator's own merge logic (`parseByRosterIndex`, near the top of `spawnmenu_manifest.lua`) tracks "already generated" by `(roster, index)` PAIR, not by section path. This confirms renaming/reordering section headers is 100% safe from creating duplicates on the next regeneration, matching the file's own "reorganize/rename freely" comment -- exactly the same operation RedFalcon does by hand normally. Section RENAMES were done with a small one-off Python script (regex on the `[Old.Category.` prefix); ALPHABETIZING required actually reordering whole blocks (header + `label`/`roster`/`index` lines + trailing blank line) within the file, done by parsing the ini into header-delimited blocks, extracting the ones matching each target category prefix, sorting by full section path (case-insensitive -- this naturally sorts `Mask Off` before `Mask On` and each leaf name alphabetically within), and reinserting the sorted run at the position of the first matched block's original location.

**The underlying Lua generator was ALSO updated for Ghost Pirate specifically, so future regeneration stays consistent.** `spawnmenu_manifest.lua`'s `monsterous_standing_path_and_label` hardcoded `{"Monsterous", "Standing"}` for every row in that roster -- rather than changing that shared default (which could misplace any OTHER future `Config.MONSTEROUS_STANDING` row), it now checks for a `row.menuPath` override first, mirroring the mechanism `mobile_quest_npc_path_and_label` already uses for exactly this ("Sailor -> People.Walkers, Ghost Pirate -> Monsterous.Walkers"). `Config.MONSTEROUS_STANDING`'s Ghost Pirate row gained `menuPath = {"Monsterous", "Ghost Pirate"}` (plus the "(Idle)" label), and `Config.MOBILE_QUEST_NPCS`'s Ghost Pirate (Mobile) row's own existing `menuPath` was updated from `{"Monsterous", "Walkers"}` to match. The Senkamati generator (`senkamati_path_and_label`) was DELIBERATELY left untouched -- its own `sub` logic ("As Original"/"Mob Body"/"Crew Reskin(...)") doesn't match what the live ini's `Plagued`/`Upright`/`Wild` entries actually show (`Mask Off`/`Mask On`, no sub-segment for Plagued at all), meaning the function's own logic has already drifted from history at least once before; rewriting it blind risked a NEW, poorly-understood regression for zero benefit (existing entries never regenerate anyway) -- any future new Senkamati roster row just needs the same kind of manual ini touch-up RedFalcon already does for other one-off cases.

**Files touched**: `spawn_menu.ini` (both the live install and the Working copy, kept in sync -- section renames + alphabetizing, no roster/index/label data changed); `config.lua` (`Config.MONSTEROUS_STANDING`'s and `Config.MOBILE_QUEST_NPCS`' Ghost Pirate rows); `spawnmenu_manifest.lua` (`monsterous_standing_path_and_label`'s new `menuPath` check).

### 19bj. Two more decor rename/recategorize batches (`TreeChanges.xlsx`, `AdditonalTreeChanges.xlsx`) -- and a real bug: a literal `.` in a display name silently splits the tree (2026-09-24/25)

Same direct-`spawn_menu.ini`-edit technique as 19bi, applied to plain `Decor` entries this time (73 rows, then 11 more) -- rename a leaf's `label`, and/or move it to a new `[Decor.<Category>.<Sub>.<Leaf>]` section path, using each row's (roster=DECOR, index=N) pair looked up via the SAME flattening `spawnmenu_manifest.lua`/`main.lua` already compute (`DECOR_ORDER` then each category array's own order) to find the CURRENT block before rewriting it -- never touching `fkeys.lua`'s array order itself, only ini text. Alphabetized the whole `Decor` subtree afterward using the same "parse into header-delimited blocks, sort by full section path case-insensitive, reinsert" script as 19bi, generalized into a reusable tree-builder (pinned top-level order + pinned `People` subcategory order, everything else free to sort) that every later batch this session reused unchanged.

**Real bug caught by RedFalcon, not by testing beforehand**: two renamed trees ("Sabal Palm - 2.5m - Choppable" / "- 4.5m -") turned the tree into unwanted subcategories -- confirmed by re-parsing the live ini afterward and finding these were the ONLY two leaf names anywhere in the entire file containing a literal `.` in their last path segment. Root cause: the ini's tree hierarchy is built by SPLITTING the bracketed section path on `.` -- a period inside what's meant to be one leaf label is indistinguishable from a real category separator to that parser, so "Sabal Palm - 2.5m - Choppable" silently became two nested levels ("Sabal Palm - 2" / "5m - Choppable"). Fixed by renaming to whole-number "3m"/"5m" (RedFalcon: "just round up") -- **the general rule this proves: any decor/spawn-menu display label is implicitly forbidden from containing a literal `.`, with no validation anywhere to catch it before it silently corrupts the tree structure.** A second, smaller mistake in the same batch: I misread the spreadsheet's stated destination for "Clay Tablet 2" (`Decor > Storage > Bales`) as obviously wrong on sight and started to "fix" it before checking -- RedFalcon confirmed it as a genuine authoring mistake and asked for a revert, which is the same lesson as 19bi's own "ground the informal reference against the real live file" caution, just triggered by second-guessing a REAL cell value instead of a screenshot this time.

**Files touched**: `spawn_menu.ini` (both installs) -- section/label renames + full alphabetize, no roster/index changes; `fkeys.lua`'s own `label` fields updated to match (kept in sync for a future full ini regen, even though the currently-populated ini never reads them again).

### 19bk. The 848-row `NewItems.xlsx` batch: a real DECOR-index-shift bug caught mid-edit, and a silently-dropped 35-row block found only because RedFalcon insisted something was missing (2026-09-25)

RedFalcon: "I have a large list of new items... alphabetize everything except [pinned top-level/People order]." The sheet's own `Category`/`Passive`/free-text columns doubled as ad-hoc structured data across this whole session (see 19bl below for the `lbtestaianim` command column) -- always worth scanning EVERY column of an import sheet for embedded structure, not just the ones that look like data at a glance.

**Real bug, caught before it shipped: extending an EXISTING category array anywhere but the very end of `DECOR_ORDER` shifts every later category's flattening index.** First pass appended ~250 new items straight into their most-similar EXISTING `invdrop_*` category arrays (Meals, Tools, etc.) to land them under the matching `Decor.Drops.<Theme>` folder for free. This is exactly the reindexing hazard `project_spawn_menu_ini_stale_indices`'s own memory already named in the abstract ("any Config array growing anywhere but its own end desyncs an already-generated ini") -- but it wasn't caught until a **verification pass** (re-flattening the edited `fkeys.lua` and diffing the first 315 original indices' names against a known-good baseline) surfaced a name mismatch at index 320 onward. Root cause: `invdrop_potions`/`invdrop_tailoring` (both extended) sit BEFORE `invdrop_seeds`/others in `DECOR_ORDER`, so every category after either of them shifted. Fixed by reverting those 15 in-place extensions and instead giving each affected theme a brand-new trailing category (e.g. `new_drops_meals`) that maps to the SAME display folder (`{"Drops", "Meals"}`) via `DECOR_CATEGORY_LABELS` -- visually identical, but the new items get fresh indices at the true end of the flattening instead of shifting anyone. **Generalizable takeaway: after ANY batch edit to a Config array/DECOR_ORDER, re-flatten and diff the ORIGINAL rows' names-by-index against what they were before the edit -- don't just check the NEW rows landed correctly, since a silent mid-list insertion breaks everything AFTER it while looking completely correct in isolation.** (Also caught, same batch: two brand-new items independently resolved to the exact same display label as an EARLIER TreeChanges rename already in the ini -- "Weathered Crates - Lg 1"/"Iron Hammer" -- disambiguated with a "2" suffix; worth a final pass checking new leaf names against the FULL existing tree, not just against each other.)

**A second real bug, found only because RedFalcon pushed back on the result: "i feel like a lot of people are missing from the list... almost all the new merchants and named folks."** The initial ingestion filtered every row on `PAKFile` (column A) being non-null, on the assumption a blank PAKFile meant "not really in the game." Re-reading the FULL 883-row sheet (not just the 848 that passed the filter) found 35 rows with a blank PAKFile column but a fully-populated `With Class`/name/category -- real Crew/Merchant/Recruit/Named characters the original export tooling simply never filled that one column in for. **The lesson: a single blank metadata column is not proof an entry is invalid -- check whether the columns actually NEEDED for the task (here, the resolvable class path) are populated, not an unrelated one that merely usually correlates.** Added as `Config.NEW_PEOPLE`, spawned via the same pacified `spawnCreature`-style path as `Config.MONSTEROUS_MOBS` (see 19bl) rather than the composite-look reskin machinery `Config.FACTION_VISITOR_LOOKS` uses -- these are real, distinct character classes, not one shared base pawn reskinned, so the wrong-shape catalog would have silently done nothing or crashed.

**Also shipped wrong on the first pass, fixed same session (see 19bl for the mechanism): the new `Config.MONSTEROUS_MOBS` roster (Drowned/Zombie/Boss/Giant Shaker) went out fully hostile** (a plain unpacified `Spawner.Spawn`, deliberately chosen at the time to distinguish "real threat" mobs from decorative statues) -- RedFalcon's actual expectation, stated only after seeing it live ("none of the new monsterous are friendly"), was that EVERY new roster this batch introduced should be placeable set-dressing like everything else, matching `LIVESTOCK`'s own pacify treatment. Fixed by having the mob-spawn helper delegate straight to the existing `spawnCreature` (friendly-faction copy + `Config.CREATURE_INVINCIBLE`), and separately by adding `MONSTEROUS_MOBS`/`CRABS`/`NEW_PEOPLE` to `main.lua`'s roster whitelist for `Spawner.StartPlacementPreview` (the "drag it into place before it wanders off" feature) -- a SEPARATE gate from pacification that's easy to forget when a new roster is wired up, since a freshly-spawned actor can be perfectly pacified and still walk off before you can grab it if this whitelist doesn't also know its roster name.

**Files touched**: `fkeys.lua` (56 new trailing DECOR categories + 15 more "existing-theme, new-trailing-category" fixups, ~832 new entries total); `spawnmenu_manifest.lua` (`DECOR_CATEGORY_LABELS` additions, `monsterous_mobs_path_and_label`/`crabs_path_and_label`/`new_people_path_and_label`, roster registration); `config.lua` (`Config.MONSTEROUS_MOBS`, `Config.CRABS`, `Config.NEW_PEOPLE`); `testbed.lua` (`spawnMonsterousMob` now pacified, `Testbed.SpawnMonsterousMobByName`/`SpawnCrabByName`/`SpawnNewPersonByName`); `main.lua` (3 new `SPAWN_MENU_HANDLERS` entries, placement-preview whitelist additions, `Spawner.FriendlyLabels` loops); `spawn_menu.ini` (both installs, ~900 new entries total, full re-alphabetize).

### 19bl. A live probe's AIController/PathFollowingComponent is NOT proof an NPC walks -- confirmed by directly observing its NATIVE behavior, not by cross-referencing components; the real fix reuses the Mobile-Quest-NPC AI-forcing trick on a class that already has its OWN (unused) AI, plus a second freeze mechanism for classes where `StopLogic()` alone doesn't stick (2026-09-25)

RedFalcon probed a spawned "Merchant - General (F)" (`BP_NPC_Merchant_Generalist_C`) via the existing HOME-key `probeDefaultParams`-style dump and asked whether the class "can have walking AI." The dump showed `AutoPossessAI=3`, `AIControllerClass=BP_NPC_AIController_QuestStatic_C`, a LIVE-POSSESSED controller instance with a real `PathFollowingComponent`, `MercunaGroundNavigationComponent`, and `MaxWalkSpeed=600.0` -- read together, a reasonable-looking case that she should already walk. **She doesn't, natively or spawned -- confirmed by RedFalcon actually watching her in the wild, not by re-reading more properties.** Correction: a real AIController/nav/movement component stack being PRESENT on a class says only that the class supports SOME AI behavior (facing/tracking/dialogue for a stationary vendor, in this case) -- it says nothing about whether THIS PARTICULAR behavior (locomotion) is the one actually driving it. The only way to know is to watch the real, unmodified in-game behavior; component inventory is circumstantial evidence, not proof, in either direction.

**The fix worked anyway, for an unrelated, independently-confirmed reason**: this class is `Character`/`Pawn`-derived with genuine movement machinery, structurally unlike the `BP_AnimatedActor_*` family 19au (an earlier session) already proved CAN'T be forced to walk (no Controller/movement machinery at all, a hard structural wall). Forcing the SAME `Config.MOBILE_QUEST_NPC_AI_CONTROLLER`/`ANIM_CLASS` override `placeMobileQuestNPC` already uses for Marita/Letty (a DIFFERENT AIController -- `BP_NPC_AIController_Citizen_Walker_C`, not the class's own native `BP_NPC_AIController_QuestStatic_C` the probe showed -- confirming the two are unrelated) onto this Merchant class via a plain `Spawner.Spawn(path, label, spot, nil, aiController, yaw, true, nil, nil, false, animClass)` call **worked, confirmed live ("yup she's walking")**. Shipped as a per-`Config.NEW_PEOPLE`-row `mobileOverride = true` flag (`spawnMobileNewPerson` in testbed.lua) rather than a blanket change, since RedFalcon's own next words were "we'll likely want to take it case by case, as i'm pretty sure some are statues" -- component presence not being proof of behavior cuts both ways, so every other entry in the roster needs the same live-observation check before assuming either outcome. Split into a separate "(Mobile)" sibling row (own `name`, same `path`) rather than a flag toggle on the original, so the plain/default Standing spawn keeps working unchanged -- same pattern as `Config.MONSTEROUS_STANDING`/`MOBILE_QUEST_NPCS`'s existing Idle/Mobile pairs.

**A general-purpose test command already existed for exactly this kind of check and is worth remembering**: `lbtestaianim <ClassPath> <AIControllerClassPath> <AnimBlueprintClassPath> [friendly: 1/0]` -- the same command originally used to prove the Mobile Quest NPC family walkable, reusable on ANY class path before committing it to a roster. RedFalcon's own follow-up "New Walkers.xlsx" batch (16 more entries, discovered walkable this same way) even carries the exact ready-to-run `lbtestaianim` command as its own spreadsheet column, one row per candidate -- worth checking a new import sheet for a command-string column like this before writing test commands by hand.

**Reverse direction, discovered the same session: naturally-walking characters (real Mob/Boss AI classes, not forced) don't reliably stop via the standard freeze mechanism.** Asked to add frozen "(Idle)" siblings for three `NEW_PEOPLE` Named characters that already walk on their own (Marlowe, Steed Bonet, Thomas Richards -- a Boss class), a single `Spawner.SetAILogic(actor, false)` call right after spawn worked for Marlowe but not the other two. Root cause, by analogy with `lbspawnnoai`'s own header comment (which already warns a same-frame `SetAILogic` call can silently fail to find a valid AIController yet): **first fix** was the SAME repeated-retry pattern this file already uses for other late-attaching-AI races (`disarmRepeated`/`pacifyCreature`'s own "re-applied over ~3s, guarded by `stillAlive`" convention) -- this alone fixed nothing further for Steed Bonet/Thomas Richards, meaning their case wasn't a timing race. **Second fix**, confirmed working: additionally zero `CharacterMovement.MaxWalkSpeed` directly and call `StopMovementImmediately()` -- a Boss class's movement is apparently driven by something (a StateTree, or Blueprint-level tick) that doesn't fully respect the AIController's own `StopLogic()`, but a movement component with zero speed can't produce visible motion regardless of what's issuing move requests. **Both fixes are now layered together in every "(Idle)" spawn path (`Config.MONSTEROUS_MOBS`' own new Idle/Mobile split reuses the identical helper)** -- retry the AI-logic stop for the timing-race case, ALSO zero movement speed directly for the "something else is driving it" case, since there was no cheap way to know in advance which one a given class needed.

**Files touched**: `testbed.lua` (`spawnMobileNewPerson`, `spawnFrozenIdleNewPerson` -- moved above its first call site to avoid this file's own forward-reference trap, see `feedback_lua_forward_reference_check` -- and its repeated-retry + `CharacterMovement` zeroing); `config.lua` (`Config.NEW_PEOPLE`'s `mobileOverride`/`idlePose` fields on affected rows including the 16-entry New Walkers batch and 3 new "(Idle)" Named rows; `Config.MONSTEROUS_MOBS` split into 8 "(Mobile)"/8 "(Idle)" row pairs, Mobile rows kept at their ORIGINAL array positions and Idle rows appended at the end to avoid the exact `DECOR_ORDER`-style reindex risk in 19bk -- this roster isn't `DECOR` but the SAME "array position determines ini index" mechanism applies to every roster, not just decor).

### 19bm. `npc_preset_scan.txt` is NOT a cumulative history -- regenerating `Config.NPC_CUSTOM_BASELINE` from it wholesale would have silently deleted most of a multi-session baseline (2026-09-25)

After a round of `probeDefaultParams` (HOME key) scans on the new Merchant/Recruit/Named archetypes, running `gen_npc_customization_reference.py` produced only 52 baseline entries -- alarming next to the 265 already accumulated in `config.lua` across many earlier sessions. Before overwriting anything, diffed the freshly-generated file's keys (the archetype's own full `DefaultParams` asset path) against `config.lua`'s existing `Config.NPC_CUSTOM_BASELINE` keys: 33 already matched, 19 were genuinely new. **Root cause**: `npc_preset_scan.txt` is `Spawner.PresetDB`'s own on-disk store, keyed by label -- it holds whatever's been scanned RECENTLY/this-session, not an ever-growing log of every scan ever performed; a full session's worth of NEW scans plus old survivors from a prior session happened to total only 64 unique labels this time, nowhere near the full historical set. The generator script's own header already says as much ("paste its contents into config.lua by hand... matching gen_clothes_lua.py/gen_socketitems_lua.py's own convention of generating a block to review before merging, not overwriting directly") -- but it's an easy detail to miss under the assumption that "the scan file" means "every scan ever." **Fixed by keying the merge on the archetype path** (extracted each of the 19 new entries' own text block out of the generated file and spliced them into `config.lua`'s existing table, leaving the other 33 -- and everything not re-scanned this session -- untouched). Classification (`Config.NPC_LOOK_CLASSIFICATION`) needed no merge this time since it's driven by `Other/randomized.xlsx` instead, a genuinely static source that didn't change.

**Files touched**: `config.lua` (`Config.NPC_CUSTOM_BASELINE`, 19 new archetype entries spliced in, 33 existing ones untouched); `npc_preset_scan.txt` (Working dev copy refreshed from the live install's copy before regenerating, since the generator reads whichever copy sits next to it, not the live one directly).

### 19bn. Hunting for missing Custom-tab dropdown options: a self-inflicted false-positive lesson, a second/more-complete clothes catalog nobody had cross-checked before, and a real stale-persistent-file bug in the Senkamati "Only with Unlock" gate (2026-09-25)

Asked to check whether any clothes/hair mesh names captured in the 19bm baseline scan were missing from the Custom-tab dropdowns. First pass flagged ~10 "missing" hairstyles and a few "missing" armor pieces -- **all false positives, entirely self-inflicted.** The report substituted a generic `_VARIANT_` placeholder for hair filenames' real middle segment instead of checking the literal string (the real assets use `_Default_`/`_Suspend...`/etc, never literally `_VARIANT_`), so the exact-string comparison against the catalog could never match even though every one of those hairstyles was already present. **The general lesson, worth remembering for any future "compare captured data against a catalog" task: always grep the LITERAL string from the real source data (`grep -o "SK_Hair_Afro_03[A-Za-z0-9_]*"` style), never reconstruct or template a name and compare against that reconstruction** -- a template that's wrong in one small way produces a confident, fully-populated, entirely false report. Separately, several "missing" armor pieces (Blackbeard Sergeant's whole outfit, two Musketeer variants) turned out to already exist in `Config.CLOTHES_ITEMS` -- a SEPARATE, more complete catalog (keyed by bodyPart, generated by `gen_clothes_lua.py` from `Other/Hair_And_Clothes_Export.xlsx`) that the first pass never checked, having only looked at the older/smaller `Config.CUSTOM_CLOTHES`. **A project with two catalogs covering overlapping ground for the same UI feature is itself worth flagging** -- always confirm which one a given dropdown ACTUALLY reads (grep the consuming code, e.g. `Config.CLOTHES_ITEMS[bodyPartKey]`) before concluding something is absent from "the" catalog.

**After the corrections, exactly two real gaps remained** (a Handyman/Blacksmith clothing set, and a Boatswain-specific heavy belt the user explicitly asked to leave out since it belongs to a special-shaped boss) -- confirming the by-hand correction process, not the original automated pass, is what actually found real, actionable gaps.

**A genuinely separate, real bug found investigating a related complaint** ("the senkamati stuff that is supposed to be hidden unless unlocked are visible in the lists"): `Config.CLOTHES_ITEMS`' `availability` strings correctly marked every Senkamati item `"... - Only with Unlock"`, and `CustomMenu.cpp`'s own dropdown-filtering code (`item.locked && !g_clothesUnlocked`) was correctly implemented -- the bug was neither the data nor the filter logic. `clothes_unlock_state.txt` (a plain one-line status file `Spawner.ToggleClothesUnlock` writes so the compiled C++ side can poll a live Lua toggle it can't otherwise read) was found holding a stale `"1"` on the live install, left over from testing `lbunlockclothes` in an EARLIER session -- while `Config.CLOTHES_UNLOCK_ALL` itself correctly reset to its `config.lua` default (`false`) on every script reload, as a plain Lua variable does. **Nothing ever re-synced the file to match the Lua variable's actual current value except the toggle function itself firing** -- so once unlocked-then-reloaded, the file stays stuck reporting "unlocked" indefinitely, across any number of future reloads or even full game restarts, until someone happens to toggle it again. **General pattern worth remembering for any Lua-writes/C++-polls status-file bridge in this project (there are several: `clothes_unlock_state.txt`, `PublishWindowVisible`, the photo-freeze-time status path, etc.): a plain Lua variable resets to its script default on every reload, but a FILE on disk does not -- any such bridge needs an explicit sync-on-load call, not just a write-on-toggle, or it silently drifts out of sync with its own source of truth across reloads.** Fixed by factoring the write into `Spawner.WriteClothesUnlockState()` and calling it once unconditionally at module load, in addition to on every real toggle.

**Files touched**: `config.lua` (`Config.CLOTHES_ITEMS`: 3 new Blacksmith Feet/Hands/Legs entries, real asset paths confirmed via `pakcontents.xlsx` before adding rather than guessed -- the folder is `.../Blacksmith/Meshes/` lowercase, the asset filenames are `SK_Armor_BlackSmith_*` capitalized, a real casing-duplicate quirk this project has hit before); `spawner.lua` (`Spawner.WriteClothesUnlockState` factored out of `Spawner.ToggleClothesUnlock`, called once at load); `clothes_unlock_state.txt` (live install, manually reset to `"0"` immediately as the live fix, ahead of the code fix taking effect on next reload).

### 19bo. The C++ Custom-tab dropdown is a hand-pasted, hardcoded array baked into the compiled mod DLL -- editing `config.lua` alone does nothing for it, and an incremental `cmake --build` rebuild is fast even though the project also builds the entire UE4SS framework from source (2026-09-25)

Direct follow-on from 19bn's Blacksmith addition: `CustomMenu.cpp` (the separate `LivingBaseSpawnMenu` C++ companion mod) can't read `config.lua` at runtime, so its own Clothes dropdown lists (`kClothesFeetItems[]`/`kClothesHandsItems[]`/etc, `struct ClothesItem { name; locked; }`) are a SEPARATE, hand-maintained copy generated once by `gen_clothes_cpp.py` (same `Other/Hair_And_Clothes_Export.xlsx` source as `gen_clothes_lua.py`) and pasted into the C++ source -- adding an item to `Config.CLOTHES_ITEMS` makes it applicable via console command immediately, but it will never appear as a selectable dropdown ENTRY until the matching C++ array line is added AND the mod is rebuilt and redeployed. Added the 3 new lines by hand (alphabetically positioned, matching the generator's own sort convention) directly in `Working/LivingBaseSpawnMenu/Mod/src/CustomMenu.cpp` rather than re-running the generator against the (unedited) spreadsheet, since only 3 lines were needed.

### 19bp. "Can the Senkamati fight with their own weapons after being made to walk?" -- an attack-animation wall confirmed 4-for-4 across melee/magic/unarmed (but NOT ranged), a real weapon-item-vs-cosmetic-mesh distinction, a self-inflicted ability-strip bug, and four new general-purpose diagnostic commands (2026-09-25)

`lbtestaianim` (19bl) had already proven a native Senkamati Warrior could be made to walk upright by forcing a foreign `AIControllerClass`+`AnimBlueprint` pair onto it. The natural next question: does it still fight with its real weapon? **Answer, after a long, methodical elimination: no, for melee/magic/unarmed -- the attack animation is baked directly into each class's OWN dedicated AnimBP graph, not a generic montage-slot system, and no foreign controller/ABP combination (native Senkamati controller, the Regular crew controller, even the Officer crew controller -- all paired with the same `ABP_BlackBeard_Regular_Sailor`) ever rendered a real attack.** The Warrior only ever blocked; the Caster's spellcast and the Thrall's unarmed strike never fired at all. **Ranged combat is the one exception**: the Hunter's spear-throw worked perfectly with the exact same foreign Officer-controller/Sailor-ABP pairing that failed for melee -- a "fire/throw a ranged weapon" ability is evidently a much more generic, weapon-family-agnostic mechanic (shared across pistols/muskets/thrown spears) than a melee combo, which needs bespoke animation-graph states per weapon family.

**A real equip-vs-cosmetic distinction, confirmed two different ways.** First: `lbtestcombat` (new, see below) proved the Macuahuitl is a genuine, fully-formed `R5MeleeWeaponItem` child object under `EquipmentComponent` (own `Visual`/`Params`/`LogicParams`, identical shape to a real crew member's saber) on EVERY test spawn regardless of controller/ABP override -- ruling out "the weapon never got equipped" as the cause of the block-only Warrior. Second, and more surprising: the *existing shipped* `Config.SENKAMATI_LOOKS` crew-reskin ("the upright Senkamati" feature, `WARRIOR_CREW_BASE = Config.WARRIOR_BASE_CLASS`, an Officer-class crew pawn cosmetically skinned as Senkamati) was ALSO checked with `lbtestcombat` and turned out to be equipped with `BP_Mob_Wpn_Crew_Saber_C` (the crew's own saber weapon item, `DA_Mob_Wpn_Crew_Saber_LogicParams`) the whole time -- the Macuahuitl mesh visible on it is a reskinned Saber, not a real Macuahuitl weapon item at all. **"Visually equipped" and "actually equipped" are two different things in this game's architecture** (a `SkeletalMeshComponent` sitting on a socket vs. a real weapon-item object under `EquipmentComponent` with its own `LogicParams`) -- always check via the weapon-item sweep, never by eye or by `lbsockets` alone, before concluding a class "wields" a specific weapon.

**A real, self-inflicted bug found along the way, independent of the animation-family wall**: `senkaMobFix` (testbed.lua, runs on every menu-spawned Wild/Monsterous Senkamati) called `Spawner.StripAbilities(actor, Config.CASTER_DISABLE_ABILITIES, "Caster")` -- removing any ability whose class name contains `"Summon"` or `"Totem"`, intended to stop her totem from attacking the player. But her totem-summon ability IS her combat opener; removing it entirely left her StateTree with nothing to activate to ever start a fight at all. Confirmed live: `lbspawn` + friendly (bypassing `senkaMobFix`, so the ability stays granted) fights normally; every menu-spawned witch (which DOES run `senkaMobFix`) never fought at all, regardless of the animation-family question. **Fixed by removing the ability-strip entirely** -- the totem-hostility problem itself is solved on the totem's own side instead (19bq).

**Four new general-purpose diagnostic commands came out of this investigation, all reusable well beyond Senkamati:**
- `lbteststatetree` (existing, extended): now also reports `Pawn.AIControllerClass` + the LIVE controller's actual class (can legitimately differ right after an override), and dumps every tag->tree entry in `Params.StateTreeMap` -- not just whichever ONE tree happens to be active -- each with a ready-to-paste `lbwakeai graft <path>` command line already filled in.
- `lbwakeai` (existing, extended): `graft` mode now takes an optional explicit `StateTreeAssetPath` argument (pass `-` or omit for the original auto-lookup/fallback behavior) so a SPECIFIC tree (e.g. a real combat tree found via `lbteststatetree`) can be forced onto a foreign controller instead of whatever it would otherwise pick.
- `lbtestcombat` (new): finds the real equipped weapon-item component (anything with a `LogicParams` field, e.g. `R5MeleeWeaponItem`) on the current target -- the actual "is this armed" state -- plus a filtered function-name sweep of `EquipmentComponent`'s class chain (equip/weapon/add/remove/set) to check whether a live weapon SWAP is even reachable via Lua at all (it isn't here -- see its own header comment: `EquipmentComponent`'s class is `R5NoInventoryEquipment`, exposes zero readable/writable properties, and its only matching functions are generic `ActorComponent` base methods -- no equip entry point exists to call).
- `lbdumpasset <AssetPath> [FieldName]` (new): loads any `/Game/...` asset by path and dumps its own properties -- fills a real gap where every other probe command in this file only ever showed a component's bare REFERENCE to a DataAsset (e.g. `Params = R5AS_AgentParams DA_Foo`), never that asset's own field values. The optional `FieldName` drills one level deeper into a nested object or `TArray` field (dumping every entry, not just an opaque array handle) -- built specifically to inspect `R5AS_TargetSelector`'s own configuration (19bq), the first time this project has needed to read INTO a nested sub-object rather than just resolve a top-level asset path.

**A real bug fixed in `resolveTestDiagActor()` along the way**: it fell back to `Spawner._lastProbedActor` (set only by `lbprobe`/`lbprobedump`/auto-target-on-spawn) but never checked `Spawner.lockedTarget` (the Numpad+ live-edit target lock) -- a SEPARATE tracking mechanism. Targeting something via Numpad+ alone (without ever running `lbprobe`) left every diagnostic built on this shared resolver reporting "stale" even with a real, freshly-locked target present. Fixed by adding `Spawner.lockedTarget.actor` as a third fallback, checked last (an explicit recent spawn/probe still wins over an older lock).

**Files touched**: `spawner.lua` (`resolveTestDiagActor` lockedTarget fallback; `Spawner.TestDumpStateTree`/`Spawner.WakeAI` extensions; new `Spawner.TestDumpCombatComponent`/`Spawner.TestDumpAsset`); `main.lua` (`lbteststatetree`/`lbwakeai` help text; new `lbtestcombat`/`lbdumpasset` console command registrations); `testbed.lua` (`senkaMobFix`'s ability-strip removed); `config.lua` (`Config.CASTER_DISABLE_ABILITIES` removed, now genuinely dead).

### 19bq. A "friendly" summoned totem still attacked friendlies and ignored real enemies -- FModel's export of its own AgentParams revealed a fully-native, non-configurable relationship categorizer, fixed by finally implementing Xenophon's own OwnerId-sync taming technique (2026-09-25)

Background: `Config.CASTER_KILL_TOTEMS` (pre-existing) destroyed the Caster's summoned totem outright on spawn, because it always attacked the player despite her own faction being friendly -- the totem is a separate actor that never inherited her faction. Once 19bp's ability-strip fix let her actually open combat again, destroying the totem was replaced with `Spawner.MakeFriendly(totem, fp)` (the same direct `FactionComponent.FactionsParams` overwrite used everywhere else in this mod) -- this fixed DAMAGE (confirmed: no longer hurts the player or friendly crew) but the totem still visibly "attacked" friendlies (including OTHER friendly Senkamati carrying the exact same faction) and never targeted real enemies at all.

**FModel's JSON export of `DA_Mob_SenkamatiCorrupted_Regular_Shaman_Caster_Totem_AgentParams` found the real mechanism**, invisible to a live Lua property walk (which only ever showed bare references like `TargetSelector = R5AS_TargetSelector_NearAlertAggressive DA_Foo:R5AS_TargetSelector_NearAlertAggressive_0`, never that sub-object's own configured fields). The totem's `TargetSelector` (`R5AS_TargetSelector_NearAlertAggressive`) requires a candidate to carry a dynamically-assigned `AS.Category.Enemy` GameplayTag before it will ever be targeted at all (`AggressiveQuery = ALL(AS.Category.Aggressive, AS.Category.Enemy, AS.Category.Alive)`). That tag is assigned by one of FIVE `Categorizers` re-evaluated every `CollectionTickInterval` (0.3s -- fast enough to rule out a stale-snapshot/timing theory outright). The one that matters, `R5AS_Categorizer_Relationship`, came back with **completely empty `Properties` in the FModel export -- zero Blueprint-exposed configuration, its actual friend/foe logic is 100% hardcoded native C++, no data lever exists to tune it directly.**

**The fix came from the totem's own native defaults, not from more faction tuning.** The totem already carried `bShouldUseOwnerFaction = true` and `Owner = the Caster who deployed it` -- both native, untouched, present before we ever touched anything. That's a strong hint the Relationship categorizer is built to defer to an OWNER's relationships rather than reading a target's own independent `FactionsParams`. The Caster herself already discriminates friend from foe correctly (she fights real enemies, never the player) -- so if the totem could genuinely inherit HER relationship view instead of getting an independently-assigned faction, it should inherit her already-correct behavior too. This is exactly Xenophon's own proven taming recipe (`project_crew_type_exploration` memory, read/explored in an earlier session but never actually implemented anywhere in this mod until now): copy the PLAYER's real `OwnerId` (`PlayerState.AccountData.AccountId`) onto the target's `OwnershipComponent.OwnerId`, keep `bShouldUseOwnerFaction = true` (already true here), call `OnRep_OwnerId()` to force the engine to re-evaluate. Layered ADDITIONALLY on top of the existing `MakeFriendly` faction fix (not replacing it, since that fix is still what keeps damage safe if the OwnerId sync ever turned out to be a no-op for targeting specifically). **Confirmed live: this fixed targeting correctly** -- RedFalcon: "that did it."

**Generalizes directly to the separate "LBE: Summonable Ghost Fighters" mod**, which hits the identical targeting problem on its own Senkamati-based summons (same native `R5AS_*` Agent/Categorizer/TargetSelector system underneath, confirmed the same architecture applies regardless of which mod's Lua does the summoning) -- the OwnerId-sync recipe above is the reusable fix, not anything specific to this totem class.

**General lesson for this project's whole `R5AS_*`/Agent-targeting-system class of problem**: a live Lua property dump only ever shows a component's bare reference to a nested DataAsset or sub-object, never that object's own configured field values -- `lbdumpasset`'s new field-drill mode (19bp) can get ONE level in for plain object/array fields, but a `GameplayTagQuery`'s own internal token stream is a compiled expression tree that generic reflection can't decode into readable tag names at all. FModel's asset export IS able to resolve and print `GameplayTagQuery` contents in plain English (`AutoDescription: " ALL(...)"`) -- reach for FModel specifically whenever a live probe bottoms out at a `GameplayTagQuery`/`GameplayTag` struct field, rather than trying to force more out of live reflection alone.

**Files touched**: `whistle.lua` (`onTotemSpawned` -- `MakeFriendly` kept, new `OwnershipComponent.OwnerId` sync + `OnRep_OwnerId()` call layered on top).

**The project's own CMake build (`Working/LivingBaseSpawnMenu/`, generator "Visual Studio 18 2026", configuration name `Game__Shipping__Win64`) compiles the ENTIRE UE4SS framework plus all its third-party deps from source, not just the mod's own few files** -- but `Output/` already holds every prior build's cached object files/libs, so `cmake --build "Output" --target LivingBaseSpawnMenu --config Game__Shipping__Win64` (naming the SPECIFIC mod target, not a full/default build) only recompiled the one changed `.cpp` file and relinked, finishing in seconds rather than repeating the full framework build. The resulting DLL lands at `Output/Mod/Game__Shipping__Win64/LivingBaseSpawnMenu.dll` and gets copied over the live install's `.../Mods/LivingBaseSpawnMenu/dlls/main.dll` (note the live folder's own generic `main.dll` naming, unrelated to the project's own build-output filename) -- and being a compiled binary, not hot-reloadable Lua, it needs a full game restart (not `lbreload`) to actually take effect.
